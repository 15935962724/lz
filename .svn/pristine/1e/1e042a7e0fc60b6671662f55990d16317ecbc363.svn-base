...
public class Managers extends TeaServlet
{
    ...
    public void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
    {
        try
        {
            ...
            response.sendRedirect("/jsp/community/Managers.jsp" + sb.toString());
/*
            Form form = new Form("foDelete", "GET", "DeleteManager");
            form.add(new HiddenField("Community", s));
            Table table = new Table();
            table.setCaption(j + " " + super.r.getString(teasession._nLanguage, "Manager"));
            if (j != 0)
            {
                boolean flag = true;
                Row row;
                for (Enumeration enumeration = tea.entity.site.Manager.find(s, i, 25); enumeration.hasMoreElements(); table.add(row))
                {
            ...
            printwriter.print(new Button(1, "CB", "CBNew", super.r.getString(teasession._nLanguage, "CBNew"), "window.open('NewManager?Community=" + s + "', '_self');"));
            if (j != 0)
            {
                ...
                printwriter.print(new Button(1, "CB", "CBDeleteAll", super.r.getString(teasession._nLanguage, "CBDeleteAll"), "if(confirm('" + super.r.getString(teasession._nLanguage, "ConfirmDeleteAll") + "')){window.open('DeleteAllManager?Community=" + s + "', '_self');}"));
            }

            Table table1 = new Table();
            table1.setCaption(j + " " + super.r.getString(teasession._nLanguage, "User"));
            {
                boolean flag = true;
                Row row1;
                for (Enumeration enumeration = tea.entity.site.Manager.find(s, i, 25, false); enumeration.hasMoreElements(); table1.add(row1))
                {
                    String s2 = (String) enumeration.nextElement();
