...
<table border="0" cellpadding="0" cellspacing="0" id="table_admin">
  ...
  %>
   <tr id="tableone" class="td05">
    <td >待处理评论信息：<a href="/jsp/talkback/TalkbackLists.jsp"><span style="color:#f00;"><%=count %></span></a></td>
  </tr>
  <%
  	}
  	if(count2>0){
  %>
   <tr id="tableone" class="td06">
    <td >待处理留言信息：<a href="/jsp/type/messageboard/MessageBoardManage.jsp"><span style="color:#f00;"><%=count2 %></span></a></td>
  </tr>
  <%} %>
  <%
  //if(count>0 || count2>0){ 
