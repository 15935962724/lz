...
function fsubmit()
{
  ...
  var len=18;//form1.cavity[0].checked?9:18;
  for(var i=0;i<len;i++)
  {
...
function f_sub()
{
	...
	//{
		...
		if(a2[i].checked)
		  ...
		  sendx("/jsp/type/score/EditScoreMember2_ajax.jsp?act=EditScore2_ajax&role=false&score="+form1.score.value+"&golf="+form1.golf.value+"&fieldid="+f1+"&fieldid2="+f2,
			 function(data)
			 {
...
<input type=hidden name="nexturl" value="/jsp/type/score/ScoresMember.jsp">
...
<br/>
<%
if(score<1)out.print("<input type=button value='上一步' onclick=window.open('/jsp/type/score/EditScoreMember1.jsp?"+request.getQueryString()+"','_self') />");
%>
