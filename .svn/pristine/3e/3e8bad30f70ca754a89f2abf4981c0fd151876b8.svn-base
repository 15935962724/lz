...
//采购单
String purid = "0";
if(teasession.getParameter("purid")!=null&&teasession.getParameter("purid").length()>0)
{
  purid = teasession.getParameter("purid");
}

sql.append(" AND gd.paid ="+DbAdapter.cite(purid.trim()));
param.append("&purid=").append(purid);
...
count = GoodsDetails.countpur(teasession._strCommunity,sql.toString());
...
function f_c()
{
  var rs = window.showModalDialog('/jsp/erp/rgPurchase.jsp?waridname='+form1.waridname.value,self,'edge:raised;scroll:0;status:0;help:0;resizable:0;dialogWidth:857px;dialogHeight:655px;');
  if(rs!=null){
   form1.purid.value = rs;
  }
}
...
<tr>
<td>
  <font color="#9F0A0E"><b>请填写采购单号查询出要退货的商品:</b></font><input type="text" name="purid" value="<%if(purid!=null&&!"0".equals(purid))out.print(purid);%>">
  <input type="button" value="..." onclick="f_c();">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
java.util.Enumeration e = GoodsDetails.findPur(teasession._strCommunity,sql.toString(),pos,pageSize);

   if(!e.hasMoreElements())
   {
     if("0".equals(purid))
     {
       out.print("<tr><td colspan=10 align=center>请先填写采购单的订单号,才能查询到商品!</td></tr>");
     }else
     {
...
while(e.hasMoreElements())
{
 ...
 Inventory iobj = Inventory.find(Inventory.getInvid(gdobj.getCommunity(),nodeid,waridname));
  ...
  <tr onMouseOver="javascript:this.bgColor='#BCD1E9'" onMouseOut="javascript:this.bgColor=''" title="点击选择商品"  onclick="f_button('/<%=nodeid%>/<%=purid%>/');"  style=cursor:pointer>
    <td width="1"><%=is %></td>
