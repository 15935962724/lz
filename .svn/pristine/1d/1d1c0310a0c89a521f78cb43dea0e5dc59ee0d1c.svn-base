...
<tr>
  <td>道号/昵称:<input name="member" value="<%=member%>"/></td>
<!--
  <td>工作地点:<script>mt.city('city0','city1',null,'<%=city%>')</script></td>
<tr>
-->
  <td>注册时间:<input name="times" value="<%=MT.f(times)%>" size="12" onClick="mt.date(this)"/> 至 <input name="timee" value="<%=MT.f(timee)%>" size="12" onClick="mt.date(this)"/></td>
...
}else
{
  ...
  <tr onMouseOver="bgColor='#FFFFCA'" onMouseOut="bgColor=''">
    ...
    <td>
       <input type="button" value="详细" onClick="f_act('view','<%=mid%>')"/>
...
function f_act(a,id,b,c)
{
  ...
  if(a=='dels')
  {
	 //form2.action="/jsp/admin/edn_ajax.jsp";
   // mt.show("确认要删除吗？",2,"form2.submit()");
    var tm=document.getElementsByName("members");
    var msm="/";
    ...
    sendx("/jsp/admin/edn_ajax.jsp?members="+msm+"&act="+a,function(data){
    	mt.show(data,1,window.location.href);
    });
  }else if(a=='edit')
  {
  ...
  {
    form2.action="/jsp/custom/daoxie/MemberView.jsp"; form2.method="get"; form2.target="_self"; form2.submit();
  }else if(a=='clear')
  {
