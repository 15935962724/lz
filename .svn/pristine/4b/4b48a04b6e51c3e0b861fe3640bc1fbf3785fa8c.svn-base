...
<%@ page import="tea.ui.*" %><%@ page import="tea.resource.*" %><%@ page import="tea.entity.custom.jjh.*" %>
<%
	...
	int nid=Integer.parseInt(tea.getParameter("vtid"));
	String nexturl=request.getParameter("nexturl");
	String nname="";
	if(nid>0){
		Voltype np=Voltype.findVoltype(nid);
		if(np.getVtname()!=null){
			nname=np.getVtname();
		}
	}
...
<html>
<head>
<title><%=r.getString(tea._nLanguage, "EditNewexperts") %></title>
<base target="dialog"/>
<link href="/res/<%=tea._strCommunity%>/cssjs/community.css" rel="stylesheet" type="text/css">
<script src="/tea/tea.js" type="text/javascript"></script>
</head>
<body>
<h1><%=r.getString(tea._nLanguage, "EditNewexperts") %></h1>
<div id="head6"><img height="6" src="about:blank"></div>
<form action="/EditVoltype.do" name="form1" target="dialog">
	<input type="hidden" name="vtid" value="<%=nid%>">
	<input type="hidden" name="act" value="edit">
	<input type="hidden" name="nexturl" value="<%=nexturl %>" >
	<table id="tablecenter">
		<tr>
			<td ID=RowHeader><%=r.getString(tea._nLanguage, "names") %>：</td>
			<td><input type="text" value="<%=nname %>" name="vtname"></td>
		</tr>
	</table>
