...
{
  if(act=="ajax_opinion")dt="/82,83,84/173,174,175/162,78,163/164,79,165/";
...
function f_code()
{
  form1.dynamictype76.value="中电控股"+$('fi_c').value+"["+$('fi_y').value+"]"+$('fi_s').value+"号";
}
function f_load()
{
  //如果备选文件大于1个，则弹出对话框。
  if(!form1.dynamictype85)return;
  var v=form1.dynamictype85.value;
  if(v.charAt(0)==':')
  {
    if(v.charAt(v.length-1)==':')v=v.substring(0,v.length-1);
    sel=v.split(":").length==2?v.substring(1):window.showModalDialog('/jsp/admin/flow/SetFlowbusinessDocs.jsp?community=' + form1.community.value + '&field=dynamictype85&t='+new Date().getTime(),self,'dialogWidth:360px;dialogHeight:280px;help:0;status:0;scroll:0;');
    form1.dynamictype85.value=sel;
  }
}
...
if(bw||br)
{
  ...
  if(authority)
  {
    out.println("<input type='button' value='盖章' onclick='form1.dynamictype_button85.onclick()' >");
    //保留未盖章的文件
  ...
  //if(step==9)out.println("<input type='button' value='下载正文' onclick=\"window.showModalDialog('/jsp/admin/flow/DownDynamicOffice.jsp?flowbusiness="+flowbusiness+"&file='+f_split(form1.dynamictype" + dynamictype+".value),self,'dialogWidth:400px;dialogHeight:100px;resizable:1;help:0;status:0;scroll:0;');\" >");
}

if(dtw.indexOf("/down/")!=-1)
out.println("<input type='button' value='下载' onclick=location.href='/servlet/EditFlowdynamicvalue?act=down&community='+form1.community.value+'&flowbusiness='+form1.flowbusiness.value+'&dynamictype=/85/166/' >");
...
if(dtw.indexOf("/82/")!=-1)//领导批示
  out.println("<input type='button' value='审阅意见' onclick=f_opinion('/82/83/84/') />");

if(dtw.indexOf("/173/")!=-1)//领导批示2
  out.println("<input type='button' value='审阅意见' onclick=f_opinion('/173/174/175/') />");

if(dtw.indexOf("/162/")!=-1)//审核人意见
  out.println("<input type='button' value='审核人意见' onclick=f_opinion('/162/78/163/') />");

if(dtw.indexOf("/164/")!=-1)//复核人意见
  out.println("<input type='button' value='复核人意见' onclick=f_opinion('/164/79/165/') />");

//会签
if(dtw.indexOf("/81/")!=-1)
{
  out.println("<input type='button' value='填写意见' onclick='f_csign(81)' />");
  sbcheck.append("&&submitSign(form1.dynamictype81_2)");
}
//发文字号
//if(dtw.indexOf("/76/")!=-1)
//{
//  out.println("<input type='button' value='文件字号' onclick=\"window.showModalDialog('/jsp/admin/cec/SetFlowbusinessSN_1031.jsp?community='+form1.community.value+'&flowbusiness='+form1.flowbusiness.value+'&issuecode='+encodeURIComponent(form1.dynamictype122?form1.dynamictype122.value:$('td122').innerHTML)+'&code='+encodeURIComponent($('td76').innerHTML)+'&dynamictype=76&t='+new Date().getTime(),self,'dialogWidth:350px;dialogHeight:105px;resizable:1;help:0;status:0;');\" />");
//}
//归档
...
function f_send()
{
  if(<%=isNew%>)
  {
    var dt85=form1.dynamictype85;
    var v=dt85.value;//正文
    if(v.indexOf(":")!=-1||dt85.defaultValue.indexOf(v)!=-1)
    {
...
<tr><td>
<input type="text" name="dynamictype82" disabled="disabled" />
<input type="text" name="dynamictype83" disabled="disabled" />
<input type="text" name="dynamictype84" disabled="disabled" />

<input type="text" name="dynamictype173" disabled="disabled" />
<input type="text" name="dynamictype174" disabled="disabled" />
<input type="text" name="dynamictype175" disabled="disabled" />

<input type="text" name="dynamictype162" disabled="disabled" />
<input type="text" name="dynamictype78" disabled="disabled" />
<input type="text" name="dynamictype163" disabled="disabled" />

<input type="text" name="dynamictype164" disabled="disabled" />
<input type="text" name="dynamictype79" disabled="disabled" />
<input type="text" name="dynamictype165" disabled="disabled" />
</table>
<!-- -end-->
...
<table id="tabbase"  <%if(ist.equals("1"))out.print("style='display:none'");%>>
  <tr>
    ...
    <td id="td75"><input type="text" name="dynamictype75" id="dynamictype75"/></td>
  </tr>
  <tr>
    ...
    if(dtw.indexOf("/76/")!=-1)
    {
      String v76=DynamicValue.find(-flowbusiness,teasession._nLanguage,76).getValue();
      if(MT.f(v76).length()<1)v76="综字["+Calendar.getInstance().get(Calendar.YEAR)+"]"+SeqTable.getSeqNo("flowbusiness.sn")+"号";
      //
      int is=v76.indexOf('['),ie=v76.indexOf(']',is);
      String fi_c=v76.substring(0,is);
      String fi_y=v76.substring(is+1,ie);
      String fi_s=v76.substring(ie+1,v76.length()-1);
      out.print("<select id='fi_c' onchange='f_code()'>");
      ...
      out.print("<input type='hidden' name='dynamictype76' value=\""+v76+"\"/>");
    }
    %></td>
  </tr>
  <tr>
...
<%--
 <tr>
    ...
    <tr class="out">
    <td  class="tabtdteshu" id="td122">
    <%
    out.print("<select name='dynamictype122'>");
    dt=DynamicType.find(122);
    ...
    <td id="td76" onpropertychange="var dt76=form1.dynamictype76; if(dt76.value!=innerHTML)dt76.value=innerHTML;"></td>
    </tr>
    </table>
    </td>
    <input type='hidden' name='dynamictype76' onpropertychange="var td76=document.getElementById('td76'); if(td76.innerHTML!=value) td76.innerHTML=value;" />
  </tr>
--%>
  <tr>
    <td class="tdtitle">签报人</td>
    <td style="padding:0px;border:0px;">
    <table id="tabbase_nei" border="0" cellspacing="0" cellpadding="0">
    <tr>
    <td  class="tabtdteshu" id="td77"><input type="text" name="dynamictype77"  /></td>
  ...
  <tr>
    <td class="tdtitle">复核</td>
    <td style="padding:0px;border:0px;">
    <table id="tabbase_nei" border="0" cellspacing="0" cellpadding="0">
    <tr>
    <td  class="tabtdteshu" id="td79"></td>
    <td class="tdtitle">日期</td>
    <td id="td80"><input type="text" name="dynamictype80" onclick='showCalendar(this,true)' readonly='true'  /></td>
    </tr>
    </table>
    </td>
  </tr>
...
{
%>
<area shape="rect" coords="140,4,244,31" href="###" id="1"/>
<area shape="rect" coords="143,75,244,103" href="###" id="2"/>
<area shape="rect" coords="144,149,244,176" href="###" id="3"/>
<area shape="rect" coords="143,223,243,250" href="###" id="6"/>
<area shape="rect" coords="143,295,245,322" href="###" id="4"/>
<area shape="rect" coords="11,370,112,396" href="###" id="5"/>
<area shape="rect" coords="143,368,244,396" href="###" id="7"/>
<%
}else
{
%>
<area shape="rect" coords="14,2,115,29" href="###" id="1"/>
<area shape="rect" coords="14,73,113,101" href="###" id="2"/>
<area shape="rect" coords="14,146,114,173" href="###" id="3"/>
<%
}%>
<%--
