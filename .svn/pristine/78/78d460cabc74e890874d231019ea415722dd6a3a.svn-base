...
public class InsertStock extends TeaServlet
{
	...
	public void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		try
		{
			...
			Date date1 = TimeSelection.makeTime(teasession.getParameter("Issue1Year"), teasession.getParameter("Issue1Month"), teasession.getParameter("Issue1Day"));
			Date date2 = TimeSelection.makeTime(teasession.getParameter("Issue2Year"), teasession.getParameter("Issue2Month"), teasession.getParameter("Issue2Day"));
			Date date3 = TimeSelection.makeTime(teasession.getParameter("Issue3Year"), teasession.getParameter("Issue3Month"), teasession.getParameter("Issue3Day"));
			Date date4 = TimeSelection.makeTime(teasession.getParameter("Issue4Year"), teasession.getParameter("Issue4Month"), teasession.getParameter("Issue4Day"));
			DbAdapter db = new DbAdapter();
			try
			{
				db.executeUpdate("insert into stock(stockid,stockname,openingprice,date,high,low,closingprice,percentchange,volume,estockname) values ('ceon'," + DbAdapter.cite("0883.HK 中海油香港") + "," + f11 + "," + DbAdapter.cite(date1) + "," + f12 + "," + f13 + "," + f14 + "," + f15 + "," + f16 + ",'0883.HK CNOOCLTD. HONGKONG')");
				db.executeUpdate("insert into stock(stockid,stockname,openingprice,date,high,low,closingprice,percentchange,volume,estockname) values ('600583ss'," + DbAdapter.cite("CEO.N 中海油 纽约") + "," + f21 + "," + DbAdapter.cite(date2) + "," + f22 + "," + f23 + "," + f24 + "," + f25 + "," + f26 + ",'CEO.N CNOOCLTD. NEWYORK')");
				db.executeUpdate("insert into stock(stockid,stockname,openingprice,date,high,low,closingprice,percentchange,volume,estockname) values ('2883hk'," + DbAdapter.cite("600583.SS 海油工程A股") + "," + f31 + "," + DbAdapter.cite(date3) + "," + f32 + "," + f33 + "," + f34 + "," + f35 + "," + f36 + ",'600583.SS CNOOC Engineering Ltd. SHANGHAI')");
				db.executeUpdate("insert into stock(stockid,stockname,openingprice,date,high,low,closingprice,percentchange,volume,estockname) values ('0883hk'," + DbAdapter.cite("2883.HK 中海油田服务") + "," + f41 + "," + DbAdapter.cite(date4) + "," + f42 + "," + f43 + "," + f44 + "," + f45 + "," + f46 + ",'2883.HK COSL HONGKONG')");
