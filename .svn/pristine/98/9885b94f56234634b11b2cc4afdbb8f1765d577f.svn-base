...
ShopCategory cg = ShopCategory.find(category);
...
<div class="maindiv clt">
    ...
    <div class="mrxia">
    <div class=""><span><%=cg.name[1]%></span></div>
    <%
    ...
    out.print("<li><span class='pic'><a href='/html/folder/14110010-1.htm?product=" + pd.product + "' target='_blank'><img src='" + pd.getPicture('s') + "' onload='AutoResizeImage(195,140,this)'  /></a></span>");
    out.print("<div class='tit'><div class='titout'><b class='tt'>"+pd.name[1]+"</b>");
    out.print("<span class='number'>商品编号："+MT.f(pd.yucode)+"</span></div>");
    out.print("<div class='texts' title='"+Lucene.t(pd.content[1])+"'>"+ MT.f(Lucene.t(pd.content[1]),258)+"&nbsp;&nbsp;<a href='ShopProductBuy.jsp?product="+pd.product+"'>>了解更多</a></div>");
    ...
    out.print("<div class='ord'><div class='price'>销售价格：<strong>&yen;"+price+"元</strong></div>");
