...
<body>
<h1><%=r.getString(teasession._nLanguage, "cnoocjobtalkback")%></h1>
...
<%@include file="/jsp/type/job/cnoocjobheader.jsp" %>
 <TABLE CELLSPACING=0 CELLPADDING=0 border=0 class="section">
    ...
    <TR>
      ...
      <TD> <IMG BORDER=0 SRC="/tea/image/hint/<%=pic%>.gif"><%=subject%></TD>
    </TR>
  </TABLE>
  <br>
  <%=content%>
  ...
  <input type=button class="edit_input" onClick="window.open('<%=file%>','_self');" value="<%=r.getString(teasession._nLanguage, "Download")%>" >
  <%  }
    %>
  <HR SIZE=1>
  <%
