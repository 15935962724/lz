...
<div id="pathdiv"><%=ts.hrefGlance(teasession._rv)%> ><A href="/servlet/SaleOrders"><%=r.getString(teasession._nLanguage, "SaleOrders")%></a> ><%=r.getString(teasession._nLanguage, Trade.TRADE_TYPE[i])%></div>
  <ul>
    ...
    {
      int k1 = Trade.count(false, teasession._rv, i, j1,community);
      if(k1 != 0)
      {%>
    <LI><A href="/servlet/SaleOrders?Type=<%=i%>&Status=<%=j1%>"><%=k1%> <%=r.getString(teasession._nLanguage, Trade.TRADE_STATUS[j1])%> <%=r.getString(teasession._nLanguage, "SaleOrders")%></A> </LI>
    <%}
  }%>
  </UL>
  <div id="head6"><img height="6" src="about:blank"></div>
