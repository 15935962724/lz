...
$(function(){
    ...
    if(isWeiXin()){
        ...
        if(openid!=undefined && openid!=""){
            ...
            alert(222)
            location.href = '/PhoneProjectReport.do?act=openid&nexturl='+location.pathname+location.search;
        }
    }else {
        location.href ='/mobjsp/yl/user/login_mob.html';
    }
    checklogin();//验证登陆
})
//验证登陆状态
...
function accountlogin(){
    ...
    $.post("/LzLogins.do",{"act" : "accountLogin","username" : username,"password":password,"verid":verid,"isWX":isWeiXin()},function (result) {
        var data = eval("(" + result + ")");
        if(data.type > 0) {
            YKRJ.show("提示",data.mes,1);
        } else {
            var wuser = {};
            wuser.profile = data.profile;
            wuser.username = data.username;
            wuser.mobile = data.mobile;
...
function messagelogin(){
    ...
    $.post("/LzLogins.do",{"act" : "moblieLogin","mob" : mob,"verid":verid,"isWX":isWeiXin()},function (result) {
        var data = eval("(" + result + ")");
        if(data.type > 0) {
            YKRJ.show("提示",data.mes,1);
        } else {
...
function accountVertify(){
    ...
    if (!messageflag) {
        ...
        messageflag = false;
        $.post("/LzLogins.do", {"act": "getVertify", "username": $("#username").val(),"type":"account"}, function (result) {
            var obj = eval("(" + result + ")");
            if (obj.code == '0') {
                accountMinutimer(60);
            } else {
...
function messageVertify(){
    ...
    if(mobile == ""){
        ...
        if (!mobmessageflag) {
            ...
            $.post("/LzLogins.do", {"act": "getVertify", "mob": mobile,"type":"message"}, function (result) {
                var obj = eval("(" + result + ")");
                if (obj.code == '0') {
                    messageMinutimer(60);
                } else {
