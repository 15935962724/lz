...
if(q!=null)
{
  sql.append(" AND author LIKE "+DbAdapter.cite("%"+q+"%"));
  par.append("&q=").append(java.net.URLEncoder.encode(q,"UTF-8"));
}
par.append("&pos=");

int sum=ReportMember.count("",sql.toString());

String title=r.getString(teasession._nLanguage, "选择姓名...");
...
<table border="0" cellpadding="0" cellspacing="0" id="tablecenter">
  <tr>
    <td>姓名:<input name="q" value="<%if(q!=null)out.print(q);%>" type="text"><input type="submit" value="GO"></td>
  </tr>
</table>
...
<%
Enumeration e=ReportMember.findByCommunity(teasession._strCommunity,sql.toString(),pos,10);
for(int i=0;e.hasMoreElements();i++)
{
  int j=((Integer)e.nextElement()).intValue();
  ReportMember robj=ReportMember.find(j);
  out.print("<tr onMouseOver=bgColor='#BCD1E9' onMouseOut=bgColor='' style='cursor:hand' onclick=\"f_click('"+robj.getAuthor()+"')\">");
  out.print("<td>"+robj.getAuthor());
  out.print("<td>"+robj.getTimes());
}
if(sum>10)
{
