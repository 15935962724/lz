...
if(totals!=null&&(totals=totals.trim()).length()>0)
{
  out.print("<input type=hidden name=totals value="+totals+">");
  %>
<h2>积分</h2>
<table cellpadding="0" cellspacing="0" bordercolor="0" id="tablecenter">
  <tr>
    <td height="32">
    <input type="hidden" name="point" value="<%=points.intValue()%>" />
      您本次购买将获得本站积分<%=points.intValue()%>分
    </td>
  </tr>
</table>
<%
     ...
     <%--
      ...
      if(state!=null&&state.length()>0)
      {
        ...
        for(int bstate_i=0;bstate_i<Common.PROVINCE.length;bstate_i++)
        {
          ...
          {
            out.print(r.getString(teasession._nLanguage,Common.PROVINCE[bstate_i]));
            break;
          }
        }
      }
...
out.print("<input type=hidden name=ps value="+ps+"><tr><td>"+Trade.PS_TYPE[ps]);

switch(ps)
{
case 0:
	out.print(" 如果您不急于使用此商品可以考虑选择平邮，此种方式邮购费由本公司负责；");
	break;
case 1:
	out.print("此方式邮递时间很短，但需费需要自己缴纳，切需要与我们联系； ");
	break;
case 2:
	out.print("邮政特快专递服务，是中国邮政的一个服务产品，主要是采取空运方式，加快递送速度，一般来说根据地区远近，1－4天到达。");
	break;
}
String freight="0.00";

String card=String.valueOf(supplier.getCard());
//if(card.startsWith(city)||(("11".equals(state)||"12".equals(state)||"31".equals(state))&&card.startsWith(state)))
//{
//	freight="20.00";
//}
out.print("<input type=hidden name=freight value="+freight+">");
%>
</table>
...
<%
int defray=Integer.parseInt(request.getParameter("defray"));
//out.print("<input type=hidden name=defray value="+defray+"><tr><td>"+Trade.DEFRAY_TYPE[defray]);
 ...
 {
   out.print(Payinstall.PAYNAME[piobj.getPay()]);
 }else if(piobj.getPaytype()==2)
 {
   out.print(piobj.getPayname());
 }
 out.print("</td></tr>");
...
if(fp!=null&&fp.length()>0)
{
  ...
  if("false".equals(fp))
  {
    out.print("<input type=hidden name=fp value="+0+">");
    out.print("<tr><td nowrap>不要发票</td><td></td></tr>");
  }
  else
  if("true".equals(fp))
  {
    out.print("<input type=hidden name=fp value="+1+">");
    out.print("<tr><td nowrap>要发票</td><td></td></tr>");
  }
}
%>
<tr><td><input type="hidden"  name="fp" value="<%=fp%>"/></td></tr>
</table>
