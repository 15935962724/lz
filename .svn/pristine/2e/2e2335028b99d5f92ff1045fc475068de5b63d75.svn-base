...
if(!f.exists())
{
  response.sendRedirect("/mobjsp/info/Alert.jsp?info="+Http.enc("没有发现索引文件"));
  return;
}
...
try
{
  ...
  for(int i=pos;i<pos+20&&i<hits[type].length();i++)
  {
    ...
    out.print("<tr><td>["+Node.find(father).getAnchor(h.language)+"] <a href='/xhtml/"+h.community+"/"+(n.getType()>=1024?"node":Node.NODE_TYPE[n.getType()].toLowerCase())+"/"+node+"-"+h.language+".htm'>"+subject+"</a>　　点击："+n.getClick()+"　　日期："+MT.f(time));
