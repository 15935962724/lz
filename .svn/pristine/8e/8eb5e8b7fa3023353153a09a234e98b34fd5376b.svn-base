...
<%
 if (!teasession._rv.toString().equalsIgnoreCase("cnooc") && !License.getInstance().getWebMaster().equals(teasession._rv._strR)) {
...
dbadapter.close();
response.sendRedirect("/jsp/community/cnoocjobsubscribers.jsp?Community=cnoocjob&Pos=0");
%>
