...
if(tmp!=null)
{
  c.set(c.DATE,Integer.parseInt(tmp));
	 ...
	 c.set(c.DATE,cdate.getDate()-1); //查看前一天的数据
...
String url = cache.getProperty("0Url");  //联表查询（不同数据库），取当前库的另外一个数据库url
String databaseName = url.substring(url.indexOf("=")+1);  //数据库名称
...
if ((year==cdate.getYear()+1900) && (month== cdate.getMonth()+1))
{alltotal=tea.entity.node.access.NodeAccessLast.countHotNodes(community,year,month,day,columnodeSql);
icount=tea.entity.node.access.NodeAccessLast.itemHotNodes(community,year,month,day,columnodeSql);
}
else
{	alltotal=tea.entity.node.access.NodeAccessLast.countHotNodes(community,year,month,columnodeSql);
//icount=tea.entity.node.access.NodeAccessLast.itemHotNodes(community,year,month,columnodeSql);
...
StringBuffer param =new StringBuffer("?node="+teasession._nNode+"&community="+teasession._strCommunity+"&act=hot");
...
<table border="0" cellspacing="0" cellpadding="0" id="tablecenter">
  <tr>
    <td> 　<h2><%=r.getString(teasession._nLanguage,"1216027517250")%></h2></td>
  </tr>
<tr><td>
...
if ((year==cdate.getYear()+1900) && (month== cdate.getMonth()+1)){
list=	NodeAccessLast.findHotNodes(community,year,month,day,columnodeSql,pos,pageSize);
}else
	//list=NodeAccessLast.findHotNodes(community,year,month,columnodeSql,pos,pageSize);
	list=NodeAccessLast.findHotNodesByMonth(community,year,month,columnodeSql,pos,pageSize);
...
while (iter.hasNext())
{
  ...
  out.print("<td nowap align='right'>"+df2.format(p)+"%</td>");
  out.print("<td align=left valign=middle width=100><img src='/tea/image/other/bar2.gif' style='width:"+w+"%' height=15></td>");
  out.print("</tr>");
}
if(showcount>0)
{
%>
    <tr><td nowap >
     <%=new tea.htmlx.FPNL(teasession._nLanguage, "/jsp/count/index.jsp?act=hot&year="+year+"&month="+month+"&day="+day+"&community=" + community + "&pos=", pos,showcount,20)%>
</td></tr>
