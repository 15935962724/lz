...
private String tree_0(Node n,int lang,String target,String role,String member)throws Exception
{
  ...
  StringBuilder h=new StringBuilder();
	  ...
	  h.append(" />");
	  if(n.getType()==1){
	 	 h.append("<a href='/jsp/general/ContributorsNodeLists.jsp?node="+j+"' target='"+target+"'>"+n.getSubject(lang)+"</a>");
...
String target="m";//target="function_fun";
...
AdminUsrRole arobj = AdminUsrRole.find(teasession._strCommunity,teasession._rv.toString());
	...
	out.print("window.open('/jsp/general/ContributorsNodeLists.jsp?node="+teasession._nNode+"', 'm');"); 
	out.print("</script>");
...
function f_c(j)
{
  ...
  }else
  {
    if(d.innerHTML.length==0)
    {
      ...
      sendx("/jsp/admin/ContributorsNodeTree.jsp?ajax=ON&node="+j,function(data)
      {
        if(data=="")data="暂无";
        d.innerHTML=data;
      } );
...
if(isFrame)
{
  out.print("<frameset cols='208,*' frameborder='no' border='0' framespacing='0'><frame src='/jsp/admin/ContributorsNodeTree.jsp?node="+teasession._nNode+"&target=m2' /><frame src='about:blank' name='m2' /></frameset>");
  return;
}
%>
</head>
...
if(ns==null)
{
  ...
  out.print("<a href=\"/jsp/general/ContributorsNodeLists.jsp?node="+teasession._nNode+"\" target="+target+">"+AdminFunction.find(afid).getName(teasession._nLanguage)+"</a>");//<a href='/jsp/general/NodeLists.jsp?node="+teasession._nNode+"' target='"+target+"'>
  out.print(tree_1(teasession._nNode,teasession._nLanguage,order,target,role,teasession._rv._strR));
}else
{
