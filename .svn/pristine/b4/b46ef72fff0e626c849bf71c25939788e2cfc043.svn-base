...
{
    for(Enumeration enumeration = tea.entity.admin.SFAccount.findByFather(nodecode); enumeration.hasMoreElements(); )
    {
        int j = ((Integer)enumeration.nextElement()).intValue();
        tea.entity.admin.SFAccount node1 = tea.entity.admin.SFAccount.find(j);
...
tea.entity.admin.SFAccount node1 = tea.entity.admin.SFAccount.find(rootid);
%>
<A target=t href=EditSFAccount.jsp?id=<%=rootid%>><%=node1.getName(teasession._nLanguage)%></A><br/>
<%tree1(out,rootid,0);%>
