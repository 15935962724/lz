...
if(member!=null && member.length()>0){}else
	{
		member = teasession._rv._strR;
		memberorder = MemberOrder.getMemberOrder(teasession._strCommunity,MemberType.getMemberType(teasession._strCommunity,member),member);
		membertype = MemberType.getMemberType(teasession._strCommunity,member);
	} 
String act = teasession.getParameter("act");
String actclssn = teasession.getParameter("actclssn");
 
if(membertype==0 || member==null){ 
	  out.print("参数不正确，请于管理员联系");
	  return;
}
...
<h1><%=r.getString(teasession._nLanguage,"会员信息管理") %></h1>
<script>
...
<input type="hidden" name="act" value="ClssnEditMember"/>
...
<tr> 
  ...
  <td><input type="text" name="EnterPassword" autocomplete="off"  value="<%if(EnterPassword!=null)out.print(EnterPassword);%>"/>&nbsp;&nbsp;<span id="passwordshow">&nbsp;</span>
  </td>
  <td >
    <%
...
<tr>
  ...
  <td><input type="text" name="ConfirmPassword" autocomplete="off"  value="<%if(EnterPassword!=null)out.print(EnterPassword); %>"/>&nbsp;&nbsp;<span id="confirmpasswordshow">&nbsp;</span></td>
   <td>
...
if(riobj.isRegister("identitys"))
{
  ...
  for(int i=0;i<Profile.IDENTITYS_TYPE.length;i++)
  {
	  ...
	  if(identitys==i)out.print(" selected ");
...
%>
<%if("member_audit".equals(teasession.getParameter("actclssn"))){ %>
       <tr>
        <td align="right"><font color="red">*</font>&nbsp;审核状态：</td>
        <td><select name="member_audit" ><!-- 20111013上午 李云鹏 让隐藏掉onchange="f_member_audit();" -->
        ...
        <!--  
        &nbsp;<span id=periodid style="display:none">
        	会员有效期： <input id="time_c" name="becometime" size="7"  value=""  style="cursor:pointer" readonly="readonly" onclick="new Calendar().show('frmsrch.becometime');"> 
	        <img style="margin-bottom:-8px;cursor:pointer;"  src="/tea/image/bbs_edit/table.gif"   onclick="new Calendar().show('frmsrch.becometime');" />
	         -->
        </td>
      </tr>
      <%}else{//编辑使用
...
AdminUsrRole aurobj = AdminUsrRole.find(teasession._strCommunity,teasession._rv._strR);
	...
	if(aurobj.getRole()!=null && aurobj.getRole().length()>4)
	{
		   		if(moobj.getPeriod()>0){
		   			out.print("<input type=button value=\"继续升级\" onclick=\"f_Upgrade('"+memberorder+"');\"  style=\"width:80px;height:20px;margin:10px 0;\">");
		   		}else{
		   			//out.print("<a href=### 	onclick=f_Upgrade('"+memberorder+"');>升级会员</a>");
		   			out.print("<input type=button value=\"升级会员\" onclick=\"f_Upgrade('"+memberorder+"');\"  style=\"width:80px;height:20px;margin:10px 0;\">");
		   		}
	}
		      
		   %>
<input type="button" value="<%=r.getString(teasession._nLanguage,"返回") %>" onClick="javascript:history.go(-1);" style="width:80px;height:20px;margin:10px 0;""/>
</form>
