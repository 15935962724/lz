...
if("POST".equals(request.getMethod()))
{
  ...
  }else//转下一步骤
  {
    ...
    String url="/jsp/admin/flow/EditFlowdynamicvalue.jsp?community="+teasession._strCommunity+"&flowbusiness="+flowbusiness+"&flow="+flow+"&nexturl="+java.net.URLEncoder.encode(nexturl,"UTF-8");
    //
...
//      }
      sb.append("<a href='http://"+request.getServerName()+":"+request.getServerPort()+url+"' target='m' onclick='opener=null;window.close();'>点这里进行办理</a>");
...
//        }
        ...
        Message.create(teasession._strCommunity,5,member,mes.toString(),"/","/",0,url,teasession._nLanguage,subject,sb.toString());
      }
    }
  ...
  if("true".equals(teasession.getParameter("start")))
  {
    ...
    if(newfb>0)nexturl="/jsp/admin/flow/EditFlowdynamicvalue.jsp?flow="+10+"&flowbusiness="+newfb+"&nexturl="+Http.enc(nexturl);
  }
...
{
  window.open('about:blank','fb_dl_add','width=450px,height=350px,top=300px,left=400px');
