...
<%
	...
	int nid=Integer.parseInt(tea.getParameter("eid"));
...
<form action="/EditNewexperts" name="form1" target="dialog">
	<input type="hidden" name="eid" value="<%=nid%>">
	<input type="hidden" name="act" value="<%=nid>0?"edit":"add" %>">
	<input type="hidden" name="nexturl" value="<%=nexturl %>" >
	<table id="tablecenter">
		<tr>
			<td ID=RowHeader><%=r.getString(tea._nLanguage, "names") %>：</td>
			<td><input type="text" value="<%=nname %>" name="ename"></td>
		</tr>
	</table>
