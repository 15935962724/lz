...
public class Crypt
{
	...
	int E_p[] = { 32, 1, 2, 3, 4, 5, 4, 5, 6, 7, 8, 9, 8, 9, 10, 11, 12, 13, 12, 13, 14, 15, 16, 17, 16, 17, 18, 19, 20, 21, 20, 21, 22, 23, 24, 25, 24, 25, 26, 27, 28, 29, 28, 29, 30, 31, 32, 1 };

	/* Permutation Choice 1 for subkey generation (64/56 to 56) */
	int PC1_p[] = { 57, 49, 41, 33, 25, 17, 9, 1, 58, 50, 42, 34, 26, 18, 10, 2, 59, 51, 43, 35, 27, 19, 11, 3, 60, 52, 44, 36, 63, 55, 47, 39, 31, 23, 15, 7, 62, 54, 46, 38, 30, 22, 14, 6, 61, 53,
			45, 37, 29, 21, 13, 5, 28, 20, 12, 4 };

	/* Permutation Choice 2 for subkey generation (56 to 48) */
	int PC2_p[] = { 14, 17, 11, 24, 1, 5, 3, 28, 15, 6, 21, 10, 23, 19, 12, 4, 26, 8, 16, 7, 27, 20, 13, 2, 41, 52, 31, 37, 47, 55, 30, 40, 51, 45, 33, 48, 44, 49, 39, 56, 34, 53, 46, 42, 50, 36, 29,
			32 };
	...
	/*
	 * Selection blocks There are 8 sblocks, each of which is referenced by a 2 bit value which picks the row, and a 4 bit value which picks the column This number is then the 4 bit output for that
	 * select block
	 */
	int sblocks[][][] = {
	...
	int P_p[] = { 16, 7, 20, 21, 29, 12, 28, 17, 1, 15, 23, 26, 5, 18, 31, 10, 2, 8, 24, 14, 32, 27, 3, 9, 19, 13, 30, 6, 22, 11, 4, 25 };

	/*
	 * Inverse permutation of IP for end Temporary - the true behavior will be implemented in a shift out register (Look at the pattern obvious in an 8x8 layout)
	 */
	int IPinv_p[] = { 40, 8, 48, 16, 56, 24, 64, 32, 39, 7, 47, 15, 55, 23, 63, 31, 38, 6, 46, 14, 54, 22, 62, 30, 37, 5, 45, 13, 53, 21, 61, 29, 36, 4, 44, 12, 52, 20, 60, 28, 35, 3, 43, 11, 51, 19,
	...
	private void do_sblocks(int[] in, int[] out)
	{
		for (int i = 0; i < 8; i++)
		{
			int val = sblocks[i][in[i * 6] << 1 | in[i * 6 + 5]][in[i * 6 + 1] << 3 | in[i * 6 + 2] << 2 | in[i * 6 + 3] << 1 | in[i * 6 + 4] << 0];
			out[i * 4 + 0] = val >> 3 & 1;
			out[i * 4 + 1] = val >> 2 & 1;
			out[i * 4 + 2] = val >> 1 & 1;
			out[i * 4 + 3] = val >> 0 & 1;
	...
	/**
	 * This method decodes the given password Encryption is based on the one way DES encryption
	 * 
