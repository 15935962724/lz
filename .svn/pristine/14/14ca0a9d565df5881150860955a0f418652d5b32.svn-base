...
String member = teasession._rv.toString();//teasession.getParameter("member");
...
function f_ymig(igd) 
{
	ymPrompt.win('/jsp/westrac/EditWestracClue.jsp?myact=my&t='+new Date().getTime()+'&wcid='+igd+'&nexturl='+form1.nexturl.value,600,538,'修改我的线索',null,null,null,true);
} 
function f_ymigshow(igd)
{
	ymPrompt.win('/jsp/westrac/WestracClueShow.jsp?myact=my&t='+new Date().getTime()+'&wcid='+igd+'&nexturl='+form1.nexturl.value,600,538,'我的线索详细查看',null,null,null,true);
}
</script>
<body>
<h1>用户&nbsp;<%=member %>&nbsp;提供的线索</h1>
