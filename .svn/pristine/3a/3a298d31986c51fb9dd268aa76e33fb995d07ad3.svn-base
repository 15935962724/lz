...
for(int i=0;i<12;i++){
	...
	if(hname.length()>0){//有医院
		...
		//没有医院
		sql=" and order_id in(select order_id from shopOrder where createdate<"+Database.cite(ymd)+" and (isclear=0 or isclear is null) and status!=5 and status!=6)";
	...
	float sum=ShopOrderData.sumamount(sql);
	float sum2=ShopOrderData.sumamount(sql2);
	totalarr[i]=sum+sum2;
}
//未回款金额（应收账款）
...
for(int i=0;i<12;i++){
	/* List<ShopOrder> lstorder=ShopOrder.find(" and DATEPART(mm,CreateDate) = "+(i+1)+" and datepart(yyyy,createdate) = "+year+" and matchnum != 0 and ((isclear=0 or isclear is null)or(isclear=1 and invoicestatus=3)) ", 0, Integer.MAX_VALUE);
	float sum=0;
	if(lstorder.size()>0){
		for(int j=0;j<lstorder.size();j++){
			ShopOrder order=lstorder.get(j);
			int n=order.getMatchnum();
	...
	inamountarr[i]=sum; */
	inamountarr[i]=totalarr[i]-noreamountarr[i];
}
