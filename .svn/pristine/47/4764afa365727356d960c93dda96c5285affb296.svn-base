...
<tr>
  ...
  <td class="th">付款状态：</td><td><select name="ispay"><%=h.options(CoursePlan.ISPAY_TYPE,ispay)%></select></td>
...
<div class="switch">
<%
for(int i=0;i<CoursePlan.STATE_TYPE.length;i++)
{
  out.print("<a href='javascript:mt.tab("+i+")' id='a_tab"+i+"'>"+(i==0?"全部":CoursePlan.STATE_TYPE[i])+"（"+CoursePlan.count(sql.toString()+(i==0?"":" AND state="+i))+"）</a>");
}
%>
</div>
<script>$("a_tab<%=state%>").className="current";</script>
<table id="tablecenter" cellspacing="0">
<tr id="tableonetr">
  <td></td>
  <td>课程名称</td>
...
{
  if(state>0)sql.append(" AND state="+state);
  ...
  <tr onMouseOver="bgColor='#FFFFCA'" onMouseOut="bgColor=''">
    ...
    <td>
      <a href="javascript:mt.act('state',<%=t.courseplan%>)">审核</a>
  ...
  <tr>
    ...
    <input type="button" name="multi" value="批量审核" onclick="mt.act('state','0')"/>
...
mt.act=function(a,id,b)
{
  ...
  {
    mt.show("状态：<input type='radio' name='state' value='2' id='_state2' checked /><label for='_state2'>通过</label>　<input type='radio' name='state' value='3' id='_state3' /><label for='_state3'>拒绝</label>",2,"form2.state.value=$$('_state2').checked?2:3;form2.submit()");
  }else
  {
    if(a=='view')
    {
