...
private String tree_0(Node n,int lang,String target,int step)throws Exception
{
  ...
  h.append(" /><a href='/jsp/general/NodeLists.jsp?node="+j+"' target='"+target+"'>"+n.getSubject(lang)+"</a></div>");
...
function f_c(j,step)
{
  ...
  }else
  {
    if(d.innerHTML.length==0)
    {
      ...
      sendx("/jsp/admin/NodeTree.jsp?ajax=ON&node="+j+"&step="+step,function(data)
      {
        if(data=="")data="暂无";
        d.innerHTML=data;
      } );
...
if(isFrame)
{
  out.print("<frameset cols='208,*' frameborder='no' border='0' framespacing='0'><frame src='/jsp/admin/NodeTree.jsp?node="+teasession._nNode+"&target=m2' /><frame src='about:blank' name='m2' /></frameset>");
  return;
}
%>
</head>
...
if(ns==null)
{
  ...
  out.print("<span class='title'><img src=\"/tea/image/other/img-globe.gif\" width=\"16\" height=\"16\"><a href='/jsp/general/NodeLists.jsp?node="+teasession._nNode+"' target='"+target+"'>"+node.getSubject(teasession._nLanguage)+"</a></span>");
