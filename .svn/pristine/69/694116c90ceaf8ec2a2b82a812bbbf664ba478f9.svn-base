...
for(int i=1;ef.hasMoreElements();)
{
  ...
  if(efb.hasMoreElements())
  {
    ...
    for(int j=1;efb.hasMoreElements();i++,j++)
    {
      ...
      if(submit)
      {
        line.append("<a href='###' onClick=f_click("+i+",'/jsp/admin/cec/FlowbusinessEdit.jsp');form1.submit();>");
      }
      line.append(MT.f(fb.name,"无名称")+"</a></td>");
      //
      ...
      if(submit)
      {
        line.append("<input type=submit value=办理 onClick=f_click("+i+",'/jsp/admin/cec/FlowbusinessEdit.jsp');>");
      }else
      {
...
//创建工作铵钮
p_flow="";
ef=Flow.find(teasession._strCommunity,s2+" AND flow NOT IN(11,12,13,14)");
