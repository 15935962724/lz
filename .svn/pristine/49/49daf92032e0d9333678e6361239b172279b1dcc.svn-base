...
<body>
<h1><%=r.getString(teasession._nLanguage, "UnshippedTrade")%></h1>
...
<tr>
  ...
  <td align=RIGHT><%=s28 + bigdecimal3.add(trade.getSh()).add(trade.getTax()).add( trade.getDiscount())%>
  <input type="hidden" name="Total" value="-<%=bigdecimal3.add(trade.getSh()).add(trade.getTax()).add(trade.getDiscount())%>"/>
  </td>
</tr>
  ...
  <%         if (trade.getCText(teasession._nLanguage).length() != 0 || trade.getCVoiceFlag())
            {
                ...
                {
                    out.print(new Row(new Cell(new Button(1, "CB", "CBPlay", r.getString(teasession._nLanguage, "CBPlay"), "window.open('/servlet/TradeCVoice?Trade=" + i + "', '_self');"))));
                }
            }
            %></TABLE>
    ...
    <TR ID=TableCaption>
      <TD COLSPAN=10>供货商留言</TD>
    </TR><%        if (trade.getVText(teasession._nLanguage).length() != 0 || trade.getVVoiceFlag())
            {
                ...
                {
                    out.print(new Row(new Cell(new Button(1, "CB", "CBPlay", r.getString(teasession._nLanguage, "CBPlay"), "window.open('/servlet/TradeVVoice?Trade=" + i + "', '_self');"))));
                }
            }
  %>
  </TABLE>
...
<td>状态:</td>
<td>
  <INPUT  id="radio" type="radio" NAME=Status VALUE=5 onclick="fexpress()" CHECKED>已经发货
  <INPUT  id="radio" type="radio" NAME=Status  VALUE=1 onclick="fexpress()" >取消订单</td>
</tr>

<tr>
<td>发货时间:</td>
<td><%=new tea.htmlx.TimeSelection("unshipped",null,true,true)%>(年-月-日-时-分)</td>
</tr>

<tr>
<td>估计到达时间:</td>
<td><%=new tea.htmlx.TimeSelection("estimate",null,true,true)%>(年-月-日-时-分)</td>
</tr>

<tr>
<td colspan="2"><INPUT TYPE="SUBMIT" VALUE="提交" ID="CBEnglish" CLASS="CB" onClick=""/>
</td>
</tr>
  </TABLE>
<%}%>
