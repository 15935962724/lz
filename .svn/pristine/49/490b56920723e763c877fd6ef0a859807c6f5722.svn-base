...
if("EditSemipurchase".equals(act))
{
      ...
      <tr onMouseOver="javascript:this.bgColor='#BCD1E9'" onMouseOut="javascript:this.bgColor=''" >
        ...
        <td onclick="f_xuanze2();"  title="点击表格可以添加商品"   style=cursor:pointer> <input type="text" name="supplysg<%=i%>" value="<%out.print(gdobj.getSupply());%>" size="4" style="background:#cccccc" readonly="readonly">&nbsp;元</td>
          ...
          <!--数量--><td>  <input type="text" name="quantitysg<%=i%>" value="<%=gdobj.getQuantity()%>" size="4" onkeyup="f_quantity2('<%=gdobj.getSupply()%>','<%=i%>','<%=chers%>');"></td>
            ...
            <!--折扣乘以数量的价格总计--> <td onclick="f_xuanze2();" title="点击表格可以添加商品"  style=cursor:pointer>  <input type="text" name="totalsg<%=i%>" value="<%=gdobj.getTotal()%>" size="4"  style="background:#cccccc" readonly="readonly">&nbsp;元</td>

              <td ><a href="#" onclick="f_delete2('<%=gdid%>');">删除</a> </td>
      </tr>
      <%} %>
      <%if(!fa2) {%>
      <tr>
        ...
        <td>
          <input type="text" name="totalsg"  size="4" readonly="readonly"  style="background:#cccccc" value="<%out.print(bs);%>">&nbsp;元
        </td>
      </tr>
      <%} %>
  </table>
