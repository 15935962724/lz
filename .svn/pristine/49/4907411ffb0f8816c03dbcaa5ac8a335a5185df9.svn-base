...
TeaSession teasession=new TeaSession(request);
  ...
  function f_a1(igd,y,m)//年份的 加减
  {
    ...
    if(igd=='-')
    {
     ...
     var mm="?node=<%=teasession._nNode%>&nodeshow=<%=nodeshow%>&community=<%=community%>&year="+addy+"&month="+m+"&nexturl=<%=nexturl%>&id=<%=id%>";
      sendx("/jsp/admin/flow/LifeCal_ajax.jsp"+mm,
      function(data)
      {
  ...
  function f_a2(igd,y,m)//月份的加减
  {
    ...
    var mm="?node=<%=teasession._nNode%>&nodeshow=<%=nodeshow%>&community=<%=community%>&year="+addy+"&month="+addm+"&nexturl=<%=nexturl%>&id=<%=id%>";
...
<!-- nodeshow：显示会议的页面  ，node：显示这个节点下的会议-->
