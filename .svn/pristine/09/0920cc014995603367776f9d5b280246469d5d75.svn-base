...
<h2>列表 ( <%=count%> )</h2>
     ...
     {
       out.print("<A href=?desc="+("desc".equals(desc)?"asc":"desc")+param.toString()+pos+" >地址 "+("desc".equals(desc)?"▼":"▲")+"</a>");
     }else
     {
       out.print("<A href=?order=addr"+param.toString()+pos+" >地址</a>");
     }
     %></td>
     <td nowrap><%
...
for(int index=1;enumer.hasMoreElements();index++)
{
  ...
  <tr onMouseOver="javascript:this.bgColor='#BCD1E9'" onMouseOut="javascript:this.bgColor=''">
    ...
    <td>
    <input type=button value=查看 onclick="window.showModalDialog('http://channel.mapabc.com/openmap/map.jsp?id=<%=obj.getSid()%>&uid=7246&eid=102341&w=600&h=500','','edge:raised;help:0;resizable:1;dialogWidth:620px;dialogHeight:558px;');">
    <%--
    <input type=button value=编辑 onclick="window.open('/jsp/admin/map/EditMapabc.jsp?community=<%=teasession._strCommunity%>&sid=<%=sid%>&nexturl='+encodeURIComponent(location),'_self');" >
    <input type=button value=删除 onclick="if(confirm('确定删除吗？'))window.open('/servlet/EditMapabc?community=<%=teasession._strCommunity%>&sid=<%=sid%>&oper=6&nexturl='+encodeURIComponent(location),'_self');" >
    <%
    if(bool&&obj.isHidden())
    {
    	out.print("<input type=button value=批准 onclick=\"window.open('/servlet/EditMapabc?community="+teasession._strCommunity+"&sid="+sid+"&oper=7&hidden=false&nexturl='+encodeURIComponent(location),'_self');\" >");
    }
    %>
    --%>
    </td>
...
<input type=button value=添加 onclick="window.open('/jsp/admin/map/EditMapReal.jsp?community=<%=teasession._strCommunity%>&nexturl='+encodeURIComponent(location),'_self');" >
