...
{
  sql.append(" AND code LIKE "+DbAdapter.cite("%"+type+"%"));
}
  ...
  %>
        ...
        <td><%=obj.getCode()%></td>
         ...
         <td nowrap><%=Item.STATES_TYPE[obj.getStates()]%></td>
                  ...
                  <td>
<input type="button" value="<%=r.getString(teasession._nLanguage,"CBEdit")%>" onClick="window.location='/jsp/criterion/IssueEditItem.jsp?item=<%=item%>&community=<%=community%>&nexturl=<%=nexturl%>';"/>                 </td>
 </tr>
  <%
}
     %>
