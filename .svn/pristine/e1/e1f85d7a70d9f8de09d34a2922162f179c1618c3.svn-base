...
<body>
<h1><%=menuid<1?"退学管理":AdminFunction.find(menuid).getName(h.language)%></h1>
...
<tr id="tableonetr">
  ...
  <td class="T1"><a href="javascript:mt.order('O5')" id="O5">审核人</a></td>
  <td class="T1"><a href="javascript:mt.order('O6')" id="O6">审核日期</a></td>
  <td>操作</td>
</tr>
<%
...
}else
{
  ...
  <tr id="<%=MT.enc(t.lmsleave)%>">
    ...
    <td><%
    out.println("<a href=### onclick=mt.act(this,'view')>申请表</a>");
    //out.println("<a href=javascript:mt.act('state',"+t.lmsleave+")>审核</a>");
    //if(acts.contains("edit"))out.println("<a href=javascript:mt.act('edit',"+t.lmsleave+")>编辑</a>");
    if(acts.contains("oper,"))
    {
      if(t.state!=2)out.println("<a href=### onclick=mt.act(this,'del')>删除</a>");
    }
    if(acts.contains("audit,"))
    {
      out.println("<a href=### onclick=mt.act(this,'state')>审核</a>");
    }
    %></td>
  </tr>
  <%
...
<form name="form3" action="/LmsExports.do?name=退学管理.xlsx" method="post" target="_ajax">
<input type="hidden" name="act" value="sql"/>
<input type="hidden" name="key" value="<%=MT.enc("SELECT p.member 学号,pl.firstname 姓名,p.card 证件号,lf.orgname 省助学发展机构,lo.orgname 学习服务中心,pa.member 审核人,ll.atime 审核日期,"+Lms.when(LmsLeave.STATE_TYPE,"ll.state")+" 状态 FROM lmsleave ll INNER JOIN Profile p ON p.profile=ll.member INNER JOIN ProfileLayer pl ON pl.profile=ll.member LEFT JOIN lmsorg lo ON lo.lmsorg=p.agent LEFT JOIN lmsorg lf ON lf.lmsorg=lo.father LEFT JOIN Profile pa ON ll.amember=pa.profile WHERE 1=1"+sql.toString())%>"/>
</form>
