...
StringBuffer sql = new StringBuffer(" and po.type = 4 and ( po.frontreartype =0 or  po.frontreartype=2 )");//后台管理员打票 订单
...
//售票员
String adminmember = teasession.getParameter("adminmember");
if(adminmember!=null && adminmember.length()>0)
{
	sql.append(" and adminmember like ").append(DbAdapter.cite("%"+adminmember+"%"));
	param.append("&adminmember=").append(URLEncoder.encode(adminmember,"UTF-8"));
}
...
　<a href="?id=<%=teasession.getParameter("id") %>&daytime=<%=OrderDetails.sdf.format(new Date()) %>">当天</a>&nbsp;
       <a href="?id=<%=teasession.getParameter("id") %>&weektime=<%=OrderDetails.sdf.format(new Date()) %>">本周</a>&nbsp;
        <a href="?id=<%=teasession.getParameter("id") %>&monthtime=<%=OrderDetails.sdf.format(new Date()) %>">本月</a>
　</td>
<td>票　　种：&nbsp;<span id ="showxx2"><select name="vote" >
    ...
    <td>演出：&nbsp;
       ...
       </td> <td>价　格：&nbsp;<input type="text" name="price" id="price" value="<%if(price>0)out.print(price); %>" onKeyUp="checkRate();">
　</td></tr>
...
<td>是否会员：&nbsp;<select name="membertype">
  			<option value="">-------</option>
  			...
  			</span></td><td>售票员：&nbsp;<span id ="showxx"><input type ="text" name="adminmember" value="<%if(adminmember!=null)out.print(adminmember); %>">
  			</span>
           
  		</td>
  		</tr>
  <tr><td class="tdp09" colspan="4"><input type="submit" value="" class="SearchInput"> </td></tr>
</table>  
  </form>
  
   <form name="form2" action="/servlet/ExportPerformExcel" method="POST">
    <input type="hidden" name="files" value="网上预订票已出票统计表">
...
<table> 
	<tr>
		<td>合计:&nbsp;网上预订票已出票&nbsp;<%=OrderDetails.count2(teasession._strCommunity,sql.toString()) %>&nbsp;张票&nbsp;&nbsp;总金额:&nbsp;<%=tp %>&nbsp;元</td>
	</tr>
</table>
	<table>
		<tr>
