...
function f_tourist2(igd)
{
	...
	if(obj.checked){
		...
		//document.getElementById("cancels").style.display='';
	}
}

</script>
<%=new tea.ui.TeaServlet().getSections(node,teasession._nStatus,teasession._rv, teasession._nLanguage, 10, false)%>
...
<div id="head6"><img height="6" src="about:blank" alt=""></div>
  ...
  for(int i=1;e.hasMoreElements();i++)
  {
    ...
    %>
    <tr  id=tableonetr>
      <td><%=r.getString(teasession._nLanguage,"5582766512") %>：<%=creator %></td>
      <td align="right"><%=Talkback.sdf3.format(obj.getTime()) %>&nbsp;&nbsp;<%=r.getString(teasession._nLanguage,"3889150508") %></td>
    </tr>
    <tr style="display:none" onmouseover=bgColor='#BCD1E9'; onmouseout=bgColor='';>
      <td colspan="2"><%=subject%>&nbsp;&nbsp;<img src="/tea/image/hint/<%=obj.getHint()%>.gif"></td>
    </tr>
    <tr onmouseover=bgColor='#BCD1E9'; onmouseout=bgColor='';>
      <td colspan="2"><img src="/tea/image/hint/<%=obj.getHint()%>.gif">&nbsp;&nbsp;<%=conten%></td>
    </tr>
    <%
    ...
    %>
      ...
      <td colspan="2">
        ...
        for(int j = 1;e2.hasMoreElements();j++)
        {
         ...
         %>
        <tr onmouseover=bgColor='#BCD1E9'; onmouseout=bgColor=''; >
            <td><%=j%>&nbsp;&nbsp;<%=r.getString(teasession._nLanguage,"5582766512") %>&nbsp;<%=trmember%>&nbsp;<%=r.getString(teasession._nLanguage,"4265227904") %>：</td>
              <td align="right"><%=Talkback.sdf3.format(trobj.getTime()) %>&nbsp;</td>
          </tr>
          <tr onmouseover=bgColor='#BCD1E9'; onmouseout=bgColor='';>
           <td colspan="2"><%=trobj.getText()%></td>
          </tr>
          <%}%>
        </table>
     </td>
    ...
    <%} %>
      ...
      <tr>
      <td align="right" colspan="2"><a href="#"  onclick="huifu('<%=tbid%>');"><%=r.getString(teasession._nLanguage,"6948521657") %></a></td>
    </tr>
    <%} %>
    ...
    <tr>
      <td align="center" colspan="2"><%=r.getString(teasession._nLanguage,"5666186480") %>。</td>
    </tr>
  ...
  <table border="0" cellpadding="0" cellspacing="0" id="tablecenter">
    ...
    if(teasession._rv == null&&(node.getOptions() & 0x8000) == 0)
    {
      ...
      &nbsp;&nbsp;
      <input type="submit" value="<%=r.getString(teasession._nLanguage,"2592261394") %>"/> </span>
      &nbsp;<input type="checkbox" name="tourist_sub" value="1" id="tourist_sub" onClick="f_tourist('tourist_sub');">&nbsp;<%=r.getString(teasession._nLanguage,"2089063390") %>
      </td>
      </form>
   <%
 }else if(teasession._rv!=null)
   ...
   <input type='hidden' name='tourist'>
    <tr><td align="right"><%=r.getString(teasession._nLanguage, "Hint")%>:</td>
      ...
      <tr id="table_main" >
        ...
        <td><INPUT NAME="subject" TYPE=TEXT class="edit_input" VALUE="" SIZE=80 MAXLENGTH=255></td>
      </tr>
      <tr>
      ...
      <td>&nbsp;</td>
       ...
       <tr>
        <td align="right"><%=r.getString(teasession._nLanguage, "2549850593")%>:</td>
        <td><img src="/NFasts.do?act=verify" id="vcodeImg" alt="<%=r.getString(teasession._nLanguage,"4795430490") %>" align="absmiddle" class="CodeImg" onClick="reloadVcode();">&nbsp;
        <input type="TEXT"  name=vertify value="" size="5" />&nbsp;&nbsp;<a href="###"   style="cursor:pointer"  onClick="reloadVcode();"><%=r.getString(teasession._nLanguage,"4249463039") %> </a>&nbsp;(<%=r.getString(teasession._nLanguage,"0207031817") %>)</td>
      </tr>
      ...
      <td><%=r.getString(teasession._nLanguage,"6020412808") %></td>
      </tr>
      </form>
  </table>
