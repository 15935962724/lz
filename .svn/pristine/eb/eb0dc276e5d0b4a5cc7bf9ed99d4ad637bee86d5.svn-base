...
par.append("&adminunitid=").append(adminunitid);
sql.append(" AND node="+node);
sql.append(" AND adminunitid="+adminunitid);
...
function f_edit(id)
{
  form2.action="/jsp/type/meeting/WestracEditMeetingApplicants.jsp";
  form2.method="get";
  form2.target="";
  form2.id.value=id;
  form2.submit();
...
function f_sub()
{
  form2.action="/servlet/EditMeetingInvite";
  form2.method="post";
  form2.state.value="1";//确认报名
  form2.act.value="Verifg";
...
<input type=hidden name="id" value="<%=ymi.getId()%>"/>
<input type=hidden name="node" value="<%=node%>">
<input type=hidden name="adminunitid" value="<%=adminunitid%>">
...
<h2>列表&nbsp;(&nbsp;目前共有&nbsp;<font color="#000000" size="2"><%=sum%></font>&nbsp;个报名人员&nbsp;)&nbsp;&nbsp;
<input type="button" value="添加报名人员" onclick="f_edit(0)">
</h2>
<table cellSpacing="0" cellPadding="0" width="100%" border="0" id="tablecenter">
  <tr id="tableonetr">
...
{
  out.print("<tr><td colspan='5' align='center'>暂无报名人员");
}
...
{
%>
  <INPUT TYPE=button name="GoFinish" ID="edit_GoFinish" class="edit_button" onclick="f_sub()" VALUE="确认报名">&nbsp;
<%
}
%>
