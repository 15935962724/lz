...
if(membername!=null && membername.length()>0)
{
	...
	sql.append(" and rcreator like ").append(DbAdapter.cite("%"+membername+"%"));
...
{
	sql.append(" and node in (select node from Report where media in (select media from MediaLayer where name like "+DbAdapter.cite("%"+medianame+"%")+"))");
	 par.append("&medianame=").append(URLEncoder.encode(medianame,"UTF-8"));
}
par.append("&pos=");

String order=teasession.getParameter("order");
if(order==null)order=cal.getSortType();
...
if(pur>0)//有创建的权限
{
  if(n.getType()==0)
  {
    if(isC||am.isProvider(0))out.println("<input type=button value="+r.getString(teasession._nLanguage,"CBNewFolder")+" onclick=window.open('/jsp/general/EditNodeSimple.jsp?NewNode=ON&Type=0&node="+teasession._nNode+"&nexturl='+encodeURIComponent(location.href),'_self');>");
    if(isC||am.isProvider(1))out.println("<input type=button value="+r.getString(teasession._nLanguage,"CBNewCategory")+" onclick=window.open('/jsp/general/EditNodeSimple.jsp?NewNode=ON&Type=1&node="+teasession._nNode+"&nexturl='+encodeURIComponent(location.href),'_self');>");
  ...
  for(int x=pos+1;e.hasMoreElements();x++)
  {
    ...
    if(isC||amobj.isProvider(nt))
    {
      ...
      Node nobj2  =   Node.find(nodeid);
       ...
       {
          out.print("<input type=button value="+r.getString(teasession._nLanguage,"CBEdit")+" onclick=window.open('/jsp/general/EditNodeSimple.jsp?node="+nodeid+"&EditNode=ON&nexturl='+encodeURIComponent(location.href),'_self');>");
       }else{
