...
int puid = h.getInt("profile");
int puid_id = h.getInt("puid");
...
<input type="hidden" name="puid_id" value="<%= puid_id %>"/>
...
function loadlist(pos){
	mypos = pos;
    var listtb = new page.loadPage({"url":"/ServiceInvoices.do?act=joinCompany&"+$("#form1").serialize(),"callBackFunc":createList,'page_size':'10','type':2,'page_size_max':'10','par':myurl,'showtype':1,'pos':pos,'showfun':loadlist});
	listtb.show();
}
loadlist(0);
...
function createList(resultlist){
	...
	if(resultlist.data_list!=''){//是否存在数据
		for(var i=0;i<resultlist.data_list.length;i++){
			...
			var $name = $("<td>"+(parseInt(i)+1+parseInt(mypos))+"</td><td class='proflie' style='display:none;'>"+proObj.profile+"</td><td class='member'>"+proObj.member+"</td><td class='company'>"+proObj.company+"</td>");
			boxDiv.append($name);
    ...
    $('.transmit').on('click', function(){
        /*parent.$('.parentIframe').text($(this).children(".member").text());*/
        parent.$('.parentIframe2').text($(this).children(".company").text());
        parent.$('input[name=company]').val($(this).children(".proflie").text());
