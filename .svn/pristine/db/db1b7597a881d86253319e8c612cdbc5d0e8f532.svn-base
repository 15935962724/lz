...
<%@page import="tea.entity.*"%><%@page import="tea.db.*"%><%@page import="tea.ui.*"%><%@page import="tea.entity.admin.*"%>
...
//地方管理员，只能看到本省
sql.append(" AND(");
String[] arr=AdminUsrRole.find(h.community,h.member).getRole().split("/");
for(int i=1;i<arr.length;i++)
{
  String name=AdminRole.find(Integer.parseInt(arr[i])).getName();
  if(i>1)sql.append(" OR");
  sql.append(" province LIKE "+DbAdapter.cite(name+"%"));
}
sql.append(")");
...
}else
{
  ...
  <tr onMouseOver="bgColor='#FFFFCA'" onMouseOut="bgColor=''">
    ...
    <td><%=MT.f(t.city)%></td>
    <td><%=MT.f(t.town)%></td>
    <td><%=MT.f(t.village)%></td>
    <td><%=MT.f(t.photo).length()>0?"<a href='###' onclick=mt.img('"+t.photo+"')>查看</a>":"无"%></td>
    <td><input type="button" value="编辑" onclick="f_act('2Edit',<%=t.donation%>)"/></td>
  </tr>
  <%}
...
<br/>
<input type="button" value="导出" onclick="f_act('exppra',0)"/>
<input type="button" value="导入" onclick="f_act('Imp','par')"/>
</form>
