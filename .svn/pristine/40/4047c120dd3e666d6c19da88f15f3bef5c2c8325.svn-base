...
if(order==null)order="time";//cal.getSortType();

boolean asc="true".equals(teasession.getParameter("asc"));

int count=Node.count(sql.toString());
sql.append(" ORDER BY n.").append(order).append(" ").append(asc?"ASC":"DESC");
...
//ajax列表<input type="text" name="medianame" id="medianame" style="width:140" onKeyUp="f_gz2();" autoComplete="off" >
  fsb.append("<input type='text' name='medianame2' id='medianame2' style='width:140' onKeyUp='f_gz2();' autoComplete='off'/>");
}
*/
if(tatype==-1)
{
...
String url=tatype==0?"/jsp/general/EditNode_yl.jsp":tatype<1024?"/jsp/type/"+Node.NODE_TYPE[tatype].toLowerCase()+"/Edit"+Node.NODE_TYPE[tatype]+"_yl.jsp":"/jsp/type/dynamicvalue/EditDynamicValue.jsp";
...
function f_act(act)
{
  ...
  {
    var rs=window.showModalDialog('/jsp/general/SelNode.jsp?info='+encodeURIComponent(act=='clone'?'复制到':'移动到')+'&t='+new Date().getTime(),self,'scroll:0;status:0;help:0;resizable:1;dialogWidth:600px;dialogHeight:400px;');
    if(!rs)return;
    var arr=rs.split(",");
...
function f_cm(igd)
{
	  var y ='edge:raised;scroll:0;status:0;help:0;resizable:0;dialogWidth:550px;dialogHeight:280px;';
...
<body>
<h1>
<div id="pathdiv"><%=n.getAdminAncestor(teasession._nLanguage).replaceAll("id=\"PathID1\">>", "id=\"PathID1\">当前栏目：")%></div>
...
if(pur>0)//有创建的权限
{
  if(n.getType()==0)
  {
    if(isC||am.isProvider(0))out.println("<input type=button value="+r.getString(teasession._nLanguage,"CBNewFolder")+" onclick=window.open('/jsp/general/EditNode_yl.jsp?NewNode=ON&Type=0&node="+teasession._nNode+"&nexturl='+encodeURIComponent(location.href),'_self');>");
    if(isC||am.isProvider(1))out.println("<input type=button value="+r.getString(teasession._nLanguage,"CBNewCategory")+" onclick=window.open('/jsp/general/EditNode_yl.jsp?NewNode=ON&Type=1&node="+teasession._nNode+"&nexturl='+encodeURIComponent(location.href),'_self');>");
  /*  //显示报纸发行按钮
...
%>
排序:<select id="s_order" onchange="form1.order.value=value;form1.submit();"><option value="time">时间<option value="starttime">发生时间<option value="sequence">顺序</select>
...
//      }
    ...
    if(isC||amobj.isProvider(nt))
    {
      ...
      Node nobj2  =   Node.find(nodeid);
       ...
       {
          out.print("<input type=button value="+r.getString(teasession._nLanguage,"CBEdit")+" onclick=window.open('/jsp/general/EditNode_yl.jsp?node="+nodeid+"&EditNode=ON&nexturl='+encodeURIComponent(location.href),'_self');>");
       }else{
      ...
      if(pur>2)out.print("<input type=button value="+r.getString(teasession._nLanguage,"CBDelete")+" onclick=mt.act('del',"+nodeid+")>");
      if(amobj.isAuditing())
      {
  ...
  if(isC||am.isProvider(c.getCategory()))
  {
	  ...
	  /* out.println("<input id=PointsSet type='button' name='multi' value='积分设置' onclick=f_act('point');>"); */
	  out.println("<input type='button' name='multi' value='显示/隐藏' onclick=mt.act('hidden')>");
