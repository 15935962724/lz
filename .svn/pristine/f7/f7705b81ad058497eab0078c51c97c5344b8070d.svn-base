...
<%
     ...
     if(teasession.getParameter("member")!=null && teasession.getParameter("member").length()>0)
     {
         ...
         param.append("&member=").append(name);
     }
     String companyname="";
...
if("POST".equals(request.getMethod()))
{
   ...
   { String jids[]= teasession.getParameterValues("jid");
    ...
    //response.sendRedirect("/jsp/type/lvyou/Findappjobs.jsp");
  //  return;
}
...
<body id="bodynone">
 <h1><%=member%>求职记录</h1>
  ...
  <form action="/jsp/type/lvyou/Findappjobs.jsp" name="form1" method="POST">
    <input type="hidden" name="id" value="<%=id%>">
     <input type="hidden" name="member" value="<%=member%>">
    <input type="hidden" name="files" value="<%=member%>求职记录"/>
     <input type="hidden" name="act">
    <table border="0" cellpadding="0" cellspacing="0" id="tablecenter">
     <tr>
    <td colspan=4>求职记录</td>
   </tr>
    <tr>
  ...
  <table border="0" cellpadding="0" cellspacing="0" id="tablecenter">
   <tr>
    ...
    ArrayList<LvyouJob> jobs = LvyouApply.find(name, companyname, province, city, catagory, state, day1, day2,member, pos, pageSize);
 //   ArrayList<LvyouJob> jobs = LvyouApply.find(member);
    
    
    int count=LvyouApply.count(name, companyname, province, city, catagory, state, day1, day2,member);
;
    ...
    %>
     <tr onMouseOver="javascript:this.bgColor='#BCD1E9'" onMouseOut="javascript:this.bgColor=''">
        ...
        <td> 
        <a href="/jsp/type/lvyou/Showappjob.jsp?jid=<%=job.getId()%>&member=<%=job.getMember()%>" >详细</a>&nbsp;
