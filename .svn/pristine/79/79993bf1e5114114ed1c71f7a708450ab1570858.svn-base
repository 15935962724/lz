...
<table border="0" cellpadding="0" cellspacing="0" id="tablecenter">
  ...
  try
  {
    Enumeration eu = OrdersBatch.find(teasession._strCommunity,sql.toString(),pos,10);
    ...
    {
      int obid = Integer.parseInt(String.valueOf(eu.nextElement()));
      ...
      %>
      <tr onMouseOver="javascript:this.bgColor='#BCD1E9'" onMouseOut="javascript:this.bgColor=''" title="点击选择商品"  onclick="f_button('<%=obid%>');"  style=cursor:pointer>
        <td nowrap="nowrap"><%= sgobj.getGoodsnumber()%></td>
        <td nowrap="nowrap"><%=sgobj.getSubject()%></td>
        <td nowrap="nowrap"><%=obobj.getPrice()%></td>
        <td nowrap><%=sobj.getName(teasession._nLanguage)%></td>
        <td nowrap="nowrap"><%=obobj.getQuantity()%></td>
      </tr>
      <%
      }
    }catch(Exception e)
