...
<h2>会员列表(<font color="#000000" size="3"><%=count %></font>)<input type="button" onclick="build();" value="创建会员"/><input type="button" onclick="delAll('<%=h.community %>','ChildMemberList');" value="批量删除"/><input type="button" onclick="excel();" value="导出Excel"/><input type="button" onclick="email();" value="发送Email"/></h2>
<form name="form2">
...
<tr>
	...
	<td><%=cm.getEmail() %></td>
	<td><%=cm.getTime() %></td>
    <td><a href='###' onMouseOver='mt.more(this)'>操作</a><div style='display:none'>
      <a href="javascript:showInfo('<%=cm.getMember() %>','<%=h.community%>');">详细</a>
      <a href="javascript:delInfo('<%=cm.getMember() %>','<%=h.community%>','ChildMemberList');">删除</a>
      <a href="javascript:editInfo('<%=cm.getMember() %>','<%=h.community%>');">编辑</a>
      <a href="javascript:clearPwd('<%=cm.getMember() %>','<%=h.community%>');">清空密码</a>
      </div>
    </td>
</tr>
<%} %>
 <%if (count > size) {  %>
  <tr> <td colspan="10"><%=new tea.htmlx.FPNL(h.language,request.getRequestURI()+sb2.toString()+"&pos=",pos,count,size)%>    </td> </tr>
  <%}  %>
</table>
</form>
</body>
