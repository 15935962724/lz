...
java.util.Enumeration e=Resume.getNode(teasession._rv.toString(),teasession._nLanguage);
Node node=Node.find(teasession._nNode);
Job job=Job.find(teasession._nNode,teasession._nLanguage);
Date d = new Date();
if(d.compareTo(job.getValidityDate())==1){
  out.println("<script type='text/javascript'>alert('招聘时间已过期!');</script>");
  out.println("<script type='text/javascript'>self.location='/servlet/Node?node=" +teasession._nNode + "'</script>");
     return;
...
//String email=getNull(Classified.find(job.getOrgId()).getEmail(teasession._nLanguage));
...
<BODY onload="if(document.getElementById('radio3').checked == true){form1.submit.disabled=false;}">
...
<table  border="0"  id="tablecenter" cellpadding="0" cellspacing="0">
  <TR>
    ...
    <TD><%=tea.entity.node.Node.find(job.getOrgId()).getSubject(teasession._nLanguage)%></TD>
  </TR>
  <TR>
  ...
  <TR>
    ...
    <TD> <%
   //Occupation ocobj =  Occupation.find(teasession._strCommunity,job.getOccId("&nbsp"));
//   for(int i=0;i<job.getOccId().split("/").length;i++)
//   {
//     Occupation ocobj =  Occupation.find();
//   }
//  out.print(Occupation.find(teasession._strCommunity,job.getOccId()).getSubject());
//  out.print(job.getOccIdToHtml);
out.print(job.getOccIdToHtml());
   %> </TD>
  </TR>
  <%--
...
<h2><%=r.getString(teasession._nLanguage,"MyResume")+teasession._nNode%></h2>
<form name="form1" action="/servlet/EditJobApply">
...
while(e.hasMoreElements())
{
    ...
    boolean boolfull=((resume.getExperience()==0||boolEmployment)&&boolEducate&&boolProfessional);
%>
  <tr>
    <td>
      <input id="radio3" type="radio" name="resume" onClick="fselect();" value="<%=nodecode%>" <%if(!boolfull){out.print(" disabled ");}else{out.print(" checked ");}%>><%=obj.getSubject(teasession._nLanguage)%></td>
...
<%
e=ApplyTable.findByMember(teasession._rv._strR,teasession._strCommunity);
...
<table border="0" cellpadding="0" cellspacing="0" id="tablecenter">
  ...
  {
    out.print("<tr><td><input name=applytable  id=radio type=radio value=0 checked>"+r.getString(teasession._nLanguage,"不使用简历文件"));
  }
...
<br>
<!--用选中的简历和申请表申请该职位-->
<input class="edit_button" name="submit" type=submit disabled value="<%=r.getString(teasession._nLanguage,"用选中的简历和简历文件申请该职位")%>">
