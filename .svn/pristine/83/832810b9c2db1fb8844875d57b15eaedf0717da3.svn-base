...
<style>
.egg{width:100%;max-width:640px;}
.eggList{list-style:none;width:100%;overflow:hidden;/*display:-webkit-box*/}
.eggList .egs{width:50%;margin:0px;list-style:none;/*background-size:100% auto !important;*/ background-position:center center;height:137px}

.eggList section{display:-webkit-box;margin-top:80px}
.eggList section div {-webkit-box-flex:1;background-size:50% auto !important;}
...
.hammer{z-index:1000 !important;left:55%;background-size:80%;display:none}
.upbox_cens{width:100%;-webkit-box-sizing:border-box;}
.title .Detail{padding:10px 15px;}
.title .Detail p{font-weight:bold;color:#000;}
    #oliy{background-size: 50% auto !important;width: 50%;margin: 0 auto;position: absolute;
left: 25%;/*margin-left: -105px;*/z-index:1000;top:5px}
 .eggList #egleft{ background-position:60% center}
.eggList #egright{ background-position: 40% center }
</style>
</head>
	...
	<div class="main">
		...
		<div class="egg">
			<div class="eggList">
				...
				<div class='egs' id="oliy"><sup></sup></div>
				<section>
				<div class='egs' id="egleft"><sup></sup></div>
				<div class="egs" id='egright'><sup></sup></div>
        </section>
...
<script type="text/javascript">
	...
	$(".eggList .egs").click(function() {
		//$(this).children("span").hide();
		if(!isSmash)
			eggClick($(this));
	});

	$(".eggList .egs").hover(function() {
		var posL = $(this).position().left + $(this).width()+10;
       //alert("left:"+$(this).position().left+",width:"+$(this).width()+",left2:"+posL);
				...
				$(".hammer").animate(
					{
						...
						//$(".hammer").hide();
						$('.resultTip').css({display:'block',top:'100px',left:_this.position().left+10,opacity:0}).animate({top: '50px',opacity:1},300,function(){
							if(v.id>0){//有奖
