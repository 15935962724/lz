...
param.append("?adminunitid="+adminunitid);
...
sql.append(" AND ym.timeStop>=getdate()");

int count = MeetingInvite.count(sql.toString());

sql.append(" order by ym.time desc ");
...
function f_enroll(nid,adminunitid)
{
	...
	mt.show('/jsp/type/meeting/WestracMeetingApplicants.jsp?node='+nid+'&adminunitid='+adminunitid,0,'报名人员信息',800,600);
}
...
ArrayList enList = MeetingInvite.find(sql.toString(),pos,size);
for(int i=0;i<enList.size();i++)
{
  	MeetingInvite ymi = (MeetingInvite)enList.get(i);
 ...
 <tr onMouseOver=bgColor="#BCD1E9" onMouseOut=bgColor="">  
  ...
  %></td>
    ...
    <%if(ymi.getState()==0)
    {	
    	if(eobj.getTimeStop().getTime()>=new Date().getTime())
    		out.println("<a href='###' onclick='f_enroll("+nid+","+adminunitid+");' >报名</a>");
    	else
    		out.println("已过报名时间");
    }else
    	out.println("已报名");
