...
<table border="0" cellpadding="0" cellspacing="0" id="tablecenter">
  ...
  try
  {
    count=db.getInt("SELECT COUNT(node) FROM Node WHERE type=84  and node in (select node from Interlocution where offerintegral>0 )  AND community="+DbAdapter.cite(teasession._strCommunity)+sql.toString());
    db.executeQuery("SELECT   node FROM Node WHERE type=84 AND community="+DbAdapter.cite(teasession._strCommunity)+sql.toString()+"  and node in (select node from Interlocution where offerintegral>0)  ORDER BY node DESC",0,10);
