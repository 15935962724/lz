...
<header class="header">服务费管理</header>
...
{ 
  out.print("<a href='javascript:mt.tab("+i+")' class='swiper-slide "+(i==tab?"current":"")+"'>"+TAB[i]+"（"+Charge.count(sql.toString()+SQL[i])+"）</a>");
}
out.print("</div>");
%>
...
$("#all").click(function(){  
	...
	mt.act=function(a,rid)
	{
	  ...
	  if(a=='data')
	  {
		  ...
		  location.href="/mobjsp/yl/shopwebnew/ChargeDatas_wxnew.jsp?chargeid="+rid+"&nexturl="+location.pathname+location.search;
	  }
	};
