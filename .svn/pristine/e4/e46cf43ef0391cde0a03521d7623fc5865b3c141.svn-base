 ...
 int count = GoodsDetails.count(teasession._strCommunity," AND paid="+DbAdapter.cite(purid));
    ...
    for(int i = 0;e.hasMoreElements();i++)
    {
      ...
      <tr onMouseOver="javascript:this.bgColor='#BCD1E9'" onMouseOut="javascript:this.bgColor=''" >
        ...
        <!--选择的价格-->
        <td  title="可以修改价格"   > <input type="text" name="supply<%=i%>" value="<%out.print(gdobj.getSupply());%>" size="4"  onkeyup="f_quantity('<%=gdid%>','<%=i%>','<%=count%>');">&nbsp;元</td>

          <!--数量--><td>  <input type="text" name="quantity<%=i%>" value="<%=gdobj.getQuantity()%>" size="4" onkeyup="f_quantity('<%=gdid%>','<%=i%>','<%=count%>');"></td>

            <!--折扣乘以数量的价格总计-->
            <td onclick="f_xuanze();" title="点击表格可以添加商品"  style=cursor:pointer>  <input type="text" name="total<%=i%>" value="<%=gdobj.getTotal()%>" size="4"  style="background:#cccccc" readonly="readonly">&nbsp;元</td>
             <td >
