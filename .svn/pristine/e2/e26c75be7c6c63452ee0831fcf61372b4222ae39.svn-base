...
int pos = s != null ? Integer.parseInt(s) : 0;


String sql=" AND n.node IN(SELECT node FROM Talkback WHERE rmember="+DbAdapter.cite(teasession._rv._strR)+")";
...
<body>
<h1><%=j%> <%=r.getString(teasession._nLanguage, "TalkbackingNodes")%></h1>
<div id="head6"><img height="6" src="about:blank"></div>
<div id="PathDiv"><%=TeaServlet.hrefGlance(teasession._rv)%> ><A href="/jsp/node/Nodes.jsp"><%=r.getString(teasession._nLanguage, "Nodes")%></A> ><%=r.getString(teasession._nLanguage, "TalkbackingNodes")%></div>
