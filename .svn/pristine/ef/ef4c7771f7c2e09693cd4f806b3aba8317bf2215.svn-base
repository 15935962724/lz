...
function f_rejection2()
{
	if(!submitCheckbox(form2.nodes,'<%=r.getString(teasession._nLanguage, "请选中操作数据")%>'))return;
	var igd = '/';
	var nodesnum=document.getElementsByName("nodes");
	for (var i=0; i<nodesnum.length; i++)
		    {
		      if (nodesnum[i].checked)
		      {
			      igd = igd+nodesnum[i].value+"/"
			  }
		    }
...
<%
	...
	out.print("<span id=cnlistRight><a href=### onclick=f_s('2','cnlistid2'); class=cnlistclass2");
	...
	out.print("<a href=### onclick=f_s('-1','cnlistid4'); class=cnlistclass4");
	if("cnlistid4".equals(divid))
	{
		out.print(" id ="+divid);
	}
	out.print(">全部稿件</a></span>");
%>
  ...
  for(int x=pos+1;e.hasMoreElements();x++)
  {
    ...
    if(n.getType()==0){
      ...
      String s1 = nobj.getSubject(teasession._nLanguage);
        ...
        String p = robj.getPicture(teasession._nLanguage);
    ...
    //处理人员
    out.print("<td id=NodeListsIDsource>"+Entity.getNULL(robj.getEditmember(teasession._nLanguage))+"</td>");
//处理状态
	out.print("<td>");
