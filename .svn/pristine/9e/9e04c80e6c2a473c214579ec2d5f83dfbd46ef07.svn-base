...
StringBuffer sql = new StringBuffer("  and type =1 "+strsql);
...
<script type="">
   function f_edit(igd)
   {
       ...
       form1.action="/jsp/erp/semi/EditSemiGoods.jsp"
       form1.submit();
   }
   ...
   function f_show(igd,str)
   {
     ...
     if(str==1){
      url = '/jsp/erp/semi/SemiPurchaseShow.jsp?purid='+igd+'&act=ruku'
     }else if(str==2)
     {
        y ='edge:raised;scroll:0;status:0;help:0;resizable:0;dialogWidth:457px;dialogHeight:215px;';
        url = '/jsp/erp/semi/SemiPurchaseShow.jsp?warid='+igd+'&act=cangku'
     }
...
<table border="0" cellpadding="0" cellspacing="0" id="tablecenter">
  <tr id="tableonetr">
    <td nowrap><a href="javascript:f_order('purchase');">入库单号</a>
     <%
    ...
    <td nowrap><a href="javascript:f_order('time_s');">入库日期</a>
     <%
          if(o.equals("time_s"))
          {
            if(aq)
...
%>
  <tr onMouseOver="javascript:this.bgColor='#BCD1E9'" onMouseOut="javascript:this.bgColor=''">
    ...
    <td align="center"><%=Purchase.FLOWSTR[pobj.getFlowtype()] %></td>
    ...
    <%
        if(flowtype==0){
           ...
           out.print("</td>");
            ...
            out.print(" <td align=\"center\"><a href=\"#\"  onclick=\"f_edit('"+pur+"');\">审核采购单</a> </td>");
        }else if(flowtype==2||flowtype==3)
        {
           out.print(" <td align=\"center\"><a href=\"#\"  onclick=\"f_edit('"+pur+"');\">审核入库</a></td> ");
        }
   %>
  </tr>
  <%} %>
...
{
  %>
  <input type="button" value="添加半成品采购单" onclick="window.open('/jsp/erp/semi/EditSemiGoods.jsp?nexturl=<%=nexturl%>','_self');"/>
  <%
}
%>
</form>
