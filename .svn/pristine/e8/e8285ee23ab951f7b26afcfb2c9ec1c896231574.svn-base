<%@page contentType="text/html;charset=UTF-8" %><%@page import="tea.entity.site.*" %><%@page import="tea.entity.*" %><%@page import="tea.resource.Resource" %>
<%@page import="tea.db.DbAdapter" %><%@page import="tea.entity.node.*" %><%@page import="java.util.*" %><%@page import="java.io.*" %>
<%@page import="tea.entity.admin.*" %><%@page import="tea.entity.member.*" %><%@page import="tea.ui.TeaSession" %><% request.setCharacterEncoding("UTF-8");
...
{
  if(act=="ajax_opinion")dt="/591,592,593/594,595,596/597,598,599/";
...
function f_send()
{
  if(<%=isNew%>)
  {
    var dt602=form1.dynamictype602;
    var v=dt602.value;//正文
    if(v.indexOf(":")!=-1||dt602.defaultValue.indexOf(v)!=-1)
    {
...
{
  //如果备选文件大于1个，则弹出对话框。
  var v=form1.dynamictype602.value;
  if(v.charAt(0)==':')
  {
    if(v.charAt(v.length-1)==':')v=v.substring(0,v.length-1);
    sel=v.split(':').length==2?v.substring(1):window.showModalDialog('/jsp/admin/flow/SetFlowbusinessDocs.jsp?community=' + form1.community.value + '&field=dynamictype602&t='+new Date().getTime(),self,'dialogWidth:360px;dialogHeight:280px;help:0;status:0;scroll:0;');
    form1.dynamictype602.value=sel;
  }
}
function f_stop()
{
...
if(bw||br)
{
  ...
  if(authority)
  {
    out.println("<input type='button' value='盖章' onclick='form1.dynamictype_button602.onclick()' >");
  }
//  if(step==9)
//  {
...
out.println("<input type='button' value='下载' onclick=location.href='/servlet/EditFlowdynamicvalue?act=down&community='+form1.community.value+'&flowbusiness='+form1.flowbusiness.value+'&dynamictype=/602/589/' >");
...
if(dtw.indexOf("/591/")!=-1)//董事长批示
{
  out.println("<input type='button' value='填写意见' onclick=f_opinion('/591/592/593/') />");
}else if(dtw.indexOf("/594/")!=-1)//公司总经理批示
{
  out.println("<input type='button' value='填写意见' onclick=f_opinion('/594/595/596/') />");
}else if(dtw.indexOf("/597/")!=-1)//公司分管领导批示
{
  out.println("<input type='button' value='填写意见' onclick=f_opinion('/597/598/599/') />");
}
//会签
if(dtw.indexOf("/600/")!=-1)//公司相关领导会签
{
  out.println("<input type='button' value='填写意见' onclick='f_csign(600)' />");
}else if(dtw.indexOf("/610/")!=-1)//签报部门意见
{
  out.println("<input type='button' value='填写意见' onclick='f_csign(610)' />");
}else if(dtw.indexOf("/613/")!=-1)//会签部门意见
{
  out.println("<input type='button' value='填写意见' onclick='f_csign(613)' />");
}else if(dtw.indexOf("/611/")!=-1)//承办情况
{
  out.println("<input type='button' value='填写意见' onclick='f_csign(611)' />");
}
...
//发文字号
if(dtw.indexOf("/607/")!=-1)//||isCreator
{
  out.println("<input type='button' value='签报字号' onclick=\"window.showModalDialog('/jsp/admin/xny/SetFlowbusinessSN.jsp?community='+form1.community.value+'&flowbusiness='+form1.flowbusiness.value+'&dynamictype='+607+'&t='+new Date().getTime(),self,'dialogWidth:350px;dialogHeight:105px;resizable:1;help:0;status:0;');\" />");
}
//归档
...
<input type="text" name="dynamictype591" disabled="disabled" />

<input type="text" name="dynamictype594" disabled="disabled" />
<input type="text" name="dynamictype597" disabled="disabled" />
<input type="text" name="dynamictype600" disabled="disabled" />

<input type="text" name="dynamictype610" disabled="disabled" />
<input type="text" name="dynamictype613" disabled="disabled" />
<input type="text" name="dynamictype611" disabled="disabled" />
</div>
<!-- -end-->
...
<input type="button" name="print" id="print"  value="签报审批表" <%if(ist.equals("1"))out.print("class='curbut'");%> onClick="getXx(this);"/>
...
{
  //if(dtw.indexOf("/20/")!=-1||dtw.indexOf("/23/")!=-1||dtw.indexOf("/28/")!=-1||dtw.indexOf("/96/")!=-1)
  {
...
<table id="tabbase"  <%if(ist.equals("1"))out.print("style='display:none'");%>>
  <tr>
    <td class="tdtitle">签报名称</td>
    <td id="td588"><input type="text" name="dynamictype588" id="dynamictype588"/></td>
  </tr>
  <tr>
    <td class="tdtitle">主 题 词</td>
    <td  id="td614"><table class="addbutton"><tr><td class="td1"><input type="text" name="dynamictype614" id="dynamictype614" /></td><td class="td2"><input type="button" value="提取" onClick="f_ajax('ajax_key','614',form1.dynamictype588.value);"/></td></tr></table></td>
  </tr>
  <tr>
  ...
  <tr>
    <td class="tdtitle">签报字号</td>
    <td id="td607"><input type="text" readonly="readonly" style="background:transparent;border:0;width:90%;" name="dynamictype607"/></td>
  </tr>
  <tr>
    <td class="tdtitle" style="border-right:0px;">联系人</td>
    <td style="padding:0px;border:0px;">
    <table id="tabbase_nei" border="0" cellspacing="0" cellpadding="0">
    <tr>
    <td class="tabtdteshu" id="td601"><input type="text" name="dynamictype601" /></td>
    <td class="tdtitle">电话</td>
    <td id="td612"><input type="text" name="dynamictype612"/></td>
    </tr>
    </table>
    </td>
  </tr>
  <tr>
    <td class="tdtitle" style="border-right:0px;">签报部门</td>
    <td style="padding:0px;border:0px;">
    <table id="tabbase_nei" border="0" cellspacing="0" cellpadding="0">
    <tr class="out2">
    <td class="tabtdteshu" id="td608"><input type="text" name="dynamictype608" /></td>
    <td class="tdtitle">签报日期</td>
    <td id="td609"><input type="text" name="dynamictype609" onclick='showCalendar(this,true)' readonly='true'  /></td>
    </tr>
    </table>
    </td>
  </tr>
...
<area shape="rect" coords="148,157,247,182" href="###" id="3" pstep="/2/" />
...
<area shape="rect" coords="150,517,248,546" href="###" id="3" pstep="/7/"  />
...
<area shape="rect" coords="149,811,248,838" href="###" id="12" />
<area shape="rect" coords="146,882,248,908" href="###" id="13" />
<%
}else if(flow==5)
{%>
<area shape="rect" coords="150,10,248,38" href="###" id="1" />
<area shape="rect" coords="148,82,247,108" href="###" id="2" />
<area shape="rect" coords="148,157,246,184" href="###" id="3" pstep="/2/" />
<area shape="rect" coords="18,388,116,416" href="###" id="4" />
<area shape="rect" coords="148,225,246,273" href="###" id="5" />
<area shape="rect" coords="149,314,246,345" href="###" id="3" pstep="/4/5/" />
...
<area shape="rect" coords="150,684,247,712" href="###" id="10" />
<area shape="rect" coords="148,754,248,782" href="###" id="11" />
<area shape="rect" coords="147,824,247,851" href="###" id="12" />
<%
...
<%
}else if(flow==8||flow==12||flow==14)
{
%>
<area shape="rect" coords="75,9,170,36" href="###" id="1" />
<area shape="rect" coords="75,80,175,118" href="###" id="2" />
<area shape="rect" coords="76,166,173,206" href="###" id="3" />
<area shape="rect" coords="76,254,172,280" href="###" id="4" />
<area shape="rect" coords="77,323,173,362" href="###" id="5" />
<area shape="rect" coords="76,408,172,434" href="###" id="6" />
<area shape="rect" coords="77,477,173,503" href="###" id="7" />
<area shape="rect" coords="76,547,172,573" href="###" id="8" />
<%
}else if(flow==16)
{
%>
<area shape="rect" coords="164,8,264,34" href="###" id="1" />
<area shape="rect" coords="164,80,264,120" href="###" id="2" />
<area shape="rect" coords="164,170,264,196" href="###" id="3" />
<area shape="rect" coords="164,244,264,270" href="###" id="4" />
<area shape="rect" coords="164,314,264,340" href="###" id="5" />
<area shape="rect" coords="164,384,263,423" href="###" id="6" />
<area shape="rect" coords="164,472,264,511" href="###" id="7" />
<area shape="rect" coords="164,559,264,585" href="###" id="8" />
<area shape="rect" coords="164,632,264,658" href="###" id="9" />
<area shape="rect" coords="164,704,264,730" href="###" id="4" pstep="/5/6/7/8/9/"/>
<area shape="rect" coords="163,775,264,814" href="###" id="10" />
<area shape="rect" coords="163,859,264,898" href="###" id="11" />
<area shape="rect" coords="164,945,264,971" href="###" id="12" />
<%
}
%>
</map>
