...
if("EditPurchase".equals(act))
{
  ...
  <table border="0" cellpadding="0" cellspacing="0" id="tablecenter">
    <tr id="tableonetr">
      ...
      <%
      if(flowtype==2||flowtype==1)//已审核未到货
      {
        out.print("<td align=center nowrap>采购数量</td>");
      }

      %>
      <td align="center" nowrap><% if(flowtype==1||flowtype==2 )//已审核未到货
      {
        out.print("到货");
      }%>数量</td>
    ...
    %>
      <tr onMouseOver="javascript:this.bgColor='#BCD1E9'" onMouseOut="javascript:this.bgColor=''" >
          <!--商品编号--><td align="center" <%if(flowtype==0){out.print("onclick=\"f_xuanze();\"   title=\"点击表格可以添加商品\"    style=cursor:pointer");}%>><%=sgobj.getGoodsnumber() %></td>
          <!--商品名称--><td <%if(flowtype==0){out.print("onclick=\"f_xuanze();\"   title=\"点击表格可以添加商品\" style=cursor:pointer");}%>   ><%=sgobj.getSubject()%></td>
          <!--规格型号--><td align="center" <%if(flowtype==0){out.print("onclick=\"f_xuanze();\"   title=\"点击表格可以添加商品\" style=cursor:pointer");}%> ><%=ssobj.getSpec()%></td>
          <!--单位--><td  <%if(flowtype==0){out.print("onclick=\"f_xuanze();\"   title=\"点击表格可以添加商品\" style=cursor:pointer");}%> ><%=ssobj.getMeasure()%></td>
        <!--单价--><td <%if(flowtype==0){out.print("onclick=\"f_xuanze();\"   title=\"点击表格可以添加商品\" style=cursor:pointer");}%>  ><input type="text" name="supply<%=i%>" value="<%out.print(ssobj.getSupply());%>" size="4" style="background:#cccccc" readonly="readonly">&nbsp;元</td>
       <!--采购数量--><td><%if(flowtype==0)
       {
         %>最小进货数量：<%=ssobj.getNum()%><%
       }
       ...
       {
         %>
         　<input style="background:#cccccc" readonly="readonly" type="text" name="quantitymm<%=i%>" value="<%if(gdobj.getQuantity()!=0){out.print(gdobj.getQuantity());}%>" size="4" ></td><td>
         <%
         }
         %>
          <input  type="hidden" name="quantitymin<%=i%>" value="<%=ssobj.getNum()%>" />
          <input type="text" name="quantity<%=i%>" value="<%if(gdobj.getQuantity()!=0){out.print(gdobj.getQuantity());}else{out.print(ssobj.getNum());}%>" size="4" <%if(flowtype==1||flowtype==2)
         {
         %>
         onkeyup="f_quantity2('<%=gdobj.getSupply()%>','<%=i%>','<%=chers%>');"
         <% }
         else
         {
         %>
         onkeyup="f_quantity('<%=gdobj.getSupply()%>','<%=i%>','<%=chers%>');"
         <%
         }
         %>
         ></td>
          ...
          if(flowtype==2)
          {
            out.print(" <td onclick=\"f_xuanze_pur();\" title=\"点击表格可以添加商品\"  style=cursor:pointer> ");
            out.print("<input type=\"text\" name=\"quantity22"+i+"\" value=\""+gdobj.getQuantity22()+"\" size=\"4\"  style=\"background:#cccccc\" readonly=\"readonly\"> ");
            out.print("</td>");
          }
          %>
          <% if(flowtype==2)//已审核未到货
          {
            out.print("    <td align=center nowrap>到货数量</td>");
          }%>
          <!--折扣乘以数量的价格总计--> <td onclick="f_xuanze();" title="点击表格可以添加商品"  style=cursor:pointer>
