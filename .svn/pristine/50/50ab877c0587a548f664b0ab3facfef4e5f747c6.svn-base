...
<input type="hidden" name="dir" value="xny">
<%
boolean none=true;
StringBuffer js=new StringBuffer();
  ...
  //列表////
  Enumeration efb=Flowbusiness.findByCommunity(teasession._strCommunity," AND flow IN("+ p_flow+")" + sql.toString());//" AND flow="+flow
  if(efb.hasMoreElements())
  {
    ...
    for(int j=1;efb.hasMoreElements();j++)
    {
      ...
      if(submit)
      {
        line.append("<a href='###' onClick=f_click("+j+",'/jsp/admin/xny/FlowbusinessEdit.jsp');form1.submit();>");
      }
      ...
      if(submit)
      {
        line.append("<input type=submit value=办理 onClick=f_click("+j+",'/jsp/admin/xny/FlowbusinessEdit.jsp');>");
      }else
      {
      ...
      if(!f.isHiddenConsign()&&member.equals(fv.getTransactor()))//有委托功能 && 是经办人.
      {
        ...
        {
          line.append("<input type=submit name=recovery value='收回委托' onclick=\"if(confirm('确认收回委托?')){ f_click("+j+",'/jsp/admin/flow/EditFlowbusinessConsign.jsp'); form1.method='post'; }else{ return false; }\">");
      ...
      if(last>0)//当前不是第一步
      {
        ...
        {
          line.append("<input type=submit class='butback' value=回上一步 onclick=f_click("+j+",'/jsp/admin/xny/EditFlowbusinessBack.jsp');>");
        }else //上步待办者不是多选
        {
      ...
      {
        line.append("<input type='submit' class='butnext' value='管理"+f.getName(teasession._nLanguage)+"' onclick=f_click("+j+",'/jsp/admin/xny/EditFlowbusinessNext.jsp');>");
      }else if(submit&&state>0)//未接收，不可传下一步
      {
        line.append("<input type='submit' class='butnext' value='传下一步' onclick=f_click("+j+",'/jsp/admin/xny/EditFlowbusinessNext.jsp');>");
      }
      //
...
<h2>创建工作列表</h2>
<table border='0' cellpadding='0' cellspacing='0' id='tablecenter'>
<tr><td><input type='button' value="创建工作" onclick="f_sel()" />
</table>
<%
}
%>
...
function f_sel()
{
  var rflow=window.showModalDialog('/jsp/admin/xny/FlowSel.jsp?community=<%=teasession._strCommunity%>&flow=<%=p_flow%>&t='+new Date().getTime(),self,'dialogWidth:400px;dialogHeight:320px;help:0;status:0;scroll:0;');
  if(!rflow)return;
  var rhead=1;
  if(rflow==9||rflow==16)//9:来文处理签 16:签报审批表
    rhead=0;
  else if(rflow==6||rflow==7|| rflow==10)
  {
    rhead=window.showModalDialog('/jsp/admin/xny/FlowDispatchHeadSel.jsp?community=<%=teasession._strCommunity%>&flow='+rflow+'&min='+(rflow==10?20:2)+'&t='+new Date().getTime(),self,'dialogWidth:500px;dialogHeight:320px;help:0;status:0;scroll:0;')
    if(!rhead)return;
    if(rhead==5&&rflow==6)
    rflow=11;//贵州桐梓河平行文、下行文发文流程
    else if(rhead==5&&rflow==7)
    rflow=12;//贵州桐梓河上行文发文流程
    else if(rhead==6&&rflow==6)
    rflow=13;//瓜州
    else if(rhead==6&&rflow==7)
    rflow=14;//瓜州
  }
  location.href="/jsp/admin/xny/FlowbusinessEdit.jsp?flow="+rflow+"&head="+rhead+"&nexturl="+encodeURIComponent(location.pathname+location.search);
}
