 ...
 */
var fs = ['', 'xx-small', 'x-small', 'small', 'medium', 'large', 'x-large', 'xx-large'];
...
var FCK = {
    ...
    GetLinkedFieldValue: function() { //iwlk
        var h = this.LinkedField.value;
        for (var i = 1; i < fs.length; i++) {
            h = h.replace(new RegExp(' style="font-size: ' + fs[i] + '">', ["g"]), ' size="' + i + '">');
        }
        var fn = FCKConfig.FontNames.split(';');
        for (var i = 0; i < fn.length; i++) {
            h = h.replace(new RegExp('<font style="font-family: ' + fn[i] + '"', ["g"]), '<font face="' + fn[i] + '"');
        }
        return h;
    },
    ...
    StartEditor: function() {
        ...
        if (FCKBrowserInfo.IsIE7) {
            if ((CTRL + 86
            /*V*/
            ) in A.Keystrokes) A.SetKeystrokes([CTRL + 86, true]);
            if ((SHIFT + 45
            /*INS*/
            ) in A.Keystrokes) A.SetKeystrokes([SHIFT + 45, true]);
        };
    ...
    UpdateLinkedField: function() {
        if(frameElement.style.display=="none")return; //iwlk
        FCK.LinkedField.value = FCK.GetXHTML(FCKConfig.FormatOutput);
        FCK.Events.FireEvent('OnAfterLinkedFieldUpdate');
        //iwlk:更新 字号/字体
        var h = FCK.LinkedField.value;
        for (var i = 1; i < fs.length; i++) {
            h = h.replace(new RegExp(' size="' + i + '">', ["g"]), ' style="font-size: ' + fs[i] + '">');
        }
        var fn = FCKConfig.FontNames.split(';');
        for (var i = 0; i < fn.length; i++) {
            h = h.replace(new RegExp('<font face="' + fn[i] + '"', ["g"]), '<font style="font-family: ' + fn[i] + '"');
        }
        FCK.LinkedField.value = h;
    },
    ...
    SwitchEditMode: function(A) {//点击源代码
...
function _FCK_EditingArea_OnLoad() {
    ...
    if (!FCKConfig.DisableEnterKeyHandler) FCK.EnterKeyHandler = new FCKEnterKey(FCK.EditorWindow, FCKConfig.EnterMode, FCKConfig.ShiftEnterMode);
...
function _FCK_KeystrokeHandler_OnKeystroke(A, B) {
    ...
    if (FCK.EditMode == 0) {
        if (B == 'Paste') return ! FCK.Events.FireEvent('OnPaste');
    } else {
        if (B.Equals('Paste', 'Undo', 'Redo', 'SelectAll')) return false;
    };
...
FCKTools.AddEventListenerEx = function(A, B, C, D) {
    ...
    if (FCK.IECleanup) FCK.IECleanup.AddItem(null,
    function() {
        o.Source = null;
        o.Params = null;
    });
...
FCKXHtml.GetXHTML = function(A, B, C) {
    ...
    if (!D) FCK.ResetIsDirty();
  	 ...
  	 var reg=new RegExp("<p>&nbsp;</p>","g"); //创建正则RegExp对象   --处理分页 <p></p>的问题
...
FCKUndo.SaveUndoStep = function() {
    ...
    if (FCKUndo.CurrentIndex + 1 >= FCKConfig.MaxUndoLevels) FCKUndo.SavedData.shift();
...
FCKPageBreaksProcessor.ProcessDocument = function(A) {//插入分页-1
...
FCKFlashProcessor.ProcessDocument = function(A) {
    ...
    while (i >= 0 && (C = B[i--])) {
        ...
        //if ((C.src&&C.src.EndsWith('.swf',true))||(D&&D.nodeValue=='application/x-shockwave-flash'))
        {
            ...
            //iwlk
            var type = C.src.EndsWith(".swf", true) ? (C.src.EndsWith("FLVPlayer_Progressive.swf", true) ? "Embed": "Flash") : "Media";
            if (FCKBrowserInfo.IsIE) {
                var F = ['scale', 'play', 'loop', 'menu', 'wmode', 'quality', 'flashvars'];
                if (type == "Voice") F = ['autostart', 'loop', 'enablecontextmenu', 'showcontrols', 'showtracker', 'showpositioncontrols', 'showaudiocontrols', 'showdisplay', 'showstatusbar'];
            ...
            var I = FCKDocumentProcessor_CreateFakeImage('FCK__' + type, E);
            I.setAttribute('_fck' + type.toLowerCase(), 'true', 0);
...
FCK.GetRealElement = function(A) {
    ...
    //iwlk
    if (A.getAttribute('_fckflash') || A.getAttribute('_fckembed') || A.getAttribute('_fckmedia')) {
...
FCKStyleDef.prototype.GetOpenerTag = function() {
    var s = '<' + this.Element;
    for (var a in this.Attributes) s += ' ' + a + '="' + this.Attributes[a] + '"';
    return s + '>';
};
...
FCKPageBreakCommand.prototype.Execute = function() {//插入分页
	//alert('aaa');
...
FCKTableCommand.prototype.Execute = function() {
    ...
    case 'TableDelete':
        ...
        break;
    default:
        alert("code_ie:" + FCKLang.UnknownCommand.replace(/%1/g, this.Name));
    }
};
...
//排版
var FCKComposeType = function() {
    this.Name = 'ComposeType';
}
FCKComposeType.prototype.Execute = function() {
	var oEditor = FCKeditorAPI.GetInstance(FCK.Name);
	  var  select=oEditor.EditorDocument.selection.createRange().text;
	    if(select!=null && select.length)
	    {
	    	alert('请不要选中内容，避免格式不对应!');
	    	return false;
	    }

	    var temp = oEditor.GetXHTML(true);
	    //清除html格式

	    temp =temp.replace(/<img/g,"Aimg");///(<\/?(?img|table|tr|th|td|thead|tbody|tfoot)[^>\/]*\/?\s*)>/gi
	    temp =temp.replace(/<table/g,"Atable");
	    temp =temp.replace(/<tr/g,"Atr");
	    temp =temp.replace(/<th/g,"Ath");
	    temp =temp.replace(/<td/g,"Atd");
	  ...
	  temp =temp.replace(/\n(&nbsp;| |　)*/g,"<br /><br />\n　　");
	   ...
	   temp = temp.replace(/\n/g, "<br /><br />");
	   temp =temp.replace(/Aimg/g,"<img");
	    temp =temp.replace(/Atable/g,"<table");
	    temp =temp.replace(/Atr/g,"<tr");
	    temp =temp.replace(/Ath/g,"<th");
	    temp =temp.replace(/Atd/g,"<td");


	    var oEditors = FCKeditorAPI.GetInstance(FCK.Name);
	   alert("内容排版已经成功,请确认。");
...
FCKComposeType.prototype.GetState = function() {
    return 0;
}
...
FCKFitWindow.prototype.Execute = function() {
   var A = window.frameElement;
    ...
    if (!this.IsMaximized) {
        ...
        ///小刘加///
        try {
            rows = parent.parent.frame.rows;
            cols = parent.parent.frame2.cols;
            if(rows == '120,*')
            {
            	parent.parent.frame.rows = "0,*";
            }else{
            	parent.parent.frame.rows = "0,0,*";
            }
           // parent.parent.frame.rows = "0,0,*";
            parent.parent.frame2.cols = "0,*,0";
        ...
        ///小刘加///
        try {
            parent.parent.frame.rows = rows;
            parent.parent.frame2.cols = cols;
...
FCKTools.GetDocumentPosition = function(w, A) {
    ...
    /*
    if ( (FCKBrowserInfo.IsIE && FCKDomTools.GetPositionedAncestor(A) == null)) {//FCKDomTools.GetCurrentElementStyle(w.document.body, 'position') != 'static' ||
...
FCKCommands.GetCommand = function(A) {
    ...
    case 'Flash':
        ...
        //iwlk
    case 'Images':
        B=new FCKDialogCommand('Images',FCKLang.DlgImagesTitle,'dialog/Images.html',550,400);
        break;
    case 'Embed':
        B = new FCKDialogCommand('Embed', FCKLang.DlgEmbedTitle, 'dialog/Embed.html', 450, 300);
        break;
    case 'Media':
        B = new FCKDialogCommand('Media', FCKLang.DlgMediaTitle, 'dialog/Media.html', 450, 300);
        break;
        //排版
    case 'ComposeType':
        B = new FCKComposeType();
        break;
...
FCKToolbarFontSizeCombo.prototype.CreateItems = function(A) {
    ...
    for (var i = 0; i < B.length; i++) {
        var C = B[i].split('/');
        this._Combo.AddItem(C[0], '<font style="font-size:' + C[1] + '">' + C[1] + '</font>', C[1]);
    }
}
...
FCKToolbarItems.GetItem = function(A) {
    ...
    case 'Flash':
        ...
        break;
        //iwlk
    case 'Images':B=new FCKToolbarButton('Images',FCKLang.InsertImagesLbl,FCKLang.ImagesEmbed,null,false,true,70);break;
    case 'Embed':
        B = new FCKToolbarButton('Embed', FCKLang.InsertEmbedLbl, FCKLang.InsertEmbed, null, false, true, 67);
        break;
    case 'Media':
        B = new FCKToolbarButton('Media', FCKLang.InsertMediaLbl, FCKLang.InsertMedia, null, false, true, 68);
        break;
        //自动排版
    case 'ComposeType':
        B = new FCKToolbarButton('ComposeType', FCKLang.ComposeType, null, null, false, true, 69);
        break;
...
FCKToolbarSet.prototype.Load = function(A) {
    ...
    if (!B) {
        B = FCKConfig.ToolbarSets["Default"]; ///////iwlk
        //alert(FCKLang.UnknownToolbarSet.replace(/%1/g,A));return;
    };
    this.Toolbars = [];
...
var FCKContextMenu = function(A, B) {
    ...
    if (FCKBrowserInfo.IsGecko) C.Document.addEventListener('draggesture',
    function(e) {
        e.preventDefault();
        return false;
    },
...
function FCK_ContextMenu_GetListener(A) {
    ...
    case 'Flash':
        return {
            ...
            function(menu, tag, tagName) {
                if (tagName == 'IMG') {
                    if (tag.getAttribute('_fckflash')) {
                        menu.AddSeparator();
                        menu.AddItem('Flash', FCKLang.FlashProperties, 38);
                    }
                    if (tag.getAttribute('_fckembed')) {
                        menu.AddSeparator();
                        menu.AddItem('Embed', FCKLang.DlgEmbedTitle, 67);
                    } //iwlk
                    if (tag.getAttribute('_fckmedia')) {
                        menu.AddSeparator();
                        menu.AddItem('Media', FCKLang.DlgMediaTitle, 68);
                    }
                }
            }
        };
