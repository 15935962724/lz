...
edn.menu=function()
{
  edn.scroll=document.getElementById("fm1").style;
...
edn.page=function(i)
{
  ...
  window.onscroll=function()
  {
    ...
    if(i<=_page||HTML.scrollHeight>HTML.clientHeight+Math.max(HTML.scrollTop,document.body.scrollTop)+(UA.indexOf(' UCBrowser/')>0?50:0)+10||PAGE.style.display=='')//只有UC浏览器要加49,50，原因未知
      return;
    console.log('load:'+_page+" max:"+i);
    PAGE.style.display='';
    setTimeout(function()
    {//无静态页，会有问题
  ...
  window.addEventListener('load',function()
  {
	...
	for(var i=0;i<arr.length;i++)
	{
      ...
      if(fv&&(src=='/tea/image/flv/ckplayer.swf'||src=='/tea/image/flv/audio.swf'))
      {
        ...
        while(bb=p.exec('&'+fv+'&'))
        {
          par[bb[1]]=decodeURIComponent(bb[2]);
        }
		var v=document.createElement('DIV');
		v.innerHTML="<video controls width='"+arr[i].width+"' height='"+arr[i].height+"' src='"+decodeURIComponent(par['f']||par['soundFile'])+"' poster='"+par['i']+"'"+(par['p']=='1'||par['autostart']=='yes'?" autoplay":"")+(par['e']=='1'||par['loop']=='yes'?" loop":"")+(par['m']=='1'?"":" preload")+" />";
