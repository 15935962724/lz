...
<%@ page import="tea.entity.yl.shop.Areas" %><%
...
//未回款预警值和报警值
float noreplywarn=sh.getNoreplywarn();
if(noreplywarn<0){
	noreplywarn=0;
}
float noreplyalarm=sh.getNoreplyalarm();
if(noreplyalarm<0){
	noreplyalarm=0;
}
int noinvoicewarn=sh.getNoinvoicewarn();
if(noinvoicewarn<0){
	noinvoicewarn=0;
}
int noinvoicealarm=sh.getNoinvoicealarm();
if(noinvoicealarm<0){
	noinvoicealarm=0;
}


	List<Areas> areas = Areas.find("", 0, Integer.MAX_VALUE);
...
<input type="hidden" name="area_name" value="<%=MT.f(sh.getArea_name()) %>"/>
<input type="hidden" name="addflag" value="<%=sh.getAddflag()%>"/>
<input type="hidden" name="issign" value="<%=issign %>" />
<table id="tablecenter">

  <tr>
    <td align="right">医院名称：</td>
    <td><input name="name" value="<%=MT.f(sh.getName())%>" <%out.print(id==0?"":"readonly");%>	size="31" alt="名称"/></td>
  </tr>
  <tr>
  	<td>省（自治区/直辖市）:</td>
		...
		<option value="">请选择省（自治区/直辖市）</option>
			<%
				for (int i = 0; i < areas.size(); i++) {
					...
					%>
			<option value="<%=areas1.getId()%>" <%=MT.f(sh.getArea()).equals(areas1.getId()+"")?"selected='selected'":""%>><%=areas1.getName()%></option>
				<%}
			%>
  ...
  <tr>
	...
	<tr>
		<td>客商编码:</td>
		<td><input type="text" name="h_code" value="<%out.print(sh.getH_code()==null?"":sh.getH_code());%>" />
		</td>
	</tr>
  <tr>
  	<td>未回款预警值:</td>
	<td><input type="text" name="noreplywarn" value="<%=MT.f(noreplywarn) %>" onkeyup="value=value.replace(/[^\d]/g,'')"/></td>
  </tr>
  <tr>
  	<td>未回款报警值:</td>
	<td><input type="text" name="noreplyalarm" value="<%=MT.f(noreplyalarm) %>"  onkeyup="value=value.replace(/[^\d]/g,'')"/></td>
  </tr>
  <tr>
  	<td>未开票粒子数预警值:</td>
	<td><input type="text" name="noinvoicewarn" value="<%=MT.f(noinvoicewarn) %>"  onkeyup="value=value.replace(/[^\d]/g,'')"/></td>
  </tr>
  <tr>
  	<td>未开票粒子数报警值:</td>
	<td><input type="text" name="noinvoicealarm" value="<%=MT.f(noinvoicealarm) %>"  onkeyup="value=value.replace(/[^\d]/g,'')"/></td>
  </tr>
  <tr>
  	<td>是否停止订货:</td>
	<td><%=sh.getIsstoporder()==0?"否":"是" %></td>
  </tr>
	<tr>
		<td>CRM编码:</td>
		<td><input type="text" name="crm_code" value="<%out.print(sh.getHos_code()==null?"":sh.getHos_code());%>" /></td>
	</tr>
  <%-- <%
  		int oldnoinvoice=sh.getOldnoinvoice();
  		String oldnoinvoicestr="";
  		if(oldnoinvoice==-1||oldnoinvoice==0){
  			oldnoinvoicestr="";
  		}else{
  			oldnoinvoicestr=String.valueOf(sh.getOldnoinvoice());
  		}
  		
  		float oldnoreply=sh.getOldnoreply();
  		String oldnoreplystr="";
  		if(oldnoreply==-1||oldnoreply==0){
  			oldnoreplystr="";
  		}else{
  			oldnoreplystr=String.valueOf(sh.getOldnoreply());
  		}
  %> --%>
  <%-- <tr>
  	<td>未开票粒子数:</td>
	<td><input type="text" name="oldnoinvoice" id="oldnoinvoice" value="<%=oldnoinvoicestr %>"  onkeyup="value=value.replace(/[^\d]/g,'')"/></td>
  </tr>
  <tr>
  	<td>应收账款:</td>
	<td><input type="text" name="oldnoreply" id="oldnoreply" value="<%=oldnoreplystr %>"  onkeyup="value=value.replace(/[^\d]/g,'')"/></td>
  </tr>
  <tr>
  	<td>日期节点:</td>
	<td><input type="text" name="timespot" id="timespot" value="<%=MT.f(sh.getTimespot()) %>" readonly="readonly" onclick="mt.date(this)"/></td>
  </tr> --%>
</table>
