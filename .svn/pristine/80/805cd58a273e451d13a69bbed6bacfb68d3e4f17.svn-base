...
{
   sql.append(" and friend like ").append(DbAdapter.cite("%"+memberids+"%"));
...
{
  sql.append(" and friend in (select member from ProfileLayer  where FirstName like "+DbAdapter.cite("%"+membername+"%")+" or LastName like   "+DbAdapter.cite("%"+membername+"%")+" )");
...
count = FriendList.count(sql.toString());
...
<%
     ...
     Iterator it = FriendList.find(sql.toString(),pos,pageSize).iterator();
