...
<%@ page import="tea.entity.league.*" %><%@ page import="tea.db.*" %><%@ page import="tea.entity.admin.erp.*" %>
<%
...
StringBuffer sql=new StringBuffer(" and  (lsstatic=1  or lsstatic=2)");
...
<title>分店信息删除管理</title>
</head>
<body onLoad="f_load()">
<h1>分店信息删除管理</h1>
<h2>查询</h2>
<form name="form3" action="/jsp/leagueshop/LeagueShopDelete.jsp" method="get" >
<input type="hidden" name="o" VALUE="<%=o%>">
<input type="hidden" name="aq" VALUE="<%=aq%>">
<input type="hidden" name="id" value="<%=request.getParameter("id")%>" >
<input type="hidden" name="act"  value="" >
<TABLE border="0" CELLPADDING="0" CELLSPACING="0" id="tablecenter">
  <tr><td nowrap="nowrap" align="right">分店名称：</td><td><input name="lsname"  value="<%if(lsname!=null){out.print(lsname);}%>"/></td><td align="right">所在区域：</td><td>
...
<tr><td align="right">加盟商姓名：</td><td><input name="lsbuyname" value="<%if(lsbuyname!=null)out.print(lsbuyname);%>"  /></td><td align="right">加盟时间：</td>
  <td>&nbsp;从&nbsp;
...
<tr>
<td align="right">分店类型：</td><td><select  name="lstype" onChange="f_ajax(value,'lsstype')"><option  value="0">-------</option>
<%
...
for(int i=0;eu.hasMoreElements();i++)
{
 ...
 <td align="right">分店级别：</td><td><select name="lsstype" ><option value="0" >-------</option><option value="5000">其他</option>
</select>　　
...
<input type="hidden" name="action" value="LeagueShopDelete">
<input type="hidden" name="sql" value="<%=sql.toString()%>">
<input type="hidden" name="files" value="LeagueShopDelete">
...
<TABLE border="0" CELLPADDING="0" CELLSPACING="0" id="tablecenter">
  ...
  {
    out.print("<tr><td nowrap colspan=7>暂无信息</td></tr>");
  }
  for(int i=0;euls.hasMoreElements();i++)
  {
    ...
    int pdnum=Paidinfull.count(teasession._strCommunity," and supplname="+lsid+" and (type!=6 and type!=7)");
    String showinfo="";
    if(pdnum!=0)
    {
      showinfo="分店尚有订单未完成！";
    ...
    {
      LeagueShop objls = LeagueShop.find(lsid);
      objls.getSummoney();
      BigDecimal bb = new BigDecimal("0");
      if(objls.getSummoney().compareTo(bb)==0)
      {
        showinfo="分店可以删除，请注意，删除分店后对应的联系人也应该删除！";
      }
      else
      {
        showinfo="分店尚有预付款！"+objls.getSummoney()+"元";
        lsid=0;
      }
    }
    %>
    <tr>
      <td nowrap><a href="/jsp/leagueshop/EditLeagueShop2.jsp?idss=<%=lsid%>"><%=objlist.getLsname()%></a></td>
      ...
      <td nowrap><%if(objtype.getLstypename()!=null){out.print(objtype.getLstypename()+"："+objserver.getLssname()); }%></td>
      <td nowrap><%=LeagueShop.LSSTATIC[objlist.getLsstatic()]%></td>
      <td nowrap="nowrap">
      <%if(objlist.getLsstatic()==1)
      {
      %>
         <input  type="button" value="删除分店" onClick="if(confirm('<%=showinfo%>')){window.open('/jsp/leagueshop/LeagueShopDelete.jsp?updatelsid=<%=lsid%>&updatetype=2', '_self');this.disabled=true;};" >
      <%
    }else if(objlist.getLsstatic()==2)
    {
      %>
      <input  type="button" value="彻底删除分店" onClick="if(confirm('<%=showinfo%>')){window.open('/jsp/leagueshop/LeagueShopDelete.jsp?updatelsid=<%=lsid%>&updatetype=3', '_self');this.disabled=true;};" >
      　　
      <input  type="button" value="恢复分店" onClick="if(confirm('<%=showinfo%>')){window.open('/jsp/leagueshop/LeagueShopDelete.jsp?updatelsid=<%=oldlsid%>&updatetype=1', '_self');this.disabled=true;};" >
      <%
    }
    %>
