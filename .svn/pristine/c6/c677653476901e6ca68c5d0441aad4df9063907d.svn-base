...
<HEAD>
  ...
  function f_open()
  {
   // window.open('/jsp/message/MemberList.jsp?community=<%=teasession._strCommunity%>','','width=450px,height=400px,top=300px,left=400px');
   var sd = window.showModalDialog("/jsp/message/MemberList.jsp?community=<%=teasession._strCommunity%>","","width=450px;height=350px;top=300px;left=400px;");
   if(sd!=undefined && sd!='/'){
     form1.name.value=sd;
   }
  }
  ...
  function f_type()
  {
    ...
    if(form1.radiotype[0].checked)
    {
      document.getElementById("mailtrid1").style.display='';
      document.getElementById("mailtrid2").style.display='none';
    }else if(form1.radiotype[1].checked)
    {
      document.getElementById("mailtrid1").style.display='none';
      document.getElementById("mailtrid2").style.display='';
    }

  }
  function f_submit()
  {
    if(form1.radiotype[0].checked &&form1.name.value=='')
    {
      alert("会员ID不能为空！");
      return false;
    }
     if(form1.radiotype[1].checked &&form1.emailname.value=='')
    {
      alert("外部邮件不能为空！");
      return false;
    }
  ...
  <DIV id="newmessage">
    ...
    <div id="head6"><img height="6" src="about:blank"></div>
      <br>
      <form name="form1" method="post" action="/servlet/UserMessage" enctype="multipart/form-data" onsubmit="return f_submit();" >
      ...
      %>
      <tr >
        <td nowrap align="right"><%=r.getString(teasession._nLanguage,"发送类型")%>:</td>
        <td nowrap >
          <input type="radio" name="radiotype" value="1" checked="checked" onclick="f_type();">&nbsp;<%=r.getString(teasession._nLanguage,"使用会员ID发送")%>&nbsp;&nbsp;
          <input type="radio" name="radiotype" value="2" onclick="f_type();">&nbsp;<%=r.getString(teasession._nLanguage,"使用外部邮件发送")%>
        </td>
      </tr>
      <tr id="mailtrid1">
        <td nowrap align="right"><%=r.getString(teasession._nLanguage, "会员ID")%>:</td>
        <td>
        <%
        if(!au.isHiddenorg()){
          %>
          <textarea name="name" id="name" readonly cols="50" rows="2"><%=name.toString()%></textarea>
          ...
          <input type="CHECKBOX" onclick="trsms.style.display=checked?'':'none'; form1.sms.disabled=!checked; form1.sms.value=form1.subject.value;" >同时发送到该人员的手机短信</td>
      </tr>
      <tr id="mailtrid2" style="display:none">
        <td nowrap align="right"><%=r.getString(teasession._nLanguage, "外部邮件")%>:</td>
        <td>
          <textarea name="emailname"  cols="50" rows="2"></textarea>
        </td>
      </tr>
...
<% }else if ("re".equals(act))//如果是回复 文章作者
{%>
  <tr >
        <td nowrap align="right"><%=r.getString(teasession._nLanguage, "会员ID")%>:</td>
        <td><%
        ...
        out.print(nobj.getCreator()._strR);
        out.print("<input type=hidden name =name value=/"+nobj.getCreator()._strR+"/ >");
        out.print(" <input type=hidden name=radiotype value=1 >");
        %></td>
      </tr>
<%
}
%>
        <tr>
          <td nowrap align="right"><%=r.getString(teasession._nLanguage, "Hint")%>:</td>
          <td>
        ...
        <tr>
          <td nowrap  align="right"><%=r.getString(teasession._nLanguage, "附件")%>:</td>
        ...
        <tr>
          <td colspan="2" nowrap><textarea name="content" rows="10" cols="60"  style="display:none" ><%=tea.html.HtmlElement.htmlToText(content)%></textarea>
            <iframe id="editor" src="/jsp/include/FCKeditor/editor/fckeditor.html?InstanceName=content&Toolbar=MySetting" width="700" height="200" frameborder="no" scrolling="no"></iframe></td>
        </tr>
        <tr>
      ...
      <div id="tm_show" style="display:none;position:absolute;width:200px;background-color:#F2F2F2;z-index:99;border:1px solid #F2F2F2;">
        <table id="tablecenter" style="margin-bottom:5px;margin-top:5px;">
          ...
          List e = Message.find(teasession._strCommunity,teasession._rv._strR,0,"", 0, Integer.MAX_VALUE);

          for(int i = 1; i<e.size();i++)
          {
            int id =(Integer)e.get(i);// ((Integer)e.nextElement()).intValue();
            String chk = "";
            if(i == 1){
              chk = "checked";
            }
