         ...
         out.print("<td>"+i+"</td>");
			for (int j = 1; j < ei.getLastCellNum(); j++) {
				...
				//out.print("<td ><input name='obj"+i+"-"+j+"' value='"+val+"' /></td>");
					...
					out.print("<td style='position: relative;'  for='"+Profile.FuwushangField[j]+"' ><input id='"+Profile.FuwushangField[j]+i+j+"' name='"+ Profile.FuwushangField[j] +"'  value='"+val+"' /></td>");
...
<script>
    function mysub(){
        //console.log(123);
        fn.ajax("/mobjsp/yl/shop/ajax.jsp?act=fuwushang",$("#form2").serialize(),function(data){
            if(data.type>0){
