  ...
  <table border="0" cellpadding="0" cellspacing="0" id="tablecenter">
    ...
    for(int i = 1;e.hasMoreElements();i++)
    {
      ...
      <tr onMouseOver="javascript:this.bgColor='#BCD1E9'" onMouseOut="javascript:this.bgColor=''" >
        <td align="center" onclick="f_xuanze();" title="点击表格可以添加商品"  style=cursor:pointer><%=sgobj.getGoodsnumber() %></td>
        <td onclick="f_xuanze();" title="点击表格可以添加商品"  style=cursor:pointer><%=sgobj.getSubject()%></td>
        <td align="center" onclick="f_xuanze();" title="点击表格可以添加商品"  style=cursor:pointer><%=sgobj.getSpec()%></td>
        <td  onclick="f_xuanze();" title="点击表格可以添加商品"  style=cursor:pointer><%=sgobj.getMeasure()%></td>
        <td><input type="text" name="quantity<%=i%>" value="<%=gdobj.getQuantity()%>" size="4" onkeyup="f_quantity('<%=gdobj.getSupply()%>','<%=i%>','<%=chers%>');"/></td>
