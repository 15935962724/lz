...
while(enumeration.hasMoreElements())
{
  ...
  Professional professionalobj=Professional.find(id);

%>
	<tr onMouseOver="javascript:this.bgColor='#BCD1E9'" onMouseOut="javascript:this.bgColor=''">
          <td><%=professionalobj.getBegda().toString().substring(0,10)%> -- <%=professionalobj.getBegda().toString().substring(0,10)%></td>
          <td><%=Professional.ZRZJB_TYPE[professionalobj.getZrzjb()]%></td>
          <td><%=professionalobj.getZfzdw()%></td>
        <td><input  type=button  value="<%=r.getString(teasession._nLanguage,"CBEdit")%>" onClick="window.open('<%=request.getRequestURI()%>?professional=<%=id%>&node=<%=teasession._nNode%>&community=<%=community%>&nexturl=<%=java.net.URLEncoder.encode(nexturl,"UTF-8")%>','_self')"/>
            <input   type=submit name=delete value="<%=r.getString(teasession._nLanguage,"CBDelete")%>" onClick="if(confirm('<%=r.getString(teasession._nLanguage,"ConfirmDelete")%>')){document.form1.professional.value=<%=id%>;return true;}else return false;"/>
        </td>
	</tr>
