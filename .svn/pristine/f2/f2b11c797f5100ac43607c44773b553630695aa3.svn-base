...
Filex.logs("qdr1031.txt",request.getQueryString()+request.getRequestURI()+"");
...
//上海出库 默认2、3
String mystatus=h.get("status","(2,3,-1,-2,-3,-4,-5)");
if(!"".equals(mystatus)){
...
<ul>
    ...
    <span><li>厂商：<%=MT.f(ProcurementUnit.findName(t.getPuid()))%></li></span>
    </li>
  <li class="btn"> 
    	<%-- <%
...
mt.act=function(a,id,status,statusContent)
{
  ...
  {
	  form2.action=("/mobjsp/yl/shop/ShopOrderDatas_sh.jsp?t="+<%=new Date().getTime()%>);
