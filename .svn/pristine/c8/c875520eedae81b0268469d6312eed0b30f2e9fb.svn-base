...
<%@page import="tea.entity.node.type.mpoll.*"%><%@page import="tea.entity.member.*"%><%@page import="tea.entity.admin.*"%><%
...
if("hour".equals(tab))
{
  ...
  try
  {
    java.sql.ResultSet rs=db.executeQuery("SELECT "+expr+" expr,SUM(CASE WHEN total>="+p.score+" THEN 1 ELSE 0 END),SUM(CASE WHEN total<"+p.score+" THEN 1 ELSE 0 END) FROM MVote WHERE poll="+p.poll+" AND time>"+DbAdapter.cite(st)+" AND time<"+DbAdapter.cite(et)+" AND deleted=0 GROUP BY "+expr+" ORDER BY expr",0,Integer.MAX_VALUE);
...
} else if("day".equals(tab))
{
  ...
  try
  {
    java.sql.ResultSet rs=db.executeQuery("SELECT "+expr+" expr,SUM(CASE WHEN total>="+p.score+" THEN 1 ELSE 0 END),SUM(CASE WHEN total<"+p.score+" THEN 1 ELSE 0 END) FROM MVote WHERE poll="+p.poll+" AND time>"+DbAdapter.cite(st,true)+" AND time<"+DbAdapter.cite(et)+" AND deleted=0 GROUP BY "+expr+" ORDER BY expr",0,Integer.MAX_VALUE);
    for (int i=1; rs.next(); i++)
    {
...
} else
{
  ...
  try
  {
    if("member".equals(tab))
    {
      java.sql.ResultSet rs=db.executeQuery("SELECT member,SUM(count0),SUM(count1) FROM MVote WHERE community="+DbAdapter.cite(h.community)+" AND folder=2 AND time>"+DbAdapter.cite(st,true)+" AND time<"+DbAdapter.cite(et)+" GROUP BY member ORDER BY SUM(count0) DESC",0,20);
      while(rs.next())
      {
        Profile m=Profile.find(rs.getInt(1));
        String tip =m.getMember();
        lab.append(",'"+MT.f(tip,6)+"'");
        //
...
<body>
<h1><%=AdminFunction.find(menu).getName(h.language)%></h1><!--投票统计-->
...
<a href="###" onclick="mt.tab(id)" id="ttemplate">分类</a>
-->
</div>
<script>
