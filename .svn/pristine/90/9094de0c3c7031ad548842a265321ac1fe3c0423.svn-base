...
for(int index=1;enumer.hasMoreElements();index++)
{
  ...
  <tr onMouseOver="javascript:this.bgColor='#BCD1E9'" onMouseOut="javascript:this.bgColor=''">
    ...
    <td><input type=button value=调看 onclick="window.open('/jsp/copyright/CrtransferView.jsp?community=<%=teasession._strCommunity%>&crtransfer=<%=id%>','','height=500,width=700,status=no,toolbar=no,menubar=no,location=no');" >
        <input type=button value=编辑 onclick="window.open('/jsp/copyright/EditCrtransfer.jsp?community=<%=teasession._strCommunity%>&crtransfer=<%=id%>&nexturl='+encodeURIComponent(location),'_self');" >
        <input type=button value=删除 onclick="if(confirm('确定删除吗？'))window.open('/servlet/EditCopyRight?community=<%=teasession._strCommunity%>&crtransfer=<%=id%>&act=deletecrtransfer&nexturl='+encodeURIComponent(location),'_self');" >
    </td>
 </tr>
<%
}
...
<input type=button value=添加 onclick="window.open('/jsp/copyright/EditCrtransfer.jsp?community=<%=teasession._strCommunity%>&crtransfer=0&nexturl='+encodeURIComponent(location),'_self');" >
