...
int arid = 0;
if(teasession.getParameter("arid")!=null && teasession.getParameter("arid").length()>0)
{
	arid = Integer.parseInt(teasession.getParameter("arid"));
}
Node nobj = Node.find(teasession._nNode);
NodeRole nrobj = NodeRole.find(NodeRole.getNrid(teasession._nNode,arid));
if("POST".equals(request.getMethod()))
{
	...
	//if(nrname!=null && nrname.length()>0){
		...
		if(nrobj.isExists()){
			nrobj.set(arid,teasession._nNode,sp.toString(),new Date(),teasession._strCommunity,teasession._rv.toString());
		}else{
			NodeRole.create(arid,teasession._nNode,sp.toString(),new Date(),teasession._strCommunity,teasession._rv.toString());
		}
	//}else
...
<body>
	...
	<table border="0" align="left" cellpadding="0" cellspacing="0" id="tablecenter" >
		<tr>
			...
			<td nowrap><input type="checkbox" name="nrname" value="13"  <%if(nrobj.isNr(13))out.print(" checked "); %>  style="cursor:pointer" >&nbsp;批量删除</td>
			<%if(nobj.getType()==0){ %>
		...
		<tr>
			...
			<td nowrap><input type="checkbox" name="nrname" value="9"   <%if(nrobj.isNr(9))out.print(" checked "); %>  style="cursor:pointer" >&nbsp;批量审核</td>
