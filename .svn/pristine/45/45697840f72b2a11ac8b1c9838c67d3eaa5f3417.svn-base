...
count = Subscribe.count(teasession._strCommunity,sql.toString());
...
function f_edit(igd)
{
  form1.subid.value = igd;
  form1.action="/jsp/general/subscribe/EditSubscribe.jsp";
  form1.submit();
}
	   ...
	   if(confirm('您确认执行【删除】操作?'))
	    {
	      ...
	      form1.action = '/servlet/EditSubscribe';
	      form1.submit();
	    }
	}
	if(igd=='publish')
	...
	<tr>
		...
		<td>
			<select name="publish">
				...
				<%
					for(int i = 0;i<Subscribe.PUBLISH_TYPE.length;i++)
					{
						...
						out.print(">"+Subscribe.PUBLISH_TYPE[i]);
						out.print("</option>");
					}
				%>
...
<input type="button" value="　增加　" onclick="window.open('/jsp/general/subscribe/EditSubscribe.jsp?node=<%=teasession._nNode %>&nexturl=<%=nexturl %>','_self');">&nbsp;
<input type="button" value="　删除　" onclick="f_sub('delete','请选择你要删除的套餐!');">&nbsp;
<input type="button" value="　公布　" onclick="f_sub('publish','请选择你要公布的套餐!');">&nbsp;
<input type="button" value="　取消公布　" onclick="f_sub('nopublish','请选择你要取消公布的套餐!');">&nbsp; 
  ...
  <table border="0" cellpadding="0" cellspacing="0" id="tablecenter">
    ...
    java.util.Enumeration e = Subscribe.find(teasession._strCommunity,sql.toString(),pos,pageSize);
    if(!e.hasMoreElements())
    {
      ...
      for(int i = 1;e.hasMoreElements();i++)
      {
        ...
        Subscribe subobj = Subscribe.find(sid);


    %>
    <tr onmouseover=bgColor='#BCD1E9'; onmouseout=bgColor=''; >
    <td width=1><input type="checkbox" name="checkbox_subid" value="<%=sid %>"/></td>
    <td><%=subobj.getSubject() %></td>
    <td><%=subobj.getMarketprice() %>&nbsp;元</td>
        <td><%=subobj.getPromotionsprice() %>&nbsp;元</td>
        
        <td><%
        if(subobj.getPublish()==0)
        {
        	out.print("<font color=Red>"+Subscribe.PUBLISH_TYPE[subobj.getPublish()]+"</font>");
        }
        else  if(subobj.getPublish()==1)
        {
        	out.print(Subscribe.PUBLISH_TYPE[subobj.getPublish()]);
        		
        }
         
        %></td>
        <td><% if(subobj.getPublish()==1){out.print(subobj.getPublishmember());} %></td>
        <td><%if(subobj.getPublish()==1){out.print(subobj.getPublishtimeToString());} %></td>  
        <td><a href="/jsp/general/subscribe/SubscribeShow.jsp?sid=<%=sid %>"  >查看</a>&nbsp;
        <%  if(subobj.getPublish()==0){ %>
        <a href="#" onclick="f_edit('<%=sid%>');">修改</a>
        <%} %>
        </td>
    </tr>
    <%} %>
