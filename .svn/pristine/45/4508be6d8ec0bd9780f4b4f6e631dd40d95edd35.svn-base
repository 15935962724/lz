...
StringBuffer sql=new StringBuffer(),par=new StringBuffer("?1=1");
...
function f_imp(){
	...
	for(var i=0;i<a.length;i++){
		var b=a[i];
		sendx("/jsp/admin/edn_ajax_city.jsp?act=impcity&cityid="+b[0]+"&namecn="+encodeURIComponent(b[2])+"&nameen="+b[1],function(data){});
		for(var j=0;j<CITY[b[0]].length;j++){
			var c=CITY[b[0]][j];
			sendx("/jsp/admin/edn_ajax_city.jsp?act=impcity&cityid="+c[0]+"&namecn="+encodeURIComponent(c[2])+"&nameen="+c[1],function(data){});
			for(var k=0;k<CITY[c[0]].length;k++){
				var d=CITY[c[0]][k];
				sendx("/jsp/admin/edn_ajax_city.jsp?act=impcity&cityid="+d[0]+"&namecn="+encodeURIComponent(d[2])+"&nameen="+d[1],function(data){});
...
if(count==0)
{
 ...
 %>
    ...
    <td>
		<input type='button' value='设置二级类'  onclick="f_setup('<%=cl.cityid%>');">&nbsp;
