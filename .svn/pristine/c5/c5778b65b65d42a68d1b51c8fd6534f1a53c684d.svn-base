...
//{
//     sendx("/jsp/erp/semi/GoodsGZ_ajax.jsp?goodstype2="+igd+"&act=goodstype&father="+form1.goodstype1.value,
//     function(data)
//     {
...
<table border="0" cellpadding="0" cellspacing="0" id="tablecenter">
   ...
   // System.out.println("SELECT DISTINCT n.node "+from.toString()+sql.toString());
    ...
    for (int index = 0; index < pos + pageSize && dbadapter.next(); index++)
    {
      if (index >= pos)
      {
        ...
        Commodity cobj = Commodity.find(Commodity.getCommodity(supplnamehidden,id));
        BuyPrice bpobj = BuyPrice.find(Commodity.getCommodity(supplnamehidden,id),1);
  %>
  ...
  <tr onMouseOver="javascript:this.bgColor='#BCD1E9'" onMouseOut="javascript:this.bgColor=''" title="点击选择商品"  onclick="f_button('<%=id%>');"  style=cursor:pointer>
    <td width="1"><%=is %></td>
      <td nowrap><%=node.getNumber() %></td><!--商品编号-->
