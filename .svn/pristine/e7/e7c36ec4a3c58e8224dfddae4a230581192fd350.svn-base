...
count = Loss.count(teasession._strCommunity,sql.toString());
...
<script type="text/javascript">
   function f_edit(igd)
   {
       ...
       form1.action="/jsp/erp/EditLoss.jsp"
       form1.submit();
   }
   ...
   function f_show(igd,str)
   {
     ...
     if(str==1){
      url = '/jsp/erp/LossShow.jsp?purid='+igd+'&act=ruku'
     }else if(str==2)
     {
...
<h1><%if(type==1){out.print("审核报损单");}else{out.print("添加报损单");}%></h1>
...
<table border="0" cellpadding="0" cellspacing="0" id="tablecenter">
  <tr id="tableonetr">
    <td nowrap><a href="javascript:f_order('purchase');">报损单号</a>
     <%
    ...
    <td nowrap><a href="javascript:f_order('time_s');">报损日期</a>
     <%
          if(o.equals("time_s"))
          {
            if(aq)
    ...
    <td nowrap><a href="javascript:f_order('member');">经办人</a>
     <%
          ...
          %></td>
    <td nowrap><a href="javascript:f_order('quantity');">报损数量</a>
     <%
...
<%
   java.util.Enumeration e = Loss.find(teasession._strCommunity,sql.toString(),pos,pageSize);
   if(!e.hasMoreElements())
   {
      out.print("<tr><td colspan=10 align=center>暂没有报损记录</td></tr>");
   }
   while(e.hasMoreElements()){
     String pur = ((String)e.nextElement());
     Loss pobj = Loss.find(pur);
...
%>
  <tr onMouseOver="javascript:this.bgColor='#BCD1E9'" onMouseOut="javascript:this.bgColor=''">
    ...
    <td align="center"><%=sobj.getName(teasession._nLanguage)%></td>
 ...
 <%
  ...
  <td >
    ...
    {
      out.print(" <a href=\"#\" onclick=f_edit('"+pur+"');>审核报损单</a>");
    }
   %>
    </td>
  </tr>
...
if(type == 0){
%>
<input type="button" value="添加报损单" onclick="window.open('/jsp/erp/EditLoss.jsp?nexturl=<%=nexturl%>','_self');"/>
<%} %>
</form>
