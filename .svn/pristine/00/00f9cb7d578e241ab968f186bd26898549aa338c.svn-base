...
<div id="tablebgnone" class="register">
  ...
  <table border="0" cellpadding="0" cellspacing="0" id="tablecenter" align="left">
    ...
    <tr id=type_1>
      <th> *
        <%=r.getString(teasession._nLanguage, "移动电话：")%>      </th>
      <td  colspan="3">
...
<%=r.getString(teasession._nLanguage, "性别：")%>   </th>
    ...
    <tr>
     ...
     <%if(member == null||member.equals(teasession._rv._strV)){ %>
     <td>
       <input type="button" value="修改密码" onClick="javascript:window.open('/jsp/registration/ChangePassword.jsp','anyname','width=500,height=200,top=600')"/>
     </td><%
     }%>
    </tr>
  </table>
