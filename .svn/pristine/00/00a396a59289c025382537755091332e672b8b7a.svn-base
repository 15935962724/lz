...
<head>
 ...
 <script>
	var arr=new RegExp("/(x?)html(\\d)/[\\w/]+/(\\d+)/(\\d+)-(\\d+).htm").exec("<%=MT.f(uri)%>");
	if(arr)
	{
...
<h1>提示信息</h1><p> <u>该页不存在，单击<A href="javascript:window.history.back();" >这里</A>返回。</u></p></TD>
  ...
  <style>
body{margin:0 auto;/*height:356px;width:714px;position:relative*/background:url(/res/Home/structure/14111170.jpg) no-repeat center center;}
.yl404 span a{font-size:12px;color:blue;position:absolute;top:55%;left:55%;}
</style>
<body><div class="yl404" style='min-height:356px;*height:380px'>
<!-- <p>很抱歉，您查看的页面找不到了！</p>-->
