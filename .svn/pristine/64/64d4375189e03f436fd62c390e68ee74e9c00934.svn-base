	...
	<div class='radiusBox newlist wSpan'>
		...
		<div class='f-tables'>
			<ul id="nvoice_edit" class='seachr emcolor' style="display: <%=soa.getMember()>0?"none":"" %>">
				<li><span align='right' nowrap><em>*</em>收货人：</span><input type="text" class='bors' name="consignees" value="<%=MT.f(soa.getConsignees()) %>" alt="收货人" placeholder="请输入收货人" /></li>
			<%
				if(profile.membertype == 2){
					out.print("<li><span><em>*</em>医&nbsp;&nbsp;&nbsp;院：</span>");
					...
					out.print("</select>");
			%>
				<br><span>科&nbsp;&nbsp;&nbsp;室：</span><select name="department">
			...
			%>
				<li class='ads'><span style='display:block'><em>*</em>所在地区：</span>
				<script>mt.city('city0','city1','city2','<%= soa.getCity()%>')</script>
				</li>
				<li><span><em>*</em>详细地址：</span><input type="text" placeholder="请输入详细地址" name="address" value="<%=MT.f(soa.getAddress()) %>" alt="详细地址" /></li>
				<li><em>注：请填写医院的详细地址。</em></li>
				<li><span><em>*</em>手机号码：</span><input placeholder="手机号" type="tel" placeholder="请输入手机号" class='bors' name="mobile" value="<%=MT.f(soa.getMobile()) %>" alt="手机号" /> </li>
				<li><span>固定电话：</span><input type="tel" placeholder="固定电话" name="telphone" value="<%=MT.f(soa.getTelphone()) %>" /></li>
				<li><span>邮编：</span><input class='bors' placeholder="邮编" type="tel" name="zipcode" value="<%=MT.f(soa.getZipcode()) %>" /></li>
	...
	<div class='f-tabless'>
		...
		<div class='radiusBox newlist wSpan'>
			<ul class="seachr emcolor">
				...
				<li><button type="button" onclick="mt.submit(this.form);">提交订单</button></li>
			</ul>
		</div>
	</div>
</form>
