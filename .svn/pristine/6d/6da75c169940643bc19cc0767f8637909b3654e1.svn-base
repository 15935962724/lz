...
#ContaintSlideNav01,#ContaintSlideNav02,#ContaintSlideNav01 li{display:block;width:100%;max-height:355px;overflow:hidden;-webkit-transition:left 800ms ease-in 0;-moz-transition:left 800ms ease-in 0;-o-transition:left 800ms ease-in 0;-ms-transition:left 800ms ease-in 0;transition:left 800ms ease-in 0;}
#ContaintSlideNav02 li{float:left;display:block;}
#ContaintSlideNav02 img{display:block;height:300px;}
#ContaintSlideNav03{display:block;height:auto;overflow:hidden; text-align:center; width:100%;text-align:center;margin:0 auto; padding-top:8px;}
#ContaintSlideNav03 a{display: inline-block;width:10px;height:10px;overflow:hidden; cursor:pointer;background:#a3a3a3;margin:10px 10px 10px 0px;-webkit-border-radius: 7px;
-moz-border-radius: 7px;
border-radius: 7px;}
#ContaintSlideNav03 a.active{background:#dc1f1f;}
#ContaintSlideNav01 ul li span{width:auto !important;max-height:350px !important;display:inline-block !important;}
#ContaintSlideNav01 ul li span img{width:100% !important;max-height:350px !important;}
</style>
<!--幻灯片start-->
...
int size=0;
if(r.getAlbum()>0){
ArrayList list= AlbumList.find(" and node="+r.getAlbum(),0,Integer.MAX_VALUE);
size=list.size();
for(int i=0;i<size;i++){
	AlbumList al=(AlbumList)list.get(i);
	%>
	<li><span><img src="/res/<%=h.community %>/ReserveAlbum1219/M_<%=al.album %>.jpg" alt=""></span></li>
	<%
}}
%>
...
for(int i=0;i<size;i++){
	%>
	<a href="#javascript(0)" class="<%=i==0?"active":"" %>"></a>
	<%
}
%>
