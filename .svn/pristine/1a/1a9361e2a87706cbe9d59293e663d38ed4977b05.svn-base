...
int count=0;//Node.count(sql.toString());
...
for(int i=1+pos;e.hasMoreElements();i++)
{
  ...
  out.print("<td>&nbsp;"+community.getStopTimeToString());
  out.print("<td>");
  ...
  out.print("<input type=button value="+r.getString(teasession._nLanguage, "<%=r.getString(lang,"fj12hq3z")%>")+" onClick=f_edit('edit','"+n.getCommunity()+"')>");
}
