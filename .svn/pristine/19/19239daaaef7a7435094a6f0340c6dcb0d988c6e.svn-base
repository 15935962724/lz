...
//  fsb.append("</select>");
  ...
  ArrayList e = Media.find(" AND m.community="+DbAdapter.cite(teasession._strCommunity)+" AND m.type=39 and m.media in (select media from medialayer)",0,Integer.MAX_VALUE);
  for(int i=0;i<e.size();i++)
  {
    Media nobj = (Media) e.get(i);
    fsb.append("<option value=" + nobj.media + ">" + nobj.getName(teasession._nLanguage) + "</option>");
    fsize++;
  }
