...
function checkRate()//判断是否编号是数字
{
	...
	function f_inquiry()//查询编号
	{
		...
		sendx("/jsp/type/perform/PerformOrders_ajax.jsp?act=BouncePiao_f_inquiry&pjnumber="+form1.pjnumber.value+"&community="+form1.community.value,
		  function(data)
		  {
	...
	function f_submit_tp()
	{
		...
		if(showqr.x!="1")//选中以后
		{
		  ...
		  sendx("/jsp/type/perform/PerformOrders_ajax.jsp?act=BouncePiao_f_submit_tp&pjnumber="+form1.pjnumber.value+"&community="+form1.community.value,
		  function(data)
		  {
	...
	{
		if(confirm('您确认要退次张票吗?')){
		  sendx("/jsp/type/perform/PerformOrders_ajax.jsp?act=BouncePiao_f_submit_qr&pjnumber="+form1.pjnumber.value+"&community="+form1.community.value,
		  function(data)
		  {
		  		 document.getElementById("showqr").innerHTML=data.trim();
		  }
	...
	function f_submit_cx()
	{
		 if(confirm('您确认要重新打印这张票吗?'))
		{
			 ...
			 var sp  ='?act=BouncePiao_f_submit_cx&pjnumber='+form1.pjnumber.value+'&community='+form1.community.value;
		  		openNewDiv('newDiv');
		           printApplet.openPrintApplet(ip,sp);
		       f_close('newDiv');  
		  	   window.open('/jsp/type/perform/BouncePiao.jsp','_self');
...
<input type="hidden" name="ip" value="<%out.print(request.getServerName()+":"+request.getServerPort()); %>">
  ...
  <table border="0" cellpadding="0" cellspacing="0" class="Search">
    <tr> 
       ...
       <td class="tdp3"><input type="button" class="SearchInput" value="" onClick="f_inquiry();"></td>
		</tr>
  </table>
<span id = "showpj">
