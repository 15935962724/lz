	  ...
	  for(int j=0;j<jids.length;j++)
	     {
	    	// id = Integer.parseInt(jids[j]);
	          LvyouApply.delete(jids[j],jobid);
	     }
   } 
...
<body id="bodynone">
 <h1>用户信息管理</h1>
  ...
  <form action="/jsp/type/lvyou/Findappusers.jsp" name="form1" method="POST">
    <input type="hidden" name="id" value="<%=id%>">
    <input type="hidden" name="jobid" value="<%=jobid%>">
    <input type="hidden" name="files" value="求职列表"/>
  ...
  <table border="0" cellpadding="0" cellspacing="0" id="tablecenter">
   <tr>
    ...
    <%
       ...
       ArrayList<ProfileLvyou> pls =LvyouApply.find(name, mobile, province, city, catagory, sex, day1, day2, state,jobid, pos, pageSize);
      // int count=ProfileLvyou.count(name, mobile, province, city, catagory, sex, day1, day2, state);
   
      int count=LvyouApply.count(name, mobile, province, city, catagory, sex, day1, day2, state, jobid);
    ...
    %>
     <tr onMouseOver="javascript:this.bgColor='#BCD1E9'" onMouseOut="javascript:this.bgColor=''">
        ...
        <td> 
        <a href="/jsp/type/lvyou/ShowUser.jsp?member=<%=plv.getMember()%>&jobid=<%=jobid%>" >详细</a>&nbsp;
