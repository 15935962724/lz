...
String status="3";//getNull(request.getParameter("status"));
...
//          System.out.println("SELECT Node.node FROM Node,SOrder WHERE SOrder.node=Node.node AND Node.type=66  AND Node.rcreator LIKE '%"+memberlike+"%'  "+sql.toString() + " GROUP BY SOrder.status");
          dbadapter.executeQuery("SELECT Node.node FROM Node,SOrder WHERE  Node.finished=1 AND Node.community="+dbadapter.cite(community)+" AND SOrder.node=Node.node AND Node.type=66  AND Node.rcreator LIKE "+dbadapter.cite(teasession._rv._strV)+"  "+sql.toString() + " ORDER BY SOrder.status");
...
int j2=dbadapter.getInt(1);
  ...
  <tr><!--EditSOrder.jsp-->
    ...
    <td><%=node.getTime("yyyy.MM.dd HH:mm aaa")%></td>
