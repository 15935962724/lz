 ...
 // String chersarr[] = chers.split("/");
  ...
  java.util.Enumeration e  = GoodsDetails.find(teasession._strCommunity," AND paid = "+DbAdapter.cite(purid),0,Integer.MAX_VALUE);
     for(int i = 1;e.hasMoreElements();i++)
     {
       ...
       Commodity cobj = Commodity.find(Commodity.getCommodity(pobj.getSupplname(),gdobj.getNode()));
       BuyPrice bpobj = BuyPrice.find(Commodity.getCommodity(pobj.getSupplname(),gdobj.getNode()),1);
