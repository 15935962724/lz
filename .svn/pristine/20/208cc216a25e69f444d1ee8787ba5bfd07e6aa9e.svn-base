...
<%java.util.Enumeration enumeration=tea.entity.node.Node.findByType(21,teasession._strCommunity);int nodeCode;
...
<%}enumeration=tea.entity.node.Node.findByType(18,teasession._strCommunity);
while(enumeration.hasMoreElements()){
     nodeCode =((Integer)enumeration.nextElement()).intValue();
%>
<OPTION VALUE="<%=nodeCode%>" ><%=tea.entity.node.Node.find(nodeCode).getSubject(teasession._nLanguage)%></OPTION>
...
<OPTION VALUE="">--请选择--</OPTION>
<% enumeration=tea.entity.node.Node.findByType(21,teasession._strCommunity);
while(                enumeration.hasMoreElements()){
   nodeCode =((Integer)enumeration.nextElement()).intValue();
%>
<OPTION VALUE="<%=nodeCode%>"><%=tea.entity.node.Node.find(nodeCode).getSubject(teasession._nLanguage)%></OPTION>
...
<INPUT  id="CHECKBOX" type="CHECKBOX" NAME="chkRights" VALUE="<%=power.getId()%>" onClick="account();" id="radio"><%=power.getName()%>
<%    }%>
          </TD>
          </TR>
          <TR>
