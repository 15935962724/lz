...
<table border="0" cellpadding="0" cellspacing="0" id="tablecenter">
  ...
  <td nowrap="nowrap">序号</td>
    ...
    <td nowrap><a href="javascript:f_order('usecausation');">事由</a> <%
          ...
          %></td>
    <td nowrap="nowrap"><a href="javascript:f_order('examinetype');">申请状态</a> <%
          if(o.equals("examinetype"))
          {
            if(aq)
            out.print("↓");
            else
  ...
  <%
       ...
       while(e.hasMoreElements())
       {
           ...
           if(saobj.getExaminetype()==0)str="等待审批";
           //if(saobj.getExaminetype()==1)str ="同意使用";
           if(saobj.getExaminetype()==-1)str ="不同意使用";
           if(saobj.getExaminetype()==1 && saobj.getSealtype()==0)str="审批完成,等待盖章";
           if(saobj.getExaminetype()==1 && saobj.getSealtype()==1)str="盖章完成";
       %>
       ...
       <td align="center"><%=index%></td>
         ...
         <%
             ...
             %>
              <input type=button value="<%=r.getString(teasession._nLanguage,"审批失败，重新编辑")%>"  onClick="editimd(<%=said%>);">
            <%
             }else{

               %>
                 <input type=button value="<%=r.getString(teasession._nLanguage,"CBEdit")%>"  onClick="editimd(<%=said%>);">
              <%
                 }
              %>
           <input type=button value="<%=r.getString(teasession._nLanguage,"CBDelete")%>"  onClick="deleteimd(<%=said%>);">
         </td>
       </tr>
    <%
    index++;
         ...
         %>
         <tr>
           <td colspan="7" align="center"><%=new tea.htmlx.FPNL(teasession._nLanguage,"?"+param.toString().replaceFirst("&pos=","&")+"&pos=",pos,count,size)%></td>
         </tr><%
       }
