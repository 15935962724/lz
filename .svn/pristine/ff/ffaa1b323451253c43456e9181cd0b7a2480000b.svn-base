...
//function f_sh()
//{
	//document.getElementById("mshowid").style.display ="";
...
function f_m()
{
		//var d = document.getElementById("mshowid").style.display;
		//if(d=='none')
		//{
		//	document.getElementById("mshowid").style.display ="";
		//	document.getElementById("mshow2").style.display="none";
		//}else
		///{
		//	document.getElementById("mshowid").style.display="none";
		//	document.getElementById("mshow2").style.display="";
...
#tablecenter td input#GoFinish3But{border:0px;width:99px;height:33px;background:url(/res/Home/1009/10099930.gif) no-repeat left top;}

.Submissioncon{border:1px solid #8EAFD7;background:#ECF3F7;margin-top:5px;font-size:12px;*height:100px;}
.Submissioncon .left{float:left;width:730px;text-align:center;}
.Submissioncon .left .Prompt{width:554px;color:#696969;text-align:left;line-height:200%;margin:0 auto;height:55px;*height:80px;padding-top:25px;}
.Submissioncon .left .Prompt span{color:#f00;}
.Submissioncon .right{float:left;width:214px;*width:242px;background:url(/res/Home/1006/100699107.gif) no-repeat left 50px;height:1008px;padding-left:28px;text-align:left;}
.Submissioncon .right .top{width:210px;border-bottom:1px solid #146B9E;background:url(/res/Home/1006/100699116.gif) no-repeat 2px 26px;height:20px;*height:44px;padding:24px 0px 0px 28px;font-size:14px;font-weight:bold;color:#146B9E;text-align:left;margin:0 auto;}
.Submissioncon .right .bottom{width:196px;padding-top:15px;font-size:13px;color:#8A8A8A;text-align:left;line-height:150%;margin-left:5px;}
.Submissioncon .left #tablecenter{width:710px;border:1px solid #4ABCFE;margin-top:5px;background:#F8FCFF;}
.Submissioncon .left #tablecenter #tabletr{font-size:14px;font-weight:bold;color:#413E3F;padding:10px 0px 0px 30px;}
.Submissioncon .left #tablecenter td{font-size:14px;color:#146B9E;padding:5px 3px;}
.Submissioncon .left #tablecenter td input,.Submission .con .left #tablecenter td textarea{border:1px solid #8EA9C2;}

.Submissioncon .left #tablecenter #Contributors_2 input,.Submissioncon .left #tablecenter #Contributors_7 input,.Submissioncon .left #tablecenter #Contributors_6 input,.Submissioncon .left #tablecenter #Contributors_5 input,.Submissioncon .left #tablecenter #Contributors_4 input,.Submissioncon .left #tablecenter #Contributors_8 input,.Submissioncon .left #tablecenter #Contributors_9 input{width:415px;}
.Submissioncon .left #tablecenter #Contributors_9 td div{font-size:12px;color:#8A8A8A;padding-top:15px;}
.Submissioncon .left #tablecenter td input.GoFinish{border:0px;width:94px;height:33px;background:url(/res/Home/1006/100699122.gif) no-repeat left top;margin-top:20px;cursor:pointer;}
</style>
...
<div class="Submissioncon">
<div class="left"><div class="Prompt">特别提示：<span>*</span>号代表必填项目，您的真实姓名和联系方式等个人信息，仅供本网调查、核实和反馈之用，将被严格保密。</div>
<div class="include">
<table border="0" align="center" cellpadding="0" cellspacing="0" id="tablecenter" height="10">
<tr>
	<td colspan="2" id="tabletr"><a href="###" onclick="f_m();">个人信息</a></td>
</tr>
</table>
<table border="0" align="center" cellpadding="0" cellspacing="0" id="tablecenter" height="10">
<thead id="mshowid" style="display:none">
...
<tr id="mshow2"><td>&nbsp;点击上面链接，显示个人信息</td></tr>
</table>
...
<tr id="Contributors_1">
    ...
    <td nowrap align="left">
      <select name="father">
        ...
        <%
        java.util.Enumeration e = Node.find(" and hidden =0 and type = 1 and "+DbAdapter.bitand("options1",4)+"!=0 and  node in (select node from Category where category =39) order by sequence asc  " ,0,Integer.MAX_VALUE);
        while(e.hasMoreElements()) 
        {
          int nid2 = ((Integer)e.nextElement()).intValue();
        ...
        %>
        </select> &nbsp;<a href="/html/folder/27647-1.htm" id="chakan">查看投稿栏目说明</a></td>
  </tr>
  ...
  <tr id="Contributors_2">
    ...
    <td align="left"><input type="text" class="edit_input"  name="signature" value="<%if(robj.getSignature(teasession._nLanguage)!=null)out.print(robj.getSignature(teasession._nLanguage)); %>"  maxlength="255">
    </td>
  </tr>
 ...
 -->
  ...
  <tr id="Contributors_9">
    ...
    <td align="center" colspan=2>
       ...
       <textarea style="display:none" name="content2" rows="12" cols="90" class="edit_input"> <%
     if(nobj.getText(teasession._nLanguage)!=null)
     {
    	 out.print(Report.getHtml(nobj.getText(teasession._nLanguage))); 
     }
     %></textarea>
      <iframe id="editor" src="/jsp/include/FCKeditor/editor/fckeditor.html?InstanceName=content2&Toolbar=<%=teasession._strCommunity%>" width="690" height="450" frameborder="no" scrolling="no"></iframe>
     ...
     <td colspan="2">　如果你上传的稿件是组图，请点击<A href = "###" onclick=f_te();>这里</A></td>
      ...
      <input type="button" value="" id="GoFinish3But" onclick=window.open('<%=nexturl %>','_self');>

      </td></tr> 
</table></div></div>
<%=community.getRuleshelp(teasession._nLanguage) %></div>
</form>
