...
<body>
<h1><%=r.getString(teasession._nLanguage, "ApprovedShippedTrade")%></h1>
...
<!--INPUT  id=CHECKBOX type="CHECKBOX" NAME=SendEmail VALUE=null CHECKED>
  同时按E-MAIL发送<BR--><br/>
   ...
   <!-- 发货日期：<%//=trade.getTimeToString()%>-->


<span id="span_express" style="" >  </span>
...
{
if(trade.getPayType()==0)//货到付款
{%>
<INPUT  id="radio" type="radio" NAME=Status VALUE=7 onclick="fexpress()" CHECKED>发货完成
...
<tr><td>到达时间:</td><td><%=new tea.htmlx.TimeSelection("come",null,true,true)%>(年-月-日-时-分)</td>
</tr>
<tr><td colspan="2"><INPUT TYPE="SUBMIT" VALUE="提交" ID="CBEnglish" CLASS="CB" onClick=""/></td>
</tr>
</table>
        <%}%>
</FORM>
