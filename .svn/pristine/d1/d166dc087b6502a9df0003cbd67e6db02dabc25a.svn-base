	...
	for(int i=0;i<clist.size();i++){
		...
		if(spdlist.size()>0){
			...
			if(MT.f(spd.content.replaceAll("<[^>]+>|&nbsp;","")).length()>30){
				out.print("<div class='texts'>"+MT.f(spd.content.replaceAll("<[^>]+>|&nbsp;",""),60)+"&nbsp;&nbsp;<a href='###' onclick=mt.act('proAuDescription',"+cg.category+","+spd.id+");>详细</a></div>");
			}else{
...
mt.act=function(act,c,t){
	if("proAuDescription"==act){
		...
		parent.location = "/xhtml/folder/14110208-1.htm?category="+c;
	}
