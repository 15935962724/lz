...
<%
  ...
  sql.append(" AND ( type = 5 OR type =6 )");
  ...
  <table border="0" cellpadding="0" cellspacing="0" id="tablecenter">
    ...
    for(int i = 1;e.hasMoreElements();i++){
      ...
      <tr onMouseOver="javascript:this.bgColor='#BCD1E9'" onMouseOut="javascript:this.bgColor=''">
        ...
        <td align="center"><%=pobj.getStimeToString() %></td>
        <td align="center"><%=pobj.getFtimeToString() %></td>
         <td align="center"><%
         if(pobj.getType() == 5)
         {
         ...
         %></td>
        <td align="center"><input type="button" value="详细" onclick="window.open('/jsp/erp/DeliverySituationDetail.jsp?paid=<%=paid%>&nexturl=<%=nexturl%>','_self');"/></td>
      </tr>
    <%} %>
     <%if (count > pageSize) {  %>
