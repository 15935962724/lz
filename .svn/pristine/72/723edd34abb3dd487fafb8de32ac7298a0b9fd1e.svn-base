...
<%
    ...
    //查看审批中的
    sql.append(" AND type=" + type+" AND hid="+menuid+" AND approval=1 ");
    ...
    <div class='radiusBox'>
        <table cellspacing="0" class='newTable'>
            ...
            <tr>
                ...
                <td><%=type==0?MT.f(ap.getYqrq()):ap.getBq()%>
                </td>
                <td>
                    <%
                        out.println("<button type=\"button\" class=\"btn btn-link\" onclick=\"mt.act('apPass'," + ap.getId() + ","+type+")\">通过</button>");
                        out.println("<button type=\"button\" class=\"btn btn-link\" onclick=\"mt.act('apRefuse'," + ap.getId() + ","+type+")\">拒绝</button>");
