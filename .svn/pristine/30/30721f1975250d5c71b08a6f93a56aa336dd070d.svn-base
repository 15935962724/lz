...
<h1><%if(type==1){out.print("审核调拨单");}else{out.print("添加调拨单");} %></h1>
...
<table border="0" cellpadding="0" cellspacing="0" id="tablecenter">
  <tr id="tableonetr">
    <td nowrap><a href="javascript:f_order('purchase');">调拨单号</a>
     <%
    ...
    <td nowrap><a href="javascript:f_order('time_s');">调拨日期</a>
     <%
          if(o.equals("time_s"))
          {
            if(aq)
          ...
          %></td>
    <td nowrap><a href="javascript:f_order('supplname');">调出仓库</a>
     <%
          ...
          %></td>
    <td nowrap><a href="javascript:f_order('waridname');">调入仓库</a>
     <%
    ...
    <td nowrap><a href="javascript:f_order('member');">经办人</a>
     <%
          ...
          %></td>
    <td nowrap><a href="javascript:f_order('quantity');">调拨数量</a>
     <%
...
<%
   ...
   {
      out.print("<tr><td colspan=10 align=center>暂没有调拨记录</td></tr>");
   }
...
%>
  ...
  <td >
    ...
    {
      out.print(" <a href=\"#\" onclick=f_edit('"+pur+"');>审核调拨单</a>");
    }
   %>
    </td>
...
if(type == 0){
%>
<input type="button" value="添加调拨单" onclick="window.open('/jsp/erp/EditCallout.jsp?nexturl=<%=nexturl%>','_self');"/>
<%} %>
</form>
