  ...
  for (i=0; i<document.body.all.length; i++) {
    ...
    if (curr.className == "yToolbar") {
      if (! InitTB(curr)) {
		...
		alert("工具栏: " + curr.id + " 初始化失败. 状态: 失败");	//Added by sanlly 20031231
      }
...
function PopulateTB(y) {
  ...
  for (i=0; i<elements.length; i++) {
	...
	if (! InitBtn(element)) {
	  ...
	  alert("正在初始化:" + element.id); 	//Added by sanlly 20031231
	  return false;
	}
      }
    ...
    default:
	 ...
	 alert("错误的类: " + element.className + " 在标签中: " + element.id + " <" + element.tagName + ">");	//Added by sanlly 20031231
      return false;
    }
  }
...
function validateMode() {
  ...
  alert("使用此工具栏需要先反选\"编辑源码\"单选框");	//Added by sanlly 20031231
  HtmlEditor.focus();
  return false;
}
...
function createLink() {
  ...
  var str=prompt("请输入链接地址:", isA ? isA.href : "http:\/\/");
...
function createImg()	{
	if (!validateMode()) return;
	var imgurl=prompt("请输入图片位置:", "http:\/\/");
