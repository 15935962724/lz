...
{
	String memberemail = teasession.getParameter("memberemail").trim();
...
}else 
{
	if(teasession.getParameter("member")!=null &&teasession.getParameter("member").length()>0&&!"/".equals(teasession.getParameter("member")))
	{
		...
		{
			   Profile  pobj = Profile.find(member.split("/")[i]);
			   sp.append(pobj.getMember()).append("/");
		}
	}
}
...
function f_sub()
{
		...
		if(form1.subject.value=='')
	{
		...
		return false;
	 	...
	 	ymPrompt.win({message:'<br><center><%=r.getString(teasession._nLanguage,"邮件信息发送中，请稍候...")%></center>',title:'',width:'300',height:'50',titleBar:false});
		document.getElementById('buttonid').value='邮件信息发送中，请稍候...';
