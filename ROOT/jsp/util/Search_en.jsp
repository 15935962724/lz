...
File f=new File(application.getRealPath("/res/" + community + "/searchindex/"+lucenelist+"_"+1));
if(!f.exists())
{
  response.sendRedirect("/jsp/info/Alert.jsp?info="+java.net.URLEncoder.encode(r.getString(0,"1169017432031"),"UTF-8"));//没有发现索引文件
  return;
}
...
try
{
  ...
  String st=ll.sorttype;
  if("".equals(st))
  {
 	 s=new Sort(new SortField[]{new SortField("", SortField.DOC, true)});
  }
...
}finally//catch(java.text.ParseException  pe2)
{
...
for(int i=0;i<v.size();i++)
{
  int id=((Integer)v.get(i)).intValue();
  Lucene lucene=Lucene.find(id);
  out.print(lucene.getBefore(1));
  out.print(lucene.getItypeToHtml(teasession));
  out.print(lucene.getAfter(1));
}
...
<%
out.print(" <span id='searchsum'>"+java.text.MessageFormat.format(r.getString(0,"Lucene.Results"),results)+"</span>");
...
try
{
  if(hits == null||sum==0)
  {
    out.print("<tr><td><br/><br/>"+tea.http.RequestHelper.format(r.getString(0,"Lucene.NotMatch"),keywords)+"<br/><br/>"+r.getString(0,"Suggestions")+"<br/></td></tr>");
  }else
  {
    for (int i=pos;i<(pos+10)&&i<sum;i++)
    {
      ...
      }else
      {
        ...
        "<tr><td id='searchdetail'><font color='#00CC00'>http://"+request.getServerName()+":"+request.getServerPort()+urlpath+"&nbsp;&nbsp;"+r.getString(0,"Time")+":"+time+"</font></td></tr>"+
        "<tr><td>&nbsp;</td></tr>");
      }
    }
    ...
    {
      out.print("<tr><td id='searchcorrelation'>"+r.getString(0,"Correlation")+":");
...
out.print("</table></div><div id = 'ali' align='center'>"+new tea.htmlx.FPNL(0,param.toString()+"&pos=",pos,sum,10) );
