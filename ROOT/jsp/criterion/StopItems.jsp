...
for(int index=1;enumer.hasMoreElements();index++)
{
  ...
  %>
        ...
        <td><%=obj.getCode()%></td>
         ...
         <td nowrap><%if(obj_au.getName()!=null)out.print(obj_au.getName());%>&nbsp;</td>
         <td nowrap><%=Item.STATES_TYPE[obj.getStates()]%></td>
                  <td nowrap><%=(obj.isNonce()?"是":"否")%>&nbsp;</td>
                  <td>
<input type="button" value="<%=r.getString(teasession._nLanguage,"CBEdit")%>" onClick="window.location='/jsp/criterion/StopEditItem.jsp?item=<%=item%>&community=<%=community%>&nexturl=<%=nexturl%>';"/>                 </td>
 </tr>
  <%
}
     %>
