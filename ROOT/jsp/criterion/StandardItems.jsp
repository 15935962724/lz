...
StringBuffer sql=new StringBuffer(" AND(states=7) ");
...
--%>
         ...
         {
         %>
<input type="button" value="<%=r.getString(teasession._nLanguage,"CBEdit")%>" onClick="window.location='/jsp/criterion/StandardEditItem.jsp?item=<%=item%>&community=<%=community%>&nexturl=<%=nexturl%>';"/>
         <%}else{%>
<input type="button" value="<%=r.getString(teasession._nLanguage,"Auditing")%>" onClick="window.location='/jsp/criterion/StandardAuditingItem.jsp?item=<%=item%>&community=<%=community%>&nexturl=<%=nexturl%>';"/>
