...
StringBuffer sql=new StringBuffer();//(" AND( states=6 OR states=7) ");
...
<td nowrap><%if(obj.getDirector()!=null)out.print(obj.getDirector());%>&nbsp;</td>
         ...
         {
         %>
<input type="button" value="<%=r.getString(teasession._nLanguage,"CBEdit")%>" onClick="window.location='/jsp/criterion/OtherEditItem.jsp?item=<%=item%>&community=<%=community%>&nexturl=<%=nexturl%>';"/>
         <%}else{%>

<input type="button" value="<%=r.getString(teasession._nLanguage,"Auditing")%>" onClick="window.location='/jsp/criterion/OtherAuditingItem.jsp?item=<%=item%>&community=<%=community%>&nexturl=<%=nexturl%>';"/>
           <%}%>
