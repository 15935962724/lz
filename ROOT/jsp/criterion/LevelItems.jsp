...
StringBuffer sql=new StringBuffer(" AND(states=6) ");
...
--%>
         ...
         {
         %>
<input type="button" value="<%=r.getString(teasession._nLanguage,"CBEdit")%>" onClick="window.location='/jsp/criterion/LevelEditItem.jsp?item=<%=item%>&community=<%=community%>&nexturl=<%=nexturl%>';"/>
         <%}else{%>

<input type="button" value="<%=r.getString(teasession._nLanguage,"Auditing")%>" onClick="window.location='/jsp/criterion/LevelAuditingItem.jsp?item=<%=item%>&community=<%=community%>&nexturl=<%=nexturl%>';"/>
