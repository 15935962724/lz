...
StringBuffer sql=new StringBuffer(" AND( states=5) ");
...
--%>
         ...
         {
         %>
<input type="button" value="<%=r.getString(teasession._nLanguage,"CBEdit")%>" onClick="window.location='/jsp/criterion/FormulatingEditItem.jsp?item=<%=item%>&community=<%=community%>&nexturl=<%=nexturl%>';"/>
         <%}else{%>

<input type="button" value="<%=r.getString(teasession._nLanguage,"Auditing")%>" onClick="window.location='/jsp/criterion/FormulatingAuditingItem.jsp?item=<%=item%>&community=<%=community%>&nexturl=<%=nexturl%>';"/>
