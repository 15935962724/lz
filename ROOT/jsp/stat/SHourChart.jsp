<%@page contentType="text/html; charset=UTF-8"%><%@page import="java.util.*"%><%@page import="tea.entity.*"%><%@page import="tea.ui.*"%><%@page import="tea.entity.stat.*"%><%@page import="tea.entity.admin.*"%><%
...
String[] TAB={"时统计","日统计","月统计"},TYPE={"走势图","柱状图"};
...
<body>
<h1><%=AdminFunction.find(menu).getName(h.language)%><%//=Menu.find(menu).name%></h1><!--趋势分析-->
...
{
  return "<%=SHour.chart(h)%>".replace(/'/g,'"');
}
mt.tab=function(a)
{
