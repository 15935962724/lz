...
<body>
<h1><%=r.getString(teasession._nLanguage, "GatheringTrade")%></h1>
...
{%>
收款:<td><input name="gathering" type="text">
   <tr><td>
发票:<td><SELECT name="invoice">
    <OPTION VALUE="0">已付</OPTION>
    <OPTION VALUE="1">未付</OPTION>
    <OPTION VALUE="2">已付增值税发票</OPTION>
  </SELECT> <tr><td>
<%
if(trade.getPayType()==0)
{
%>
状态:<td>
<INPUT  id="radio" type="radio" NAME=Status VALUE=7 onclick="fexpress()" CHECKED>完成
  <INPUT  id="radio" type="radio" NAME=Status  VALUE=1 onclick="fexpress()" >取消订单 <tr><td>
    发货时间:<td><%=new tea.htmlx.TimeSelection("unshipped",null,true,true)%>(年-月-日-时-分) <tr><td>
      估计到达时间:<td><%=new tea.htmlx.TimeSelection("estimate",null,true,true)%>(年-月-日-时-分) <tr><td>
<%}else{%>
状态:<td>
<INPUT  id="radio" type="radio" NAME=Status VALUE=4 onclick="fexpress()" CHECKED>发货
  <INPUT  id="radio" type="radio" NAME=Status  VALUE=1 onclick="fexpress()" >取消订单 <tr><td>
    发货时间:<td><%=new tea.htmlx.TimeSelection("unshipped",null,true,true)%>(年-月-日-时-分) <tr><td>
      估计到达时间:<td><%=new tea.htmlx.TimeSelection("estimate",null,true,true)%>(年-月-日-时-分) <tr><td>
<%
...
%>
  <INPUT TYPE="SUBMIT" VALUE="提交" ID="CBEnglish" CLASS="CB" onClick="return submitFloat(foProcess.gathering,'无效收款')"/>
<%}%>
</table>
</FORM>
<script type="">
