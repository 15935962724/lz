...
TeaSession teasession = new TeaSession(request);
            ...
            boolean flag = trade.getCVoiceFlag();//System.out.println(flag);
...
<tr id="TableCaption"><td COLSPAN=10>
<%=r.getString(teasession._nLanguage, "CText")%></td></tr><%if(flag4)
  {%>
<tr><TD><%=new FileInput(teasession._nLanguage, "Voice")%></TD></tr>
<tr><td COLSPAN=2><TEXTAREA name=CText COLS=60 ROWS=8 class="edit_input"><%=HtmlElement.htmlToText(s26)%></TEXTAREA></td></tr><%}else
            if(s26.length() != 0 || flag)
            {%>
                <td><%=s26%>
<%              if(flag)
                { %><input type="button" value="<%=r.getString(teasession._nLanguage, "CBPlay")%>" ID="CBPlay" CLASS="CB" onClick="window.open('/servlet/TradeCVoice?Trade=<%=i%>', '_self');">
              <%}
			}%>
</table>
