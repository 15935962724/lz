...
//if(referer==null||referer.indexOf("/jsp/order/")==-1)
//{
...
Profile p = Profile.find(obj.getCustomer()._strV);
...
<tr>
  ...
  <td><textarea name=address rows=2 cols=40><%=p.getAddress(teasession._nLanguage).replaceAll("</","&lt;/")%></textarea></td>  <td>*我们将根据此地址发送您选的商品并计算发货费用；</td>
</tr>
  <tr>
    <Td id="left"><%=r.getString(teasession._nLanguage, "EmailAddress")%>:</Td>
    <td><input type="TEXT" class="edit_input"  name=email value="<%=p.getEmail()%>" size=40 maxlength=40></td><td>*我们会通过邮件形式通知您发货情况；</td>
  </tr>
  <tr>
    <Td id="left"><%=r.getString(teasession._nLanguage, "LastName")%><%=r.getString(teasession._nLanguage, "FirstName")%>:</Td>
    <td><input type="TEXT" class="edit_input"  name=firstname value="<%=p.getFirstName(teasession._nLanguage)%>" size=20 maxlength=20>          </td>
      <td>*接收时需要接受人签字；</td>        </tr>
      <tr>
        <Td id="left"><%=r.getString(teasession._nLanguage, "Organization")%>:</Td>
        <td><input type="TEXT" class="edit_input"  name=organization value="<%=p.getOrganization(teasession._nLanguage)%>" size=40 maxlength=40></td>  <td>*指明接收单位，以免地址有误时错送；</td>
      </tr>
      <tr>
  <Td id="left"><%=r.getString(teasession._nLanguage, "Zip")%>:</Td>
  <td><input type="TEXT" class="edit_input"  name=zip value="<%=p.getZip(teasession._nLanguage)%>" size=20 maxlength=20></td><td>*正确填写，以方便发货方更快的找到您；</td>
</tr>
<tr>
  <Td id="left"><%=r.getString(teasession._nLanguage, "Telephone")%>:</td>
  <td><input type="TEXT" class="edit_input"  name=telephone value="<%=p.getTelephone(teasession._nLanguage)%>" size=20 maxlength=20></td><td>*紧急联系方式，请正确填写。</td>
</tr>
</table>
...
<table border="0" cellpadding="0" cellspacing="0" id="tablecenter">
  <tr onMouseOver="javascript:this.bgColor='#BCD1E9'" onMouseOut="javascript:this.bgColor=''">
    <td><input name="defray" type="radio" value="0" checked="checked">支付宝</td>
    <td>本次消费需要<%=s2+total%></td>
  </tr>
 </table>
...
<%
//如果记经确认地址的订单，则显示 支付证明
if(obj.getStatus()>=Trade.TRADES_CONFIRMED)
{
...
<table border="0" cellpadding="0" cellspacing="0" id="tablecenter">
  <tr>
    <td>会员留言:</td>
    <td><textarea name="remark" cols="50" rows="4"><%=obj.getRemark(teasession._nLanguage)%></textarea></td>
  </tr>
  <tr>
    <td>供货商留言:</td>
...
switch(obj.getStatus())
{
  case 0:
  case 1:
  out.println("<input type=submit value=取消订单 onclick='form1.status.value=1;'>");
  break;
}
