...
if(isurged==0){
	sql.append(" AND (isurged = "+isurged+" or isurged is null ) ");
	par.append("&isurged="+isurged);
}else if(isurged==1){
	sql.append(" AND isurged = "+isurged);
	par.append("&isurged="+isurged);
}
//按订单编号查询
...
String[] TAB={"全部订单","已完成开票","未完成开票","逾期未完成开票"};
//逾期未回款订单设为从开票日期起，超过一年还未回款的订单（只针对已完成开票的订单）
//String[] SQL={" and ((isclear=0 or isclear is null)or(isclear=1 and invoicestatus=3))"," AND isinvoicenum= (select quantity from shopOrderData where order_id=so.order_id) and ((isclear=0 or isclear is null)or(isclear=1 and invoicestatus=3 )) "," and (isinvoicenum<(select quantity from shopOrderData where order_id=so.order_id) or (so.status=7 and so.isinvoicenum>(select quantity from shopOrderData where so.order_id=order_id))) and (isclear is null or isclear=0)"," and (isinvoicenum<(select quantity from shopOrderData where order_id=so.order_id) or (so.status=7 and so.isinvoicenum>(select quantity from shopOrderData where so.order_id=order_id))) and DATEDIFF(y, createdate, GETDATE()) >=365 and (isclear is null or isclear=0) "};
//String[] SQL={" and status!=5 and status!=6 and (ishidden=0 or ishidden is null)"," and status!=5 and status!=6 and (ishidden=0 or ishidden is null) and invoicestatus=3 ","and status!=5 and status!=6 and (ishidden=0 or ishidden is null) and (invoicestatus is null or invoicestatus!=3)"," and status!=5 and status!=6 and (ishidden=0 or ishidden is null) and (invoicestatus is null or invoicestatus!=3) and DATEDIFF(y, createdate, GETDATE()) >=365 "};
String[] SQL={" and status!=5 and status!=6"," and status!=5 and status!=6 and  invoicestatus=3 and isclear=0 ","and status!=5 and status!=6  and (invoicestatus is null or invoicestatus!=3) and isclear=0"," and status!=5 and status!=6 and (ishidden=0 or ishidden is null) and (invoicestatus is null or invoicestatus!=3) and DATEDIFF(y, createdate, GETDATE()) >=365 "};
...
<tr>
	...
	<td><%=replynos %></td> --%>
	<td><%=statusContent %></td>
	<td><%=order.getIsurged()==0?"否":"<span style='color:red'>是</span>" %></td>
	<td>
...
<%
  ...
  if(sum>20)out.print("<tr class='fenye'><td colspan='20' align='right'>"+new tea.htmlx.FPNL(h.language, par.toString(), pos, sum,20));
}%>
</table>
</div>
</form>
...
<%
	if(tab==3){
%>
<button class="btn btn-primary" type="button" onclick="urgednoinvoice('<%=urgedpids %>')">形成催缴名单</button>
<%
	}
%>
</div>
<form action="/ShopOrders.do" name="form3"  method="post" target="_ajax" >
	<input name="act" value="exp_noinvo" type="hidden" />
...
mt.act=function(a,invoiceid)
{
  ...
  if(a=='data')
  {
	  form2.action="/jsp/yl/shopnew/NoInvoiceOrderDatas.jsp";
...
mt.receive2=function(v,n,h){
	...
	function urgednoinvoice(pids){
		  mt.show('/jsp/yl/shopnew/Listurged.jsp?pids='+pids,1,'逾期未开票催缴名单',800,500);
	}
