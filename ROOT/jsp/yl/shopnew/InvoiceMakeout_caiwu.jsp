...
<body>
	...
	<form name="form2" action="/Invoices.do" method="post" target="_ajax" onsubmit="return mt.check(this)&&mt.show(null,0)">
	<input type="hidden" name="invoiceid" value="<%=invoiceid %>" />
	<input type="hidden" name="act" value="submitcaiwu" />
	...
	<table id="tablecenter" cellspacing="0">
		...
		<tr>
			<td>发票号</td>
			<td><input type="text" name="invoiceno" alt="发票号" value="<%=MT.f(invoice.getInvoiceno()) %>"/></td>
		</tr>
		<tr>
			<td>医院</td>
			<td><%=invoice.getHospital() %></td>
		</tr>
		<tr>
			<td>开票价</td>
			<%if(invoice.getNum()==0||invoice.getAmount()==0){%>
			  <td></td>
			<%}else {%>
		...
		<tr>
			<td>快递单号</td>
			<td><input type="text" name="courierno"
					   <%
						   int mypuid = invoice.getPuid();
						   if(mypuid!= Config.getInt("gaoke")){
							   out.print("disabled=\"disabled\"");
						   }
					   %>
					   value="<%=MT.f(invoice.getCourierno()) %>"/></td>
		</tr>
		<tr>
			<td>开票日期</td>
			<td>
				<%-- <input type="text" alt="开票日期" id="makeoutdate" name="makeoutdate"  value="<%=MT.f(invoice.getMakeoutdate())%>" 
   readonly="readonly" onclick="mt.date(this)"> --%>
   <input class="Wdate" onfocus="WdatePicker({readOnly:true,maxDate:'%y-%M-%d'})" id="makeoutdate" name="makeoutdate"/> 
			<input type="button" value="获取当前日期"  onclick="fngettime()" />
			</td>
		</tr>
		<tr>
			<td>用户备注</td>
	...
	<%--<input class="btn" type="submit" value="开具发票">--%>
		<button class="btn btn-default" type="button" onclick="submitform()">开具发票</button>
...
function submitform() {
    console.log("ok");
	var no = form2.invoiceno.value;
	var time = form2.makeoutdate.value;
	console.log("no="+no);
