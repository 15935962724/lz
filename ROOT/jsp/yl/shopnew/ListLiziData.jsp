...
<%
int sum=ChangeLiziData.count(sql.toString());
if(sum<1)
{
...
}else
{
  ...
  List<ChangeLiziData> lstchange=ChangeLiziData.find(sql.toString()+" order by ordernum desc ", pos, size);
  for(int i=0;i<lstchange.size();i++)
  {
	  ChangeLiziData data=lstchange.get(i);
...
<tr>
    ...
    <td><%=member %></td>
    <td><%=data.getNoinvoice() %></td>
    <td><%=data.getIsinvoice() %></td>
    <td><%=MT.f(data.getTimespot()) %></td>
    <td><%=ChangeLiziData.ISBACK[data.getIsback()] %></td>
</tr>
