...
try{
	...
	inti =ei.getLastDataRowNum();

	int itemcount = 0;
			for (int i = ei.getDataRowNum(); i <= 0; i++) {
				//inti = ei.getDataRowNum();
				Row row = ei.getRow(i);
				JSONObject json = new JSONObject();
				for (int j = 0; j < ei.getLastCellNum(); j++) {
					intj = ei.getLastCellNum();
...
if(ei.getLastDataRowNum()==0){
	 ...
	 for (int i = ei.getDataRowNum()+1; i <= ei.getLastDataRowNum(); i++) {
         ...
         out.print("<td >"+i+"</td>");
			for (int j = 1; j < ei.getLastCellNum(); j++) {
				...
				out.print("<td style='position: relative;'  for='"+ProductStock.ProductStockField[j]+"' ><input id='"+ProductStock.ProductStockField[j]+i+j+"' name='"+ ProductStock.ProductStockField[j] +"' "+(j==3||j==4?"onclick=\"mt.date(this)\"":"")+" value='"+val+"' /></td>");
                count++;
			}
			out.print("</tr>");
		}
	 out.print("<script>");
	/* out.print("$('#form2').validate({");
	 out.print("submitHandler:function(form){");
	out.print("alert('提交事件!');");
	...
	for (int i = ei.getDataRowNum()+1; i <= ei.getLastDataRowNum(); i++) {
		for (int j = 1; j < ei.getLastCellNum(); j++) {
			...
			out.print(ProductStock.ProductStockField[j]+": {");
...
<script>
	function mysub(){
		fn.ajax("/ProductStocks.do?act=editPSList",$("#form2").serialize(),function(data){
			if(data.type>0){
