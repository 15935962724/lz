...
function createList(resultlist){
	...
	if(resultlist.data_list!=''){//是否存在数据
		for(var i=0;i<resultlist.data_list.length;i++){
			...
			var $name = $("<td>"+(parseInt(i)+1+parseInt(mypos))+"</td><td class='proflie' style='display:none;'>"+proObj.profile+"</td><td class='member'>"+proObj.member+"</td><td class='mobile'>"+proObj.mobile+"</td><td>"+proObj.email+"</td><td>"+proObj.company+"</td>");
			boxDiv.append($name);
    ...
    $('.transmit').on('click', function(){
        ...
        parent.$('input[name=profile]').val($(this).children(".proflie").text());
