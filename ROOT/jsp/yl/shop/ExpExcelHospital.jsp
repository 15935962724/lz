         ...
         out.print("<td>"+i+"</td>");
			for (int j = 1; j < ei.getLastCellNum(); j++) {
				...
				if(j>1){
					if(j==3){
						String opstr = "";
						for(String key : ShopHospital.shtype.keySet()){
							opstr += "<option value='"+ShopHospital.shtype.get(key)+"' "+(MT.f(val.toString()).indexOf(ShopHospital.shtype.get(key))!=-1?"selected='selected'":"")+" >"+key+"</option>";
						}
						out.print("<td><select name="+ShopHospital.ShopHospitalField[j]+">"+opstr+"</select></td>");
					}else if(j==4){
						String opstr = "";
						for(String key : ShopHospital.shlv.keySet()){
							opstr += "<option value='"+ShopHospital.shlv.get(key)+"' "+(MT.f(val.toString()).indexOf(ShopHospital.shlv.get(key))!=-1?"selected='selected'":"")+">"+key+"</option>";
						}
						out.print("<td><select name="+ShopHospital.ShopHospitalField[j]+">"+opstr+"</select></td>");
					}else if(j==5){
						String opstr = "";
						for(String key : ShopHospital.shmap.keySet()){
							opstr += "<option value='"+key+"' "+(key.indexOf(val.toString())!=-1?"selected='selected'":"")+">"+key+"</option>";
						}
						out.print("<td><select name="+ShopHospital.ShopHospitalField[j]+">"+opstr+"</select></td>");
					}else{
                        out.print("<td style='position: relative;'  for='"+ShopHospital.ShopHospitalField[j]+"' ><input id='"+ShopHospital.ShopHospitalField[j]+i+j+"' name='"+ ShopHospital.ShopHospitalField[j] +"'  value='"+val+"' /></td>");
                    }
				}else{
					out.print("<td style='position: relative;'  for='"+ShopHospital.ShopHospitalField[j]+"' ><input id='"+ShopHospital.ShopHospitalField[j]+i+j+"' name='"+ ShopHospital.ShopHospitalField[j] +"'  value='"+val+"' /></td>");
				}
...
<script>
    function mysub(){
        //console.log(123);
        fn.ajax("/mobjsp/yl/shop/ajax.jsp?act=editShopHospital",$("#form2").serialize(),function(data){
            if(data.type>0){
