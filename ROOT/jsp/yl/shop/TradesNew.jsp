...
sql.append(" order by state,time desc ");
...
if(state<1)
{
  out.print("<td>状态:<select name='s'>"+h.options(Trade.NEWSTATE_TYPE,s)+"</select><script>form1.s.options[0].text='---不限---';</script>");
}
%>
...
}else
{
  ...
  <tr onMouseOver="bgColor='#FFFFCA'" onMouseOut="bgColor=''">
    ...
    <td><%=sb.toString()%></td>
    <td><%=MT.f(t.aname)%></td>
    <td>￥<%=MT.f(t.actually)%></td>
    <td><%=MT.f(t.time,1)%></td>
    <td><%=MT.f(Trade.NEWSTATE_TYPE,t.state)%></td>
    <td>
...
function f_act(a,id,b)
{
  ...
  {
    form2.action='/jsp/yl/shop/TradeViewNew.jsp';form2.target='_self';form2.method='get';form2.submit();
  }
}
</script>
