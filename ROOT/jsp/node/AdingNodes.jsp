...
String sql=" AND n.rcreator="+DbAdapter.cite(teasession._rv._strR)+" AND n.node IN(SELECT node FROM Ading)";
...
<body>
<h1><%=j%><%=r.getString(teasession._nLanguage, "AdingNodes")%></h1>
<div id="head6"><img height="6" src="about:blank"></div>
  <div id="PathDiv"><A href="/servlet/Glance?Member=<%=teasession._rv%>" TARGET="_blank"><%=teasession._rv%></A> >
    <A HREF="/jsp/node/Nodes.jsp"><%=r.getString(teasession._nLanguage, "Nodes")%></A> >
    <%=r.getString(teasession._nLanguage, "AdingNodes")%>
</div>
...
else
{
  ...
  while(e.hasMoreElements())
  {
    ...
    out.print("<td><input type='button' onClick=window.open('/jsp/ading/Adings.jsp?node="+k+"'); value='"+r.getString(teasession._nLanguage, "Detail")+"'>");
  }
}
%>
</table>
