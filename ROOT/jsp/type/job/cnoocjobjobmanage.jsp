...
DbAdapter dbadapter = new DbAdapter();
        ...
        {//System.out.println("SELECT Distinct( Job.orgid) FROM Node,Job  WHERE Node.hidden=0 AND Node.node=Job.node and type=50 AND community="+dbadapter.cite(community)+sb.toString()+" ORDER BY sequence");
            dbadapter.executeQuery("SELECT Distinct( Job.orgid) FROM Node,Job  WHERE Node.hidden=0 AND Node.node=Job.node and type=50 AND community="+dbadapter.cite(community)+sb.toString());
            for (; dbadapter.next(); vector.addElement(new Integer(dbadapter.getInt(1))));

            dbadapter.executeQuery("SELECT Node.node FROM Node,Job  WHERE  Node.hidden=0 AND Node.node=Job.node and type=50 AND community="+dbadapter.cite(community)+sb.toString()+sb2.toString()+" ORDER BY sequence");
...
<div id="jigouwai" style="width:98%;">
  ...
  <tr  bgcolor="#FFFBF0">
    <td align="left" style="padding:5px "><span >&nbsp;您所管辖的机构下正在招聘的职位 <%=vector2.size()%> 个。  请选择机构
...
<%--            <td nowrap>
    ...
    <td nowrap>
        <div align="center"><Span ID=JobIDAlterCopy>
            <A href="/jsp/type/job/cnoocjobEditJob.jsp?node=<%=nodeCode%>&edit=alter&nexturl=/jsp/type/job/cnoocjobjobmanage.jsp?node=<%=nodeCode%>">修改</A>
        </Span>
        </div>
    </td>
<%--    <td nowrap>
...
<div style="TEXT-ALIGN: center;padding-top:10px">
  ...
  <INPUT TYPE="BUTTON" VALUE="创建职位" ID="CBNewJob" CLASS="in" onClick="window.open('/jsp/type/job/cnoocjobEditJob.jsp?NewNode=ON&Type=50&TypeAlias=0&node=15470&nexturl=/jsp/type/job/cnoocjobjobmanage.jsp?node=15542', '_self');"/>
  <INPUT TYPE="BUTTON" VALUE="末发布职位" ID="CBNewJob" CLASS="in" onClick="window.open('/jsp/type/job/cnoocjobendissue.jsp?NewNode=ON&Type=50&TypeAlias=0&node=15470&nexturl=/servlet/Folder?node=15542', '_self');"/>
</div>
</form>
<script>
