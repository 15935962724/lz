...
<div id="pathdiv"><%=node.getAncestor(teasession._nLanguage)%></div>
    ...
    out.print("<input type='hidden' name=nexturl value="+parameter+">");
          ...
          <TR>
            <TD>职位编号&nbsp;</TD>
            <TD>
              <INPUT class="edit_input" TYPE="text" NAME="txtRefCode" MAXLENGTH="30" VALUE="<%=job.getTxtRefCode()%>">
            </TD>
          </TR>
          <TR>
            <TD>* 所属机构&nbsp;</TD>
...
<OPTION VALUE="实习" <%=getSelect(job.getSltJobType().equals("实习"))%>>实习</OPTION>
          ...
          <TR>
            ...
            <TD> <TABLE BORDER="0" CELLSPACING="0" CELLPADDING="0" id="tablecenter">
                <TR>
                  <TD><input type=hidden name=sltOccId_BigClass><%---无意思的input ,只是为了解决JS的报错问题,JS 有了这个对象,--%>
                      <SELECT  class="edit_input" "width:200px"  ONCHANGE="javascript:SelectOption('Occupation',true, form1.sltOccId_BigClass,form1.sltOccId_list, '', '', '');">
...
%><option><%=enumerationOccId.nextElement()%></option><%}%>
          ...
          <TR>
            <TD>* 工作地区&nbsp;</TD>
            <TD> <TABLE id="talecenter" BORDER="0" CELLSPACING="0" CELLPADDING="0">
                <TR>
...
<OPTION VALUE="10"<%=getSelect(job.getSltReqWyearId()==10)%>>10年</OPTION>
          ...
          <TR>
            ...
            <TD> <TEXTAREA  class="edit_input" NAME="ObjectCN" COLS="70" ROWS="5"><%=HtmlElement.htmlToText(text)%></TEXTAREA>
              <br/>
              最多可以输入3000个字。<a id="linkObjectCN" href="javascript:alert(&quot;现已输入了&quot;+document.all['ObjectCN'].value.length+&quot;个字！&quot;);">计算字数</a>）<br>
...
<input  type="hidden" value="<%=teasession._nNode%>" name="Node"/>
     ...
     <input type="submit" name="GoBack" id="edit_GoBack" class="edit_button" VALUE="<%=r.getString(teasession._nLanguage, "Super")%>">
             ...
             <input class="edit_button"  type="submit" value="保存" name="save"/>
              <input  class="edit_button" type="button" value="取消" name="cancel" onclick="window.open('/servlet/Job?node=<%=teasession._nNode%>','_self')"/>
            </TD>
          </TR>
        </FORM>
      </TABLE>
