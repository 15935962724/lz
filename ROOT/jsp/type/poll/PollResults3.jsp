...
<form action="<%=request.getRequestURI()%>"  name="form1">
...
<%=r.getString(teasession._nLanguage,"Time")%>:
  <input name="begin" readonly="readonly" type="text" size="9" value="<%=begin%>"><img onClick="td_calendar('form1.begin');" align="absmiddle" src="/tea/image/public/Calendar2.gif" style="cursor:hand;" >
  --
  <input name="end" readonly="readonly" type="text" size="9" value="<%=end%>"><img onClick="td_calendar('form1.end');" align="absmiddle" style="cursor:hand;"  src="/tea/image/public/Calendar2.gif">
</td>
<%-- <td>
<%=r.getString(teasession._nLanguage,"Point")%>:<input name="point_from" type="text" size="5" value="<%=point_from%>">--<input name="point_to" type="text" size="5" value="<%=point_to%>">
</td>
--%>
<td>
...
<table border="0" cellpadding="0" cellspacing="0" id="tablecenter">
   ...
   <td><%=r.getString(teasession._nLanguage,"MemberId")%></td>
      <td><%=r.getString(teasession._nLanguage,"Point")%>
                <%--
      ...
      java.util.Enumeration  enumeration3= Poll.find(" AND type=2 AND node="+param_poll);
...
try
{
  db.executeQuery("SELECT DISTINCT vmember FROM PollResult WHERE 1=1"+sql.toString());
  while(db.next())
  {
    String member=db.getString(1);

    out.print("<tr onmouseover=\"javascript:this.bgColor='#BCD1E9'\" onmouseout=\"javascript:this.bgColor=''\" >");
    out.print("<td><A target=_blank href=\"/jsp/type/poll/PollResults2.jsp?community="+community+"&member="+member+"\" >");
    ...
    PollPoint pp_obj=PollPoint.find(param_poll,member);
    out.print("<td>"+pp_obj.getPoint()+"</td>");
    String time=null;
    ...
    {
      java.util.Enumeration enumeration5= Poll.find(" AND node="+param_poll);
...
<input type="button" value="导出" onclick="window.open('/servlet/PollResults?act=export&poll=<%=param_poll%>','_self');" >
