...
String bg = "XX";
if(request.getParameter("bg")!=null && teasession.getParameter("bg").length()>0)
{
...
<td width="1">&nbsp;</td>
  ...
  try
  {
    count=db.getInt("SELECT COUNT(node) FROM Node WHERE type=84 AND community="+DbAdapter.cite(teasession._strCommunity)+sql.toString());
    db.executeQuery("SELECT node FROM Node WHERE type=84 AND community="+DbAdapter.cite(teasession._strCommunity)+sql.toString()+" ORDER BY node DESC");
    for (int l = 0; l < pos + 25 && db.next(); l++)
    {
      if (l >= pos)
      {
        ...
        (reply_count > 0 && (System.currentTimeMillis() - InterlocutionReply.find(InterlocutionReply.findLast(n._nNode, teasession._nLanguage)).getTime().getTime() - 1000 * 60 * 60 * 24L) < 0))
        ...
        out.println("<td nowrap><input type=button value="+r.getString(teasession._nLanguage,"回复管理")+" onClick=\"window.open('/jsp/type/interlocution/InterlocutionReplyManage.jsp?bg="+bg+"&community="+teasession._strCommunity+"&node="+id+"&nexturl="+java.net.URLEncoder.encode(nexturl,"UTF-8")+"');\">");
        if(n.isHidden())
  ...
  %>
<tr><!--td colspan="3"><input type="checkbox" onclick="if(form2.nodes){form2.nodes.checked=this.checked;for(var i=form2.nodes.length;i>0;i--)form2.nodes[i-1].checked=this.checked; }" /><%=r.getString(teasession._nLanguage,"全选")%></td --><td colspan="6" align="center"><%=new tea.htmlx.FPNL(teasession._nLanguage,param.toString(),pos,count)%></td></tr>
</table>
