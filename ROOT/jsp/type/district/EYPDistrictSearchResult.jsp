...
StringBuffer sql=new StringBuffer(" FROM Node n,NodeLayer nl,District d WHERE n.community="+DbAdapter.cite(teasession._strCommunity)+" AND n.type=83 AND n.node=nl.node AND n.hidden=0 AND n.node=d.node");
String qs[]=null;
	...
	else
	{
		...
		for (int l = 0; l < pos + 10 && db.next(); l++)
		{
			if (l >= pos)
			{
				...
				District d=District.find(id,teasession._nLanguage);
			...
			%>
			  <tr>
			    <td><a id=companyname href="/servlet/Node?node=<%=id%>" ><%=subject%></a> ( <%=s.toString()%> )  点击量:<%=obj.getClick()%></td>
			  </tr>
			  <tr>
			    <td><br/></td>
			  </tr>
