...
sql.append(" AND node IN(SELECT node FROM plant WHERE 1=1");
String species=h.get("species","");
if(species.length()>0)
{
  sql.append(" AND species1 LIKE "+DbAdapter.cite("%"+species+"%"));
  par.append("&species="+h.enc(species));
}
...
<tr>
  <td class="th">种名：</td><td><input name="species" value="<%=species%>"/></td>
  <td class="th">属名：</td><td><input name="genus" value="<%=genus%>"/></td>
  <td class="th">科名：</td><td><input name="family" value="<%=family%>"/>　　<input type="submit" value="查询"/></td>
</tr>
</table>
</form>

<h2>列表 <%=sum%></h2>
<form name="form2" action="/Plants.do" method="post" target="_ajax">
<input type="hidden" name="community" value="<%=h.community%>"/>
...
mt.act=function(a,id,b)
{
  ...
  {
    form2.action='/jsp/type/plant/EditPlant.jsp';
