...
function f_s()
{
  ...
  {
    form1.action="/jsp/type/score/EditScore2.jsp";
    form1.submit();
  }
}
</script>
...
<h2>选择会员</h2>
<table cellpadding="0" cellspacing="0" id="tablecenter">
<tr><td>会员ID:<input name="member2" value="<%if(member2!=null)out.print(member2);%>"/></td>
  <td>用户名:<input name="code2" value="<%if(code2!=null)out.print(code2);%>"/></td>
  <td>手机号:<input name="mobile2" value="<%if(mobile2!=null)out.print(mobile2);%>"/></td>
...
<%
e=Profile.find(sql2,pos2,20);
for(int i=pos1+1;e.hasMoreElements();i++)
{
  String m=(String)e.nextElement();
  Profile p=Profile.find(m);
  out.print("<tr onMouseOver=bgColor='#BCD1E9' onMouseOut=bgColor=''><td><input type=checkbox name='member' onclick='f_c(this)' value='"+m+"'");
  if(m.equals(mid))out.print(" checked=''");
  out.print(" />");
  out.print("<td>"+m);
  out.print("<td>");
  if(p.code!=null)out.print(p.code);
  out.print("<td>"+p.getMobile());
}
%>
</table>
<%=new tea.htmlx.FPNL(teasession._nLanguage, par.toString()+"&pos2=", pos2, sum2,20)%>
