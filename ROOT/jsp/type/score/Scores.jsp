...
{
  sql+=" AND member LIKE "+DbAdapter.cite("%"+m+"%");
  par.append("&m="+h.enc(m));
}
...
function f_new()
{
  window.open("/jsp/type/score/EditScore1.jsp?community=<%=h.community%>&role=<%=isRole%>","_self");
}
function f_edit(sid)
{
  window.open("/jsp/type/score/EditScore2.jsp?community=<%=h.community%>&role=<%=isRole%>&score="+sid,"_self");
}
function f_del(sid)
{
...
<td>会员ID:<input name="m" value="<%=MT.f(m)%>"/>
