...
StringBuffer sql=new StringBuffer(" and  node in (select node from Node  where  type = 57 )");
...
try
{
	  db2.executeQuery("SELECT count(id) FROM BBSReply WHERE 1=1 "+sql.toString());
	  if(db2.next())
	  {
		  count = db2.getInt(1);
	  }
...
<body>
<h1><%=r.getString(teasession._nLanguage, "跟贴管理")%></h1>
...
<h2><%=r.getString(teasession._nLanguage, "列表")%>&nbsp;共有记录&nbsp;<%=count %>&nbsp;条&nbsp;
...
try
{
  ...
  db.executeQuery("SELECT id FROM BBSReply WHERE 1=1 "+sql.toString());
 
  while(db.next())
  {  
    int brid=db.getInt(1);
    BBSReply obj=BBSReply.find(brid);
    Node nobj = Node.find(obj.getNode());
    String value=Node.Html2Text(obj.getContent(teasession._nLanguage));
...
<tr onmouseover="javascript:this.bgColor='#BCD1E9'" onmouseout="javascript:this.bgColor=''" >
  ...
  <td><%=nobj.getSubject(teasession._nLanguage) %></td>
    <%
    out.print("<td><A target=_blank href=/servlet/BBS?node="+obj.getNode()+"#"+brid+">");
    if(obj.isHidden())
    {
    ...
    out.print("<td><a href=/jsp/type/bbs/EditBBSReply.jsp?node="+obj.getNode()+"&bbsreply="+brid+"&nexturl=/jsp/type/bbs/AdminBBSReplyManage.jsp?search=0>查阅</a></td>");
  }
}finally 
{
  db.close();
