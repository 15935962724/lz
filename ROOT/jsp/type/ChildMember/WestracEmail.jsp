...
if("POST".equals(request.getMethod()))
{
	...
	for(int i=1;i<memberemail.split("/").length;i++)
	{
		...
		if(m.trim().length()>0){
         ...
         se.sendEmail(Profile.getEmail(m),subject,contents);//p.getEmail()
		}
...
}else 
{
	...
	{
		String member = teasession.getParameter("member");//会员编号
		for(int i=1;i<member.split("/").length;i++)
		{
			String code = member.split("/")[i];
			sp.append(code).append("/");
		}
	}else
	{
		String sql = teasession.getParameter("sql");
		sql = MT.dec(sql);
		 java.util.Enumeration e = Profile.findCode(sql,0,Integer.MAX_VALUE);
