...
if((node.getOptions1()& 1) == 0)
{
  ...
  if (!node.isCreator(teasession._rv)&&!obj_am.isProvider(79))
  {
...
<body>
<h1><%=r.getString(teasession._nLanguage, "TravelTicketCorrelative")%></h1>
<div id="head6"><img height="6" src="about:blank"></div>
  ...
  <%
    ...
    <FORM name="foNew<%=type%>" METHOD=POST action="/servlet/EditCorrelative"   onsubmit="" >
      ...
      <table border="0" cellpadding="0" cellspacing="0" id="tablecenter">
        <tr>
          <td nowrap><%=r.getString(teasession._nLanguage, Node.NODE_TYPE[49])%>:</TD>
          <td nowrap>
            <select name="cnode">
              <option value="0">--------------------</option>
              <%
              enumer=    tea.entity.node.Node.findByType(49,node.getCommunity());
              while(enumer.hasMoreElements())
              {
                int id=((Integer)  enumer.nextElement()).intValue();
