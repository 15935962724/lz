...
/*
long lp1len=0;
java.io.File file=new File(application.getRealPath(event.getLocalePicture()));
lp1len=file.length();

long lp2len=0;
file=new File(application.getRealPath(event.getLocalePicture2()));
lp2len=file.length();

long lp3len=0;
file=new File(application.getRealPath(event.getLocalePicture3()));
lp3len=file.length();
*/
...
<form name="form1" method="post" action="/servlet/EditEvent"  ENCtype=multipart/form-data onSubmit="return(submitText(this.name, '<%=r.getString(teasession._nLanguage, "InvalidName")%>'))">
...
<INPUT TYPE="hidden" NAME="TypeAlias" VALUE="<%=request.getParameter("TypeAlias")%>">
 <table border="0" cellpadding="0" cellspacing="0" id="tablecenter">
    ...
    <tr>
      ...
      <td>
        <%=new TimeSelection("timeStart", event.getTimeStart())%><%-- <input type="text" name="timeStart" value="<%=event.getTimeStart()%>">--%>
       <%-- <%=new TimeSelection("timeStop", event.getTimeStop(),true,true)%><%-- <input type="text" name="timeStop" value="<%=event.getTimeStop()%>">--%>
      </td>
    </tr>
    <tr>
      <td><%=r.getString(teasession._nLanguage, "举行活动夜店名称")%>        :
</td>
      <td>
        <input type="text" class="edit_input" onKeyUp="checkint(this)" onChange="checkint(this)" name="nightShop" value="<%=event.getNightShop()%>"><input type="button" class="edit_input"  onclick="on_sel_rela('form1.nightShop')" value="浏览..."/>
      </td>
    </tr>    <tr>
      <td><%=r.getString(teasession._nLanguage, "分类")%>        :
</td>
      <td>
        <input type="text"  class="edit_input" name="sort" value="<%=event.getSort()%>">
    ...
    <tr>
      <td><%=r.getString(teasession._nLanguage, "活动要求")%>        :
</td>
      <td>
        <input type="text" class="edit_input"  name="request" value="<%=event.getRequest()%>">
      </td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>
        <TEXTAREA name=synopsis ROWS=5 COLS=60 class="edit_input"><%=HtmlElement.htmlToText(event.getSynopsis())%></TEXTAREA>
      </td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>
        <input type="text" name="organise"  class="edit_input" value="<%=event.getOrganise()%>">
      </td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>
        <input type="text" class="edit_input"  name="corp" value="<%=event.getCorp()%>">
      </td>
    </tr>
    <tr>
      ...
      <td>
        <input type="text" class="edit_input"  name="carfare" value="<%=event.getCarfare()%>">
      </td>
    </tr>
    <tr>
      ...
      <td>
        <input type="text" class="edit_input"  name="feature" value="<%=event.getFeature()%>">
      </td>
    </tr>
    <%--
      <tr>
    ...
    --%>
      ...
      <td>
          <INPUT TYPE=FILE NAME=introPicture onDblClick="window.open('<%=event.getIntroPicture()%>');" class="edit_input"><%if(iplen != 0) out.print(iplen + r.getString(teasession._nLanguage, "Bytes"));%><INPUT  id=CHECKBOX type="CHECKBOX" NAME=ClearIntroPicture VALUE=null><%=r.getString(teasession._nLanguage, "Clear")%>
      </td>
    </tr>
     <tr>
      ...
      <td>
          <INPUT TYPE=FILE NAME=bigPicture onDblClick="window.open('<%=getNull(event.getBigPicture()).replaceAll("\\\\","/")%>');" class="edit_input"><%if(lp1len != 0) out.print(lp1len + r.getString(teasession._nLanguage, "Bytes"));%><INPUT  id=CHECKBOX type="CHECKBOX" NAME=ClearBigPicture VALUE=null><%=r.getString(teasession._nLanguage, "Clear")%>
      </td>
    </tr>
    <tr>
