...
{	
	var s = "您确定要操作审核吗";
	if(form1.verifg.value==1){
		s = "您确定要取消审核吗";
	}
	
	if(confirm(s)){
		sendx("/jsp/admin/edn_ajax.jsp?act=WestracEventMemberListVerifg&eid="+igd,
				 function(data)
				 {
			        data = data.trim();
			 		if(data=='true')
...
function f_Delete(igd)
{
	if(confirm("您确定要删除这条记录吗？删除以后，不能恢复！")){
		sendx("/jsp/admin/edn_ajax.jsp?act=WestracEventMemberListDelete&eid="+igd,
				 function(data)
				 {
			        data = data.trim(); 
			 		if(data=='true')
...
function f_Wer(igd)
{
	 ...
	 var url = '/jsp/type/event/WestracEventRegistration.jsp?t='+new Date().getTime()+"&node="+form1.node.value+"&eid="+igd+"&community="+form1.community.value;
  ...
  if(submitCheckbox(form1.checkeid,strigd))
  {
		...
		if(igd=='delete')//
		{
			if(confirm('您确定要删除吗\n删除以后数据不能恢复？'))
			{
				...
				form1.act.value='WestracEventMemberListDeleteALL';
		...
		}else if(igd=='verifg')
		{
			...
			{
				form1.action='/servlet/EditEvent';
				form1.act.value='WestracEventMemberListVerifgALL';
...
function f_jf(igd)
{
 ...
 //确认是否到会
function f_confirmtype(igd)
 {

		 var y ='edge:raised;scroll:0;status:no;help:0;resizable:0;dialogWidth:330px;dialogHeight:130px;';
		 var url = '/jsp/type/event/WestracEventConfirmtype.jsp?t='+new Date().getTime()+"&node="+form1.node.value+"&eid="+igd+"&community="+form1.community.value;
		 var rs = window.showModalDialog(url,self,y);
...
function f_jfall()
{
  if(!submitCheckbox(form1.checkeid,'请选择要设置积分的用户'))return;
  
	  var members2="/";
	 
	  if(typeof(form1.checkeid.length)=='undefined')
	 {
		  members2 =members2+form1.checkeid.value+"/";
	 }else{
		  for(var i=0;i<form1.checkeid.length;i++)
		  { 
			  if(form1.checkeid[i].checked){
			     members2 =members2+form1.checkeid[i].value+"/";
			   }
		  }

	  }
    var rs=window.showModalDialog('/jsp/orth/SetIntegral.jsp?eid='+encodeURIComponent(members2),self,'scroll:0;status:0;help:0;resizable:1;dialogWidth:600px;dialogHeight:280px;');
    if(!rs)return;
...
{
	 var y ='edge:raised;scroll:0;status:no;help:0;resizable:0;dialogWidth:930px;dialogHeight:610px;';
	 var url = '/jsp/type/event/WestracEventAccompMember.jsp?t='+new Date().getTime()+"&node="+form1.node.value+"&community="+form1.community.value;
	 var rs = window.showModalDialog(url,self,y);
}
//创建报名表
function f_adminreg()
{
	 var y ='edge:raised;scroll:0;status:no;help:0;resizable:0;dialogWidth:930px;dialogHeight:610px;';
	 var url = '/jsp/type/event/WestracAdminEditEventreg.jsp?t='+new Date().getTime()+"&node="+form1.node.value+"&community="+form1.community.value;
	 var rs = window.showModalDialog(url,self,y);
...
{
	var y ='edge:raised;scroll:0;status:no;help:0;resizable:0;dialogWidth:930px;dialogHeight:610px;';
	 var url = '/jsp/type/event/WestracEventImportsreg.jsp?t='+new Date().getTime()+"&node="+form1.node.value+"&community="+form1.community.value;
	 var rs = window.showModalDialog(url,self,y);
...
<h1>活动名称：<%=nobj.getSubject(teasession._nLanguage) %>&nbsp;&nbsp;会员报名列表</h1>
<form name="form1" action="?" method="POST" target="tar" >
<input type="hidden" name="id" value="<%=request.getParameter("id") %>">
<input type="hidden" name="node" value="<%=teasession._nNode %>">
<input type="hidden" name="community" value="<%=teasession._strCommunity%>">
<input type="hidden" name="verifg" value="<%=verifg %>">
...
<h2>
<a href ="###" onclick="f_v('0')">未审核</a>
<a href ="###" onclick="f_v('1')">已审核</a>
<%
	if(verifg==1)
	{
		if(eventobj.getStay()==0)//可以有随行人员
		{
			out.println("<a href =\"###\" onclick=\"f_accom();\">查看随行人员</a>");
		}else if(eventobj.getStay()==1)
		{
			out.println("活动不安排住宿,没有随行人员");
		}
	}
%>
...
<table border="0" CELLPADDING="0" CELLSPACING="0" id="tablecenter">
 <tr id="tableonetr">
  <td width="1"><input type='checkbox' id="checkall" name="checkall" onclick='CheckAll()' title="全选" style="cursor:pointer"></td>
      ...
      <td nowrap>积分</td>
      <td nowrap>活跃度</td>
      <%if(verifg==1){out.println("<td nowrap>是否到会</td>");} %>
      <td>操作</td>
...
for(int i=0;eu.hasMoreElements();i++)
{
 ...
 <td width=1><input type=checkbox name=checkeid value="<%=eid%>" style="cursor:pointer"></td>
    ...
    <td nowrap="nowrap"><%=p.getMobile()%></td>
     ...
     <td><%if(p.getTime()!=null){out.print(Entity.sdf.format(p.getTime()));}%></td>
 	<td><%=p.getIntegral() %></td>
 	<td><%=Profile.ACTIVES_TYPE[p.getActives()] %></td>
 	   <%if(verifg==1)
 	   {
 		   out.println("<td nowrap>");
 		   
 		   if(eobj.getConfirmtype()==0)
 		   {
                out.println("未确认");
 		   }else if(eobj.getConfirmtype()==1)
 		   {
  ...
  %>
      ...
      <tr>
       ...
       <td colspan="7">
       <%if(verifg==1){ %>
       <input type=button value="取消审核" onclick="f_sub('verifg','请选中要取消审核的会员');">
       
       <%}else{ %>
        <input type=button value="审核" onclick="f_sub('verifg','请选中要审核的会员');">
       <% }%>
       ...
       &nbsp;<input type="button" value="删除" onClick="f_sub('delete','请选择您要删除的会员!');">
        <%if(verifg==1){ %>
         ...
         <input type=button value="创建报名表" onclick="f_adminreg();">
         <input type=button value="导入报名表" onclick="f_imp();">
