...
{
  if (!node.isCreator(teasession._rv)&&!AccessMember.find(node._nNode, teasession._rv._strV).isProvider(51))
  {
...
tea.entity.node.Investor investor =tea.entity.node.Investor.find(0, teasession._nLanguage);
%>
...
<h1><%=r.getString(teasession._nLanguage, Node.NODE_TYPE[51])%></h1>
  <div id="head6"><img height="6" src="about:blank"></div>
      ...
      <div id="pathdiv"><%=node.getAncestor(teasession._nLanguage)%>  </div>
       ...
       <FORM name=form1 onSubmit="return checkform(this);" action="/servlet/EditInvestor"  method=post>
               <%
    ...
    Date issDate=null;
        ...
        investor= tea.entity.node.Investor.find(teasession._nNode, teasession._nLanguage);
...
<table border="0" cellpadding="0" cellspacing="0" id="tablecenter">
     <TBODY>
  <TR>
        <TD align=right><%=r.getString(teasession._nLanguage, "index")%>：</TD>
        <TD colSpan=3><INPUT name=id  class="edit_input" value="<%=investor.getId()%>" maxlength="50"  >
      ...
      <TR>
        <TD align=right><%=r.getString(teasession._nLanguage, "Fundname")%>：</TD>
        <TD colSpan=3><INPUT name=fund_name  class="edit_input" value="<%=investor.getFundname()%>" maxlength="50">
          <FONT color=#ff0000>(*)</FONT></TD>
      </TR>
      <TR>
        <TD vAlign=top align=right><%=r.getString(teasession._nLanguage, "Fundinfo")%>：</TD>
      ...
      <TR>
        <TD align=right><%=r.getString(teasession._nLanguage, "Fundsum")%>：</TD>
...
<FONT
  color=#ff0000>(*)</FONT></TD>
      ...
      <TR>
        <TD align=right><%=r.getString(teasession._nLanguage, "Fundarea")%>：</TD>
        <TD colSpan=3>
          <input name="fund_area_pro"  class="edit_input" value="<%=investor.getFundarea()%>" maxlength="20"/>
          <FONT
      color=#ff0000>(*)</FONT></TD>
      </TR>
      <TR>
        <TD align=right><%=r.getString(teasession._nLanguage, "Fundtrade")%>：</TD>
        <TD colSpan=3><SELECT name=fund_trade>
      ...
      <TR>
        <TD vAlign=top align=right><%=r.getString(teasession._nLanguage, "Fundsymbiosis")%>：</TD>
        <TD colSpan=3><INPUT  id="radio" type="radio" checked="checked" value=合资 name=fund_symbiosis <%=getCheck(investor.getFundsymbiosis().equals("合资"))%>>
          合资　　　
      ...
      <TR>
        <TD vAlign=top align=right><%=r.getString(teasession._nLanguage, "Fundwill")%>：</TD>
        <TD colSpan=3><TEXTAREA class="edit_input"  name=fund_will rows=6 cols=45><%=investor.getFundwill()%></TEXTAREA></TD>
      </TR>
      <TR>
        <TD align=right><%=r.getString(teasession._nLanguage, "Fundwebsite")%>：</TD>
        <TD colSpan=3><INPUT name=fund_website class="edit_input"  id=fund_website value="<%=investor.getFundwebsite()%>" size="60" maxlength="20"></TD>
      </TR>
      <TR>
        <TD align=right><%=r.getString(teasession._nLanguage, "Fundperiod")%>：</TD>
        <TD colSpan=3><SELECT name=fund_period>
      ...
      <TR>
        <TD align=right><%=r.getString(teasession._nLanguage, "Fundlocal")%>：</TD>
        <TD colSpan=3>
          <INPUT name=fund_local  class="edit_input" value="<%=investor.getFundlocal()%>" maxlength="50">
          (<FONT
      color=#ff0000>详细地址</FONT>) </TD>
      </TR>
      <TR>
        <TD align=right><%=r.getString(teasession._nLanguage, "Fundidcard")%>：</TD>
        <TD colSpan=3><INPUT name=fund_IDcard  class="edit_input" value="<%=investor.getFundidcard()%>" size=18 maxlength="50"> </TD>
      </TR>
      <TR>
        <TD align=right><%=r.getString(teasession._nLanguage, "Fundlinkman")%>：</TD>
        <TD colSpan=3><INPUT name=fund_linkMan  class="edit_input" value="<%=investor.getFundlinkman()%>" size=10 maxlength="50">
          <FONT
      color=#ff0000>(*)</FONT></TD>
      </TR>
      <TR>
        <TD align=right><%=r.getString(teasession._nLanguage, "Fundtel")%>：</TD>
        <TD colSpan=3><INPUT name=fund_tel  class="edit_input" value="<%=investor.getFundtel()%>" maxlength="15">
          <FONT
      color=#ff0000>(*)以下（电话、传真、电子邮件、地址最少填写一项）</FONT></TD>
      </TR>
      <TR>
        <TD align=right><%=r.getString(teasession._nLanguage, "Fundfax")%>：</TD>
        <TD colSpan=3><INPUT name=fund_fax class="edit_input"  value="<%=investor.getFundfax()%>" maxlength="15"></TD>
      </TR>
      <TR>
        <TD align=right><%=r.getString(teasession._nLanguage, "Fundmail")%>：</TD>
        <TD colSpan=3><INPUT name=fund_mail  class="edit_input" value="<%=investor.getFundmail()%>" maxlength="50"></TD>
      </TR>
      <TR>
        <TD align=right><%=r.getString(teasession._nLanguage, "Fundpostcode")%>：</TD>
        <TD colSpan=3><INPUT  name=fund_postcode class="edit_input" value="<%=investor.getFundpostcode()%>" size=10 maxlength="10"></TD>
      </TR>
      <TR>
        <TD align=right><%=r.getString(teasession._nLanguage, "Fundaddress")%>：</TD>
        <TD colSpan=3><INPUT name=fund_address  class="edit_input" value="<%=investor.getFundaddress()%>" size=35 maxlength="128"></TD>
      </TR>
 <TR>
        <TD align=right><%=r.getString(teasession._nLanguage, "IssueTime")%>：</TD>
        <TD colSpan=3><%=new TimeSelection("Issue", issDate)%></TD>
      </TR>
 <TR align=middle>
