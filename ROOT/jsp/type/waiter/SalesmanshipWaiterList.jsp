...
//System.out.println("SELECT waiter,COUNT(waiter),SUM(gathering) FROM Salesmanship,Trade WHERE Salesmanship.trade=Trade.trade AND status=7" +sql.toString()+" GROUP BY waiter");
               dbadapter.executeQuery("SELECT waiter,COUNT(waiter),SUM(gathering) FROM Salesmanship,Trade WHERE Salesmanship.trade=Trade.trade AND status=7" +sql.toString()+" GROUP BY waiter");
  ...
  <tr>
    <td><%=obj.getCode()%></td>
    <td><a href="/jsp/type/waiter/SalesmanshipTradeList.jsp?waiter=<%=waiter%>&from=<%=from%>&to=<%=to%>"><%=dbadapter.getInt(2)%></a></td>
