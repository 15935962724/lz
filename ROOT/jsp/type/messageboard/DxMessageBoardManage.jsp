...
{
  sql.append(" AND node IN (SELECT node FROM MessageBoard WHERE mobile LIKE "+DbAdapter.cite("%"+mobile+"%")+")");
  param.append("&mobile="+java.net.URLEncoder.encode(mobile,"UTF-8"));
}
  ...
  function c_seat(id,m,mem)
    {
      ...
      var url = "/jsp/admin/edn_ajax.jsp?community="+form2.community.value+"&act=Dxmeminfo&node="+mem+"&member=<%=teasession._rv.toString()%>";
      url = encodeURI(url);
      send_request(url);
...
<td><input name="mobile"  value="<%if(mobile!=null)out.print(mobile);%>" type="text">
...
<br>
<form id="form2">
<h2>列表 ( <%=count%> )&nbsp;&nbsp;<input type="button" value="删除选中记录" onClick="delSelect();">&nbsp;
<span style="display:none"><input type="button" value="导出Excel表" onClick="f_excel();"></span>
</h2>
...
for(int i=pos+1;e.hasMoreElements();i++)
{
 ...
 out.print("</a>");
  ...
  out.println("<td nowrap><input type=button value="+r.getString(teasession._nLanguage,"回复管理")+" onClick=\"window.open('/jsp/type/messageboard/DxMessageBoardReplyManage.jsp?community="+teasession._strCommunity+"&node="+id+"&nexturl="+java.net.URLEncoder.encode(nexturl,"UTF-8")+"');\">");
  if(n.isHidden())
  {
