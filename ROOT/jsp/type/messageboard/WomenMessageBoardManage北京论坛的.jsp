<%@page contentType="text/html;charset=UTF-8" %><%@page import="tea.resource.*" %><%@page import="java.util.*" %><%@page import="tea.entity.*" %><%@page import="tea.entity.node.*" %><%@page import="tea.ui.*" %><%@page import="tea.db.DbAdapter" %><%@page import="java.net.URLEncoder"%>
...
for(int i=pos+1;e.hasMoreElements();i++)
{
 ...
 // }
  ...
  out.println("<td nowrap><input type=button value="+r.getString(teasession._nLanguage,"CBDelete")+" onClick=\"if(confirm('"+r.getString(teasession._nLanguage,"ConfirmDelete")+"')){window.open('/servlet/DeleteNode?community="+teasession._strCommunity+"&node="+id+"&nexturl="+java.net.URLEncoder.encode(nexturl,"UTF-8")+"','_self'); this.disabled=true;}\"></td></tr>");
}
%>
