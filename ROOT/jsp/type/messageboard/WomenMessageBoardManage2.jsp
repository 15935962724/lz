<%@page contentType="text/html;charset=UTF-8" %><%@page import="tea.entity.member.*" %><%@page import="tea.resource.*" %><%@page import="java.util.*" %><%@page import="tea.entity.*" %><%@page import="tea.entity.node.*" %><%@page import="tea.ui.*" %><%@page import="tea.db.DbAdapter" %><%@page import="java.net.URLEncoder"%>
...
if(act!=null&&"deleteBoeard".equals(act)){
	RV rv = teasession._rv;
	String nu=teasession.getParameter("nexturl");
	String one=teasession.getParameter("one");
	...
	if(cb!=null&&cb.length>0){
	    for(int j = 0;j < cb.length;j++)
	    {
	        ...
	        Vector v=node.getLanguages();
			Iterator it=v.iterator();
			while(it.hasNext()){
				int sit=(Integer)it.next();
		        node.delete(sit);
			}
			
	       // Node.delete(nid,node.getType(),teasession._nLanguage);
	        Logs.create(teasession._strCommunity,rv,3,teasession._nNode,node.getSubject(teasession._nLanguage));
	        TeaServlet.delete(node);
	    }
	}else if("true".equals(one)){
		int nid=teasession._nNode;
		 Node node = Node.find(nid);
	        Vector v=node.getLanguages();
			Iterator it=v.iterator();
			while(it.hasNext()){
  ...
  function c_seat(id,m,mem)
   ...
   function f_excel()
   {
			...
			if(confirm("您确定要导出数据吗?"))
		    {
				...
				form2.act.value='WomenMessageBoardManage2';
				form2.submit();
			}
...
<tr>
	...
	<td>
	<input name="subject"  value="<%if(subject!=null)out.print(subject);%>" type="text"></td>
...
<td><input name="q"  value="<%if(q!=null)out.print(q);%>" type="text"></td>
 </tr>
<tr>
...
<tr ID=tableonetr>
<td width="1"><input type="checkbox" name="selectAll" id="selectAll"  onclick="mt.select(form2.cb,checked);" /></td>
    <td nowrap>留言人</td>
     <td nowrap>单位</td>
     <td nowrap>职务</td>
    <td nowrap>内容</td>
...
for(int i=pos+1;e.hasMoreElements();i++)
{
 ...
 // if(n.isHidden()){out.print("<font color=red><strike>");}
  ...
  out.print("<td>"+n.getDescription(teasession._nLanguage)+"</td>");
  out.print("<td>&nbsp;");
  String log_content=n.getText(teasession._nLanguage); 
  if(log_content!=null)
  {
    log_content=Report.getHtml2(log_content);//log_content.replaceAll("<","&lt;");  log_content.replaceAll("\r\n","<br/>").replaceAll(" ","&nbsp;") // out.print("<a href=\"javascript:var obj=window.showmodaldialog ('','','height=250,width=500,left=400,top=300,scrollbars=yes,toolbar=no,status=no,menubar=no,location=no,resizable=yes');obj.document.write(document.getElementById('content_"+id+"').value);\" >"+log_content.replaceAll("&nbsp;","").substring(0,25)+"...</a>");//&#39;
    
    if(log_content.length()>25){  
     // out.print("<a href=\"javascript:var obj=window.showmodaldialog ('','','height=250,width=500,left=400,top=300,scrollbars=yes,toolbar=no,status=no,menubar=no,location=no,resizable=yes');obj.document.write(document.getElementById('content_"+id+"').value);\" >"+log_content.replaceAll("&nbsp;","").substring(0,25)+"...</a>");//&#39;
     %>

        <textarea style=display:none id="content_<%=id %>"><%=log_content%></textarea>
       <a href='#' onclick="mt.show(document.getElementById('content_<%=id %>').value,1,'查看',800,400)"><%=log_content.replaceAll("&nbsp;","").substring(0,25) %></a>
    <%} 
    else{ 
 ...
 // }
  ...
  out.println("<td nowrap><input type=button value="+r.getString(teasession._nLanguage,"CBDelete")+" onClick=\"if(confirm('"+r.getString(teasession._nLanguage,"ConfirmDelete")+"')){window.open('?act=deleteBoeard&community="+teasession._strCommunity+"&one=true&node="+id+"&nexturl="+java.net.URLEncoder.encode(nexturl,"UTF-8")+"','_self'); this.disabled=true;}\"></td></tr>");
}
%>
