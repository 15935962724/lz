...
sql.append(" AND n.type=65 AND n.finished=1 AND vcreator="+DbAdapter.cite(h.username));
sql.append(" AND node IN(SELECT node FROM Service)");
...
}else
{
  ...
  <tr onMouseOver="bgColor='#FFFFCA';" onMouseOut="bgColor=''">
    ...
    <td></td>
    <td><%=MT.f(s.locationid)%></td>
    <td><%=MT.f(s.commericial)%></td>
    <td><%=MT.f(s.landmark)%></td>
    <td>
...
<br/>
<input type="button" value="添加" onclick="mt.act('add','14050241')"/>
</form>
...
mt.act=function(a,id,b)
{
  ...
  {
    if(a=='edit'){
      location.href='/jsp/type/service/EditService.jsp?node='+id;
      return;
    }
    else if(a=='add'){
    	form2.action='/jsp/type/service/EditService.jsp';
    }
    form2.submit();
  }
