...
<table border="0" cellspacing="0" cellpadding="0" id="tablecenter">
  ...
  <%-- <tr>
    <td><%=r.getString(teasession._nLanguage, "所属区域")%>: </td>
    <td>      <select name="Area">
      <%
      for(int st=0;st<ListingDetail.SHOW_TYPE.length;st++)
      {
        out.print("<option value='"+st+"'");
        if(ld.getIstype("Area")==st)out.print(" selected=''");
        out.print(">"+ListingDetail.SHOW_TYPE[st]);
      }
      %>
      </select>
	<td><%=r.getString(teasession._nLanguage, "Before")%><input name="Area_1" mask="max" value="<%=HtmlElement.htmlToText(ld.getBeforeItem("Area"))%>" type="text">
    ...
    <%=r.getString(teasession._nLanguage, "Sequence")%>:<input name="Area_3" type="text" value="<%=ld.getSequence("Area")%>" mask="int" maxlength="3" size="4">
	<input  id=CHECKBOX type="CHECKBOX" name="Area_4"   <%=getCheck(ld.getAnchor("Area"))%>><%=r.getString(teasession._nLanguage, "Anchor")%>
     </td>
  </tr> --%>
  <tr>
    <td><%=r.getString(teasession._nLanguage, "所属机构")%>: </td>
    <td>      <select name="shop">
      <%
      for(int st=0;st<ListingDetail.SHOW_TYPE.length;st++)
      {
        out.print("<option value='"+st+"'");
        if(ld.getIstype("shop")==st)out.print(" selected=''");
        out.print(">"+ListingDetail.SHOW_TYPE[st]);
      }
      %>
      </select>
	<td><%=r.getString(teasession._nLanguage, "Before")%><input name="shop_1" mask="max" value="<%=HtmlElement.htmlToText(ld.getBeforeItem("shop"))%>" type="text">
	<%=r.getString(teasession._nLanguage, "After")%><input name="shop_2" mask="max" value="<%=HtmlElement.htmlToText(ld.getAfterItem("shop"))%>" type="text">
    <%=r.getString(teasession._nLanguage, "Sequence")%>:<input name="shop_3" type="text" value="<%=ld.getSequence("shop")%>" mask="int" maxlength="3" size="4">
	<input  id=CHECKBOX type="CHECKBOX" name="shop_4"   <%=getCheck(ld.getAnchor("shop"))%>><%=r.getString(teasession._nLanguage, "Anchor")%>
     </td>
  </tr>
  <tr>
