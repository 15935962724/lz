...
}else
{
  ...
  <tr id="<%=MT.enc(p.profile)%>">
    <td><input type="checkbox" name="members" value="<%=MT.f(p.emailbooking).length()>0?MT.f(p.emailbooking):MT.f(p.getEmail())%>" /></td>
    <td><a href="###" onclick="mt.act(this,'view')"><%=MT.red(p.member,username)%></a></td>
    <td><%= MT.f(p.getMobile()) %></td>
    <td onclick="selected('<%= MT.f(p.emailbooking).length()>0?MT.f(p.emailbooking):MT.f(p.getEmail()) %>');"><a href="###"><%= MT.f(p.emailbooking).length()>0?MT.f(p.emailbooking):MT.f(p.getEmail()) %></a></td>
