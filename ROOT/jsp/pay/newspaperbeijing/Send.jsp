...
String pkorder = teasession.getParameter("pkorder");//订单

if(pkorder!=null && pkorder.length()>0)
{
...
String v_mid = "3051";		//商户编号，签约由易支付分配。250是测试商户编号。
String MD5Key="guojiahuaj751122"; //MD5Key,签约后由商户自定义一个16位的数字字母组合作为密钥，发到huangyi@payeasenet.com.说明商户编号，公司名称和密钥。
...
String v_oid = ddate1+"-"+v_mid+"-"+pkorder+ddate;  //订单编号，订单编号的格式:yyyymmdd-v_mid-流水号。流水号可以按照自己的订单规则生成，但是要保证每一次提交，订单号是唯一值，否则会出错

String v_rcvname = "3051"; //收货人姓名，建议用商户编号代替。
String v_rcvaddr = "3051"; //收货人姓名，可以用常量
...
String v_url="http://"+request.getServerName()+":"+request.getServerPort()+"/jsp/pay/newspaperbeijing/Receive.jsp";  //支付完成后返回地址。此地址是支付完成后，订单信息实时的向这个地址做返回。返回参数详见接口文档第二部分。
