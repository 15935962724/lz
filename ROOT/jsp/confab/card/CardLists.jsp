...
try
{
  ...
  while(db.next())
  {
    ...
    if(d_obj.getSort()==sort){
      ...
      out.print("<td><A target=_blank href=/jsp/confab/card/DynamicValueViewTab.jsp?node="+node_id+"&community="+community+" >"+d_obj.getType()+"-"+node_id+"</A></td>");
      ...
      if(enumer_code.hasMoreElements())
      {
        ...
        out.print("<input type=button value=\"浏览\" onclick=window.open('/jsp/confab/card/CardQrcView.jsp?node="+node_obj._nNode+"','','width=500px,height=300px');> <input type=button onclick=\" window.open('/jsp/confab/EditConfabDynamicValue.jsp?community="+community+"&node="+node_id+"&nexturl="+java.net.URLEncoder.encode(nexturl,"UTF-8")+"','_self'); \" value=编辑 > <input type='button' value="+r.getString(teasession._nLanguage, "CBDelete")+" onClick=\"if(confirm('"+r.getString(teasession._nLanguage, "ConfirmDeleteTree")+"')){window.open('/servlet/DeleteNode?node="+node_id+"&nexturl="+java.net.URLEncoder.encode(nexturl,"UTF-8")+"', '_self');this.disabled=true;}\" >");
