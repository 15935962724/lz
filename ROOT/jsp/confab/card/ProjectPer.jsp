  ...
  while(db.next())
  {
     ...
     if(d_obj.getSort()==sort){
       ...
       while(de.hasMoreElements())
       {
         ...
         if(dobj.isQrc())
         {
           ...
           if(fa){
             out.print("<td>"+ dobj.getName(teasession._nLanguage)+"</td>");
   ...
   // DynamicValue av = DynamicValue.find(node_id,teasession._nLanguage);
    //out.print(av.getValue());
      ...
      out.print("<td width=1 >"+(++sum)+"</td>");//序号
     // out.print("<td>"+d_obj.getType()+"-"+node_id+"</A></td>");//编号
     // out.print("<td><A target=_blank href=/jsp/type/dynamicvalue/DynamicValueViewTab.jsp?node="+node_id+"&community="+community+" >"+node_obj.getSubject(teasession._nLanguage)+"</A></td><td>");
...
/**********************************************////////////循环动态类的内容******////
       ...
       while(de2.hasMoreElements())
       {
         ...
         if(dobj2.isQrc())
         {
            DynamicValue dvobj2 = DynamicValue.find(node_id,teasession._nLanguage,dobj2.getDynamictype());
             ...
             out.print("<td>"+ dvobj2.getValue()+"</td>");
 ...
 /********************************------------------********************************************/
        ...
        out.print("<td> <input type=button onclick=\" window.open('/jsp/type/dynamicvalue/DynamicValueViewTab.jsp?node="+node_id+"&community="+community+"','_blank');\" value=详细内容 ");
        out.print("<input type=button onclick=\" window.open('/jsp/confab/EditConfabDynamicValue.jsp?community="+community+"&Type="+type+"&node="+node_id+"&nexturl="+java.net.URLEncoder.encode(nexturl,"UTF-8")+"','_self'); \" value=编辑 > <input type='button' value="+r.getString(teasession._nLanguage, "CBDelete")+" onClick=\"if(confirm('"+r.getString(teasession._nLanguage, "ConfirmDeleteTree")+"')){window.open('/servlet/DeleteNode?node="+node_id+"&nexturl="+java.net.URLEncoder.encode(nexturl,"UTF-8")+"', '_self');this.disabled=true;}\" >");
