...
//{
  sql.append(" AND n.node IN (SELECT dv.node FROM DynamicType dt,DynamicValue dv WHERE dv.dynamictype=dt.dynamictype AND dt.type='code' AND dv.value IS NOT NULL AND dv.value!='' )");
/*}else
{
...
<body>
<h1><%//=r.getString(teasession._nLanguage, "")%><%=r.getString(teasession._nLanguage,"1215652235225")%></h1>
...
<tr><td><%=r.getString(teasession._nLanguage, "Code")%>:<input name="code" type="text">
  ...
  <img onClick="showCalendar('formform111.begin');"  align="absmiddle" src="/tea/image/public/Calendar2.gif" style="cursor:hand;" >--
  <input name="end" readonly="readonly" type="text" size="9">
  <img onClick="showCalendar('formform111.end');" align="absmiddle" style="cursor:hand;"  src="/tea/image/public/Calendar2.gif">
...
<table border="0" cellpadding="0" cellspacing="0" id="tablecenter">
  <TR id="tableonetr">
    ...
    <td><%=r.getString(teasession._nLanguage,"1215652471709")%></td>
    <td></td>
  </tr>
  <%
      DbAdapter dbadapter=new DbAdapter();
       try
       {
         dbadapter.executeQuery("SELECT DISTINCT n.node FROM Node n,NodeLayer nl WHERE n.hidden=0 AND n.type>=1024 AND n.node=nl.node AND n.community="+dbadapter.cite(community)+sql.toString());
         while(dbadapter.next())
         {
           ...
           Confabreception cr_obj=Confabreception.find(nodeobj._nNode,teasession._nLanguage);
...
if(code!=null)out.print(code);
    ...
    <td nowrap><%=cr_obj.getTime()==null?"否":"是"%></td>
    <td nowrap>

      <input type="button" value="<%=r.getString(teasession._nLanguage,"CBEdit")%>" onClick="window.open('/jsp/confab/EditConfabReception.jsp?node=<%=nodeobj._nNode%>&community=<%=community%>&code=<%=code%>&nexturl='+encodeURIComponent(self.location+''),'_self');"/>
