...
<h2>列表 ( <%=count%> )</h2>
     ...
     <td nowrap width="1">&nbsp;</td>
       <td>
         ...
         {
           out.print("<A href=?order=username"+param.toString()+pos+" >被授权许可方</a>");
         }
         %></TD>
         <TD nowrap>&nbsp;</TD>
       </tr>
...
for(int index=1;enumer.hasMoreElements();index++)
{
  ...
  <tr onMouseOver="javascript:this.bgColor='#BCD1E9'" onMouseOut="javascript:this.bgColor=''">
    ...
    <td><input type=button value=调看 onclick="window.open('/jsp/copyright/CrallowView.jsp?community=<%=teasession._strCommunity%>&crallow=<%=id%>','','height=500,width=700,status=no,toolbar=no,menubar=no,location=no');" >
        <input type=button value=编辑 onclick="window.open('/jsp/copyright/EditCrallow.jsp?community=<%=teasession._strCommunity%>&crallow=<%=id%>&nexturl='+encodeURIComponent(location),'_self');" >
        <input type=button value=删除 onclick="if(confirm('确定删除吗？'))window.open('/servlet/EditCopyRight?community=<%=teasession._strCommunity%>&crallow=<%=id%>&act=deletecrallow&nexturl='+encodeURIComponent(location),'_self');" >
    </td>
 </tr>
<%
}
...
<input type=button value=添加 onclick="window.open('/jsp/copyright/EditCrallow.jsp?community=<%=teasession._strCommunity%>&crallow=0&nexturl='+encodeURIComponent(location),'_self');" >
<input type=button value=导入 onclick="window.open('/jsp/copyright/CrImport.jsp?community=<%=teasession._strCommunity%>&act=crallow&nexturl='+encodeURIComponent(location),'_self');" >
