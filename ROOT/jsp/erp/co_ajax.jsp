...
<table border="0" cellpadding="0" cellspacing="0" id="tablecenter">
  ...
  {
    out.print("<tr onMouseOver=\"javascript:this.bgColor='#BCD1E9'\" onMouseOut=\"javascript:this.bgColor=''\"  onclick=f_xuanze_co(); title=点击表格可以添加商品  style=cursor:pointer><td colspan=12 align=center>暂无记录，请点击表格,来添加商品。</td></tr>");
  }
  for(int i = 1;e.hasMoreElements();i++)
  {
    ...
    <tr onMouseOver="javascript:this.bgColor='#BCD1E9'" onMouseOut="javascript:this.bgColor=''" >
      <td onclick="f_xuanze_co();" title="点击表格可以添加商品"  style=cursor:pointer><%=nobj.getNumber() %></td>
      <td onclick="f_xuanze_co();" title="点击表格可以添加商品"  style=cursor:pointer><%=nobj.getSubject(teasession._nLanguage)%></td>
      <td  onclick="f_xuanze_co();" title="点击表格可以添加商品"  style=cursor:pointer><%=g.getSpec(teasession._nLanguage) %></td>
      <td  onclick="f_xuanze_co();" title="点击表格可以添加商品"  style=cursor:pointer><%
      Brand b=null;
      ...
      %></td>
      <td  onclick="f_xuanze_co();" title="点击表格可以添加商品"  style=cursor:pointer><%=g.getMeasure(teasession._nLanguage)%></td>
      <!--选择的价格-->
       <input type="hidden" name="supply<%=i%>" value="<%out.print(gdobj.getSupply());%>" size="4" >
      <!--数量-->
      <td>  <input type="text" name="quantity<%=i%>" value="<%=gdobj.getQuantity()%>" size="4" onkeyup="f_quantity_co('<%=gdid%>','<%=i%>','<%=count%>');"></td>
        <!--折扣乘以数量的价格总计-->
       <td onclick="f_xuanze_co();" title="点击表格可以添加商品"  style=cursor:pointer>  <input type="text" name="total<%=i%>" value="<%=gdobj.getTotal()%>" size="4"  style="background:#cccccc" readonly="readonly">&nbsp;元</td>
       <td ><a href="#" onclick="f_delete_co('<%=gdid%>');">删除</a> </td>
    </tr>
    <%}if(count>0){%>
    <tr>
      <td colspan="4">&nbsp;</td>
