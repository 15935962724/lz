...
if("ccDetails".equals(act))
{
  ...
  int ssid = Integer.parseInt(teasession.getParameter("ssid"));;//半成品号

  SemiSupplier ssobj = SemiSupplier.find(ssid);
  SemiGoods sgobj = SemiGoods.find(ssobj.getSgid());

  String purchase = teasession.getParameter("purchase");//销售单编号
  if(SemiGoodsDetails.isGdid(teasession._strCommunity,ssid,purchase,type,purchase)>0)
  {
    out.print("您所选的商品在列表中已经存在！");
    return;
  }
  ...
  if(ssobj.getSupply()==null)
  {
    sup =new java.math.BigDecimal("0");
  }
  String total =sup.toString();

  SemiGoodsDetails.createjj(purchase,ssid,sup.toString(),1,total,"",teasession._strCommunity,type,0,0,0,ssobj.getSid(),purchase);
  return;
}
//删除一个商品
