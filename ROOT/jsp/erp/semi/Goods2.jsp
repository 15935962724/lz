...
if(teasession.getParameter("sid")!=null && teasession.getParameter("sid").length()>0)
{
  ...
  sql.append(" and sid=").append(sid);
  param.append("&sid=").append(sid);
}
...
count =SemiSupplier.count(teasession._strCommunity,sql.toString());
...
//{
//  sendx("/jsp/erp/semi/SemiGoods_ajax.jsp?goodstype2="+igd+"&act=goodstype&father="+form1.goodstype1.value,
//  function(data)
//  {
...
<table border="0" cellpadding="0" cellspacing="0" id="tablecenter">
  ...
  try
  {
    ...
    for(int i=0;eu.hasMoreElements();i++)
    {
      ...
      %>
      <tr onMouseOver="javascript:this.bgColor='#BCD1E9'" onMouseOut="javascript:this.bgColor=''" title="点击选择商品"  onclick="f_button('<%=ssid%>');"  style=cursor:pointer>
        <td nowrap="nowrap"><%=sgobj.getGoodsnumber()%></td>
        <td nowrap="nowrap"><%=sgobj.getSubject()%></td>
        <td nowrap="nowrap"><%=ssobj.getSpec()%></td>
        <td nowrap="nowrap"><%=ssobj.getMeasure()%></td>
        <td nowrap="nowrap"><%=ssobj.getSupply()%></td>
        <td nowrap="nowrap"><%=ssobj.getInfo()%></td>
      </tr>
      <%
      }
    }catch(Exception e)
