...
StringBuffer sql = new StringBuffer("and  p.type > 1");
...
count = GoodsDetails.countNode(teasession._strCommunity,"Paidinfull",sql.toString());
...
<h2>查询</h2>
  ...
  <input type="hidden" name="files" value="消费单商品销售情况统计">
  <input type="hidden" name="act" value="Sale2">
...
（列表数量为：&nbsp;<%=count%>&nbsp; ）显示列表如下：</h2>
  ...
  <table border="0" cellpadding="0" cellspacing="0" id="tablecenter">
    ...
    <% 
       java.util.Enumeration e = GoodsDetails.findPurNode(teasession._strCommunity,"Paidinfull",sql.toString(),pos,pageSize);// ICSalesList.findNo(sql.toString(),pos,pageSize);
    ...
    <td width="1"><%=i+pos%></td>
      ...
      <td><%=nobj.getSubject(teasession._nLanguage)%></td>
       ...
       %></td>
      <td><%=GoodsDetails.countQuantity(teasession._strCommunity,"Paidinfull","and gd.node ="+nid)%></td>
      <td ><%=GoodsDetails.countTotal(teasession._strCommunity,"Paidinfull","and gd.node ="+nid)%>&nbsp;元</td>
      <td align="center"><input type="button" value="查看详细" onclick="window.open('/jsp/erp/Sale2Show.jsp?nid=<%=nid%>&nexturl=<%=nexturl%>','_self');"/></td>
    </tr>
    <%} %>
    <%if(count>0) {%>
    <tr>
      ...
      <td align="right"><b>总计：</b></td>
       <td ><%=GoodsDetails.countQuantity(teasession._strCommunity,"Paidinfull",sql.toString())%></td>
       <td colspan="2" ><%=GoodsDetails.countTotal(teasession._strCommunity,"Paidinfull",sql.toString())%>&nbsp;元</td>
