...
StringBuffer sql=new StringBuffer(" AND quantity > 0");
...
if(supplname>0)
{
  ...
  if(lsobj.getBrands().split("/").length>0)
  {
...
count = Inventory.count2(teasession._strCommunity,sql.toString());
...
java.util.Enumeration e = Inventory.find2(teasession._strCommunity,sql.toString(),pos,pageSize);
...
while(e.hasMoreElements())
{
  ...
  <tr onMouseOver="javascript:this.bgColor='#BCD1E9'" onMouseOut="javascript:this.bgColor=''" title="点击选择商品"  onclick="f_button('<%=nodeid%>');"  style=cursor:pointer>
    <td width="1"><%=is %></td>
    ...
    <td nowrap><%if(bpobj.getSupply()!=null)out.print(bpobj.getSupply());%></td>
  </tr>
  <%
is++;
}
