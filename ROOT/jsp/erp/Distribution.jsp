...
count = Distribution.count(teasession._strCommunity,sql.toString());
...
function f_edit(igd)
{
  ...
  form1.action = "/jsp/erp/EditDistribution.jsp";
  form1.submit();
}
function f_delete(igd)
{
  if(confirm('您确定要删除此内容吗？')){
    ...
    form1.action = "/servlet/EditDistribution";
    form1.submit();
  }
}
//库房备货
...
if(type==1)
{out.print("审核配送单");}
else if(type==0){out.print("添加配送单");}
else if(type==2){out.print("配送单库房备货");}
else if(type==3){out.print("配送单库房发货");}
else if(type==4){out.print("配送单完成发货");}
%></h1>
...
<table border="0" cellpadding="0" cellspacing="0" id="tablecenter">
    <tr>
      ...
      <td>配送单号:&nbsp;<input type="text" name="dibidsssd" value="<%if(dibidsssd!=null)out.print(dibidsssd);%>"/></td>
      </tr>
      <tr>
        <td>加盟店名称:&nbsp;<input type="text" name="lsname" value="<%if(lsname!=null)out.print(lsname);%>"/></td>
        <td>配送仓库:<select name="waridname">
...
<%
java.util.Enumeration e = Distribution.find(teasession._strCommunity,sql.toString(),pos,pageSize);
...
while(e.hasMoreElements()){
  ...
  Distribution diobj =Distribution.find(dibids);
...
%>
  <tr onMouseOver="javascript:this.bgColor='#BCD1E9'" onMouseOut="javascript:this.bgColor=''">
    ...
    <td align="center"><%=diobj.getTotal() %></td>
       <%
    if(type==0)
    {
      out.print(" <td >");
    ...
    if(type==1)
    {
      out.print("<a href=\"#\" onclick=\"f_edit('"+dibids+"');\">审核配送单</a>&nbsp; ");
    }else if(type==0)
    {
...
<br />
<%if(type==0){ %>
<input type="button" value="添加配送单" onclick="window.open('/jsp/erp/EditDistribution.jsp?nexturl=<%=nexturl%>','_self');"/>
<%} %>
</form>
