...
if(teasession._rv != null)
{
  Profile profile = Profile.find(teasession._rv.toString());
   MemberId=profile.getMember();//用户名
   EnterPassword=profile.getPassword();//密码
   email = profile.getEmail();//电子邮件
   firstname=profile.getFirstName(teasession._nLanguage);//姓名
   sex=profile.isSex();//性别
   card = profile.getCard();// 身份证号
   birth=profile.getBirth();//生日
   city=profile.getCity(teasession._nLanguage);//省份
   address=profile.getAddress(teasession._nLanguage);//地址
   phonenumber=profile.getMobile();//手机号
   zip =profile.getZip(teasession._nLanguage);//邮政编码
   telephone=profile.getTelephone(teasession._nLanguage);//电话号码
   fax = profile.getFax(teasession._nLanguage);////传真号码
   position=profile.getTitle(teasession._nLanguage);//职称
   organization=profile.getOrganization(teasession._nLanguage);//单位
   section=profile.getSection(teasession._nLanguage);//科室
   degree = profile.getDegree(teasession._nLanguage);//学历
}
...
<h1> 普通会员修改信息</h1>
<form name="frmsrch" method="POST" action="/jsp/orth/EditPtyh.jsp" onSubmit="return f_edit(this);">
...
out.print("<td><input type=hidden name=MemberId value=");
if(MemberId!=null)
{
  out.print(MemberId);
}
