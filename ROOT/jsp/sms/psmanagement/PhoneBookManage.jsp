<%@page contentType="text/html;charset=UTF-8" %><%@ page import="tea.db.DbAdapter"%><%@ page import="tea.resource.Resource" %><%@ page import="tea.entity.node.*" %><%@ page import="tea.ui.TeaSession" %><% request.setCharacterEncoding("UTF-8");
...
int count=SMSPhoneBook.count(teasession._strCommunity,teasession._rv._strV,sql.toString());
...
<input type=hidden name="act" value="deletephonebook">
   ...
   <table border="0" cellpadding="0" cellspacing="0" id="tablecenter">
    ...
    enumer= SMSPhoneBook.find(teasession._strCommunity,teasession._rv._strV,sql.toString(),pos,10);
    while(enumer.hasMoreElements())
    {
      ...
      <tr onMouseOver="javascript:this.bgColor='#BCD1E9'" onMouseOut="javascript:this.bgColor=''">
        <td  nowrap width="1"><input type="CHECKBOX" name="id"  value="<%=smspb.getId()%>"></td>
          ...
          <td><input type="button" onclick="window.open('/jsp/sms/psmanagement/EditPhoneBook.jsp?community=<%=teasession._strCommunity%>&id=<%=smspb.getId()%>&nexturl='+encodeURIComponent(document.location),'_self');" value="<%=r.getString(teasession._nLanguage,"CBEdit")%>" ></td>
      </tr>
      <%
      }
      %>
...
<input type="button" onClick="window.location='/jsp/sms/psmanagement/EditPhoneBook.jsp?id=0&nexturl='+encodeURIComponent(document.location);" name="Submit" value="<%=r.getString(teasession._nLanguage,"Add")%>">
</form>
<br><br>
<div id="head6"><img height="6" src="about:blank"></div>
<br/>
