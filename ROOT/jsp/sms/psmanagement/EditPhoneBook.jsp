...
<form name=form1 method=post action="/servlet/EditContact" onSubmit="return submitText(this.name, '<%=r.getString(teasession._nLanguage, "InvalidSubject")%>')&&submitText(this.telephone,'<%=r.getString(teasession._nLanguage, "电话不正确")%>')&&submitText(this.mobile,'<%=r.getString(teasession._nLanguage, "手机不正确")%>')&&submitEmail(this.email,'<%=r.getString(teasession._nLanguage, "Email不正确")%>')&&submitText(this.gid, '<%=r.getString(teasession._nLanguage, "无效所属组")%>');">
<input type=hidden name="community" value="<%=teasession._strCommunity%>">
<input type=hidden name=id value="<%=id%>">
<input type=hidden name=nexturl value="<%=nexturl%>" >
<input type=hidden name="act" value="editphonebook">
