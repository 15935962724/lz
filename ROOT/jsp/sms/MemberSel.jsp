...
sql.append(" AND recycle=0 AND type=0");
...
<tr>
  <td>用户名:<input name="username" value="<%=username%>" size="14"/></td>
  <td>手机号:<input name="mobile" value="<%=mobile%>" size="14"/></td>
...
{
  Enumeration e=Profile.find(sql.toString(),pos,7);
...
<td colspan='2' align='right'><%="共"+sum+"条！"+new tea.htmlx.FPNL(h.language, par.toString(), pos, sum,7)%></td>
</tr>
<%
}%>
</table>
...
function f_ok()
{
  ...
  for(var i=0;i<arr.length;i++)
  {
    ...
    v+=t.value+'|';
    n+=t.alt+'；';
    h+="<span><input type='hidden' name='"+t.name+"' value='"+t.value+"'/>"+t.alt+"<img src='/tea/image/d7.gif' onclick='mt.del(this)' />　</span>";
  }
  pmt.receive(v,n,h);
  pmt.close()
}
