  ...
  function f_a1(igd,y,m)//年份的 加减
  {
    ...
    if(igd=='-')
    {
     ...
     var mm="?node=<%=teasession._nNode%>&community=<%=community%>&year="+addy+"&month="+m+"&nexturl=<%=nexturl%>&id=<%=id%>";
      sendx("/jsp/admin/flow/Cal_ajax.jsp"+mm,
      function(data)
      {
          ...
          for(int j=0;j<6;j++)
          {
            ...
            for(int   i=j*7;i<(j+1)*7;i++)
            {
              ...
              if(days[i].length()>0)
              {
                ...
                //  if(Node.isMotly(teasession._strCommunity,"  and path like '%/2207371/%' and mostly =1 and time >="+DbAdapter.cite(ds+" 00:00")+" and time <="+DbAdapter.cite(ds+" 23:59")+" "))
                //  {
                //    out.print("<span id =langse>");
                //    out.print(days[i]);
                //    out.print("</span>");
                 // }else   if(Node.isMotly(teasession._strCommunity,"  and path like '%/2207371/%' and mostly =0 and time >="+DbAdapter.cite(ds+" 00:00")+" and time <="+DbAdapter.cite(ds+" 23:59")+" "))
                 // {
