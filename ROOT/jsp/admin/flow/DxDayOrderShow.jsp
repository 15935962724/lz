  ...
  <META HTTP-EQUIV="Pragma" CONTENT="no-cache">
    <META HTTP-EQUIV="Cache-Control" CONTENT="no-cache">
      <META HTTP-EQUIV="Expires" CONTENT="0">
        ...
        {
          var mm="/jsp/admin/flow/DxDayOrderShow.jsp?community=<%=community%>&month="+document.sm.month.value+"&year="+<%=year%>;
          window.open(mm,"_self");
        }
        function changeYear()
        {
          var mm="/jsp/admin/flow/DxDayOrderShow.jsp?community=<%=community%>&month="+document.sm.month.value+"&year="+document.sm.YEAR.value;
          window.open(mm,"_self");
        }
        //--></script>
        </head>
...
<link href="/tea/ym/skin/simple/ymPrompt.css" rel="stylesheet" type="text/css">
        ...
        <body>
        <form   name="sm"   method="post"   action="/jsp/admin/flow/DxDayOrderShow.jsp?community=<%=community %>">
          <table   border="0"   width="168"   height="20" class="rili">
...
<td width=28%>&nbsp;</td>
<td width=28%><input   type=button   value="本月" onclick="location='/jsp/admin/flow/DxDayOrderShow.jsp?community=<%=community %>'"></td>
            </tr>
          </table>
              ...
              <tr>
                <%   for(int   i=j*7;i<(j+1)*7;i++)   { 
                  ...
                  <td    nowrap>
                    ...
                    if(days[i]!=null && days[i].length()>0)
                	{
                		out.println(  "<br>"+Entity.getNongli(Integer.parseInt(year),(Integer.parseInt(month)+1),Integer.parseInt(days[i])));
                	}
                  
	                    if(dobj.getAffaircontent()!=null && dobj.getAffaircontent().length()>0)
	                    {
	                    	 out.println(dobj.getAffaircontent());
	                    }
                    %>
                    </td>
