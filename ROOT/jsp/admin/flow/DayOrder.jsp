  ...
  <font   color="green">六</font></td>
              ...
              <tr>
                <%   for(int   i=j*7;i<(j+1)*7;i++)   {   %>
                  <td    nowrap>
                    <a   href="/jsp/admin/flow/Affair.jsp?community=<%=teasession._strCommunity%>&year=<%=year%>&month=<%=Integer.parseInt(month)+1%>&date=<%=days[i]%>"  ><%
                    ...
                    if(au_obj.isExists())
                    {
                      ...
                      java.util.Enumeration e = DayOrder.findByCommunityold(teasession._strCommunity,sql);
                      if(days[i].length()>0)
                      {
