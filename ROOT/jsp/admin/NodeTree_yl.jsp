...
private String tree_0(Node n,int lang,String target,int step)throws Exception
{
  ...
  StringBuilder h=new StringBuilder();
  h.append("<dl id=leftuptree"+step+" ><dt class='new_div'><img src='/tea/image/tree/tree_"+(type==0?"plus":"minus")+".gif' align=absmiddle id=img"+j);
  if(type==0)h.append(" onclick='f_c("+j+","+step+");'");
  h.append(" /><a href='/jsp/general/NodeLists_yl.jsp?node="+j+"' target='"+target+"'>"+n.getSubject(lang)+"</a></dt>");
  h.append("<dd class='tree' id='divid"+j+"' style='display:none'></dd></dl>");
  return h.toString();
}
%><%
