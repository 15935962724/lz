...
<table width="100%"   border="0" cellpadding="0" cellspacing="0" id="toutable"  style="<%if(picex!=null)out.print("background-image:url("+picex+")"); %>" >
        ...
        <tr>
          ...
          <td width="50%" class="tou" >
           <!-- <a href="<%=url%>" id="ToDesktop" target="m" ><%=r.getString(teasession._nLanguage,"ToDesktop")%></a>-->
            <a id="ToProspects" href="/servlet/Node?node=<%=c.getNode()%>" target="_top">公司网站</a><a id="cancels" href="/servlet/Logout?community=<%=teasession._strCommunity%>&node=<%=c.getNode()%>" target="_top">退出系统</a><a href="javascript:f_help();" id="ToHelp" >系统帮助</a>
          </td>
        </tr>
        <tr>
  ...
  <tr>
    <td valign="bottom" align="left" ><div id=gongnengcaidan><span id=qian></span><span id=zhong><%
    boolean flag=false;
...
//      if(af.getType()==0)
//      {
//        out.print("<a id='"+(flag?"folder":"currentfolder")+"' href='/jsp/admin/index_leftup.jsp?id="+afid+"&node="+teasession._nNode+"&community="+teasession._strCommunity+"' onfocus=this.blur() onclick=fchange(this) target=MenuFrame >"+af.getName(teasession._nLanguage)+"</a>");
//        flag=true;
...
%>
    ...
    if(aur.getUnit()!=0){
      if(!au.isHiddenorg()){
      %>
    <a id=person href="/jsp/user/list/OnlineMember.jsp?community=<%=teasession._strCommunity%>" onClick="fright(1);" target="RightFrame">在线人员</a><a  id=person href="/jsp/user/list/AllMembers.jsp?community=<%=teasession._strCommunity%>" onClick="fright(2);" target="RightFrame">全体人员</a>
    <%}} %>
</td>
  </tr>
</table>
