...
<script language="javascript" src="/res/Home/cssjs/1L1.js"></script>
<script>
...
<body onLoad="fload();" class="bodymember">
<table width="100%"  border="0" cellpadding="0" cellspacing="0" id="membertoutable"  style="<%if(picex!=null)out.print("background-image:url("+picex+")"); %>" >
  <tr >
    <td rowspan="2"  class="logotd">&nbsp;</td>
    <td colspan="3" align="left" valign="bottom">
      <table border="0" width="100%" cellpadding="0" cellspacing="0">
        <tr>
          <td rowspan="2" align="center" class="center">
           <%
          ...
          String rs[]=role.split("/"); 
            ...
            out.print("<SPAN ID=profileunit>"+(unit>0?AdminUnit.find(unit).getName():"")+"</SPAN>");
            out.print("<SPAN ID=strrid>"+teasession._rv._strR+"，您好！欢迎进入个人中心，今天是&nbsp;<script>var now=new Date();document.write(now.getYear()+'年'+(now.getMonth()+1)+'月'+now.getDate()+'日。');</script></span>");
          %></td>
          <td class="tou">        
            <a id="ToProspects" href="/jsp/admin/MemberDesktop.jsp" target="m"><%=r.getString(teasession._nLanguage,"会员中心首页") %></a>
          
           <!--  <a href="javascript:f_help();" id="ToHelp" ><%=r.getString(teasession._nLanguage,"5785493564") %></a>-->
           <a href ="http://<%=request.getServerName() %>" id="ToHelp" target="_blank" >回到网站首页</a>
             <a id="cancels" href="/servlet/Logout?community=<%=teasession._strCommunity%>&node=<%=c.getNode()%>" target="_top"><%=r.getString(teasession._nLanguage,"退出会员中心") %></a>
          </td>
        </tr>
    ...
    while(e.hasMoreElements())
    {
      ...
      if(popedom.indexOf("/"+j+"/")!=-1)
      {
        ...
        if(af.isHidden())
        {
          ...
          if(af.getType()==0)
          {
            out.print("<a id=folder href=/jsp/admin/index_leftupmember.jsp?id="+j+"&node="+teasession._nNode+"&community="+teasession._strCommunity+" onfocus=this.blur() onclick=fchange(this) target=MenuFrame >"+af.getName(teasession._nLanguage)+"</a>");
