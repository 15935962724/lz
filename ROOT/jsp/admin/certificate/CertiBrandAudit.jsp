...
StringBuffer sql=new StringBuffer(" and type="+type);
...
<html>
  ...
  function editimd(igd)
  {
    ...
    form1.action='/jsp/admin/certificate/EditCertiBrandAudit.jsp';
    form1.submit();
  }
  ...
  <div id="head6"><img height="6" src="about:blank"></div>
    ...
    <form name=form1 METHOD=post  action="?">
      ...
      <input type="hidden" name="nexturl" value="/jsp/admin/certificate/CertiBrandAudit.jsp?type=<%=type%>">
    ...
    <option value="0">----------</option>
      ...
      try {
        db
        .executeQuery("SELECT node FROM Node WHERE type>=1024 AND community="
        + db.cite(community)
        + " AND path LIKE "
        + db.cite("%/" + father + "/%")
        + " AND vcreator="
        + db.cite(teasession._rv._strV));
        while (db.next()) {
          ...
          if (d_obj.getSort() == sort) {
            ...
            out.print("  selected");
            out.print(">"
            + node_obj.getSubject(teasession._nLanguage));
            out.print("</option>");
          }
        }
      } finally {
...
<td width="1" align="center"><%=index%></td>
            ...
            <td align="center" ><input  type ="button"  value="审核" onClick="editimd('<%=certibrand%>');">            </td>
          </tr>
<%
index++;
	}
        ...
        %>
        <tr>
          <td colspan="5" align="center"><%=new tea.htmlx.FPNL(teasession._nLanguage, "?"
          + param.toString().replaceFirst("&pos=", "&")
          + "&pos=", pos, count, size)%></td>
          </tr>
