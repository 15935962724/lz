...
par.append("&community="+h.community+"&node="+h.node);
...
<input type="hidden" name="community" value="<%=h.community%>"/>
<input type="hidden" name="NewNode" value="ON" disabled="disabled"/>
...
}else
{
  ...
  for(int i=1+pos;e.hasMoreElements();i++)
  {
    ...
    Node nr=Node.find(nid);

    out.print("<tr onMouseOver=bgColor='#FFFFCA' onMouseOut=bgColor=''>");
    out.print("<td>"+i);
    out.print("<td>"+nr.getAnchor(h.language));
    out.print("<td>"+nr.getCreator()._strR);
    out.print("<td>"+MT.f(nr.getTime(),1));
  %>
    <td>
...
<input type="button" value="创建" onclick="f_act('add',<%=h.node%>)"/>
<input type="button" value="返回" onclick="window.open('/jsp/admin/nfast/Opinions.jsp?node=<%=Node.find(n.getFather()).getFather()%>','_self')"/>

<script>
mt.disabled(form2.docs);
form2.nexturl.value=location.pathname+location.search;
function f_act(a,id,v,c)
{
  ...
  {
    if(a=='add')form2.NewNode.disabled=false;
    form2.action="/jsp/type/report/EditReport.jsp";form2.method="get";form2.target="_self";form2.submit();
  }
}
</script>
</body>
