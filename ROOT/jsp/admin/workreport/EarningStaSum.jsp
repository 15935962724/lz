    ...
    <table border="0" cellpadding="0" cellspacing="0" id="tablecenter">
      <tr id="tableonetr">
        ...
        <tr onMouseOver="javascript:this.bgColor='#BCD1E9'" onMouseOut="javascript:this.bgColor=''">
          ...
          <td  title="<%=fobj.getMarketexplain(teasession._nLanguage)%>"><%=fobj.getMarketcost()%></td>
          <td ><%=Worklog.getMembercost(teasession._strCommunity,fid)%></td>
          <td  title="<%=fobj.getOtherexplain(teasession._nLanguage)%>"><%=fobj.getOtherexpenses() %></td>
        ...
        <tr>
          ...
          <td><input type="button" onclick="window.open('/jsp/admin/SumFlowitem.jsp?time_k=<%=time_c%>&time_j=<%=time_d%>','_self')"  value="返回"/></td>
        </tr>
        <%} %>
        <%if(count > pageSize){%>
