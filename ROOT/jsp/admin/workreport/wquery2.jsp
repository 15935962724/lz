...
StringBuffer sql = new StringBuffer(" AND type = 0 AND workproject="+workproject);
...
///客户
String flowitem=teasession.getParameter("flowitem");
if(flowitem!=null && flowitem.length()>0)
{
  sql.append(" AND flowitem IN (SELECT flowitem FROM FlowitemLayer WHERE name LIKE "+DbAdapter.cite("%"+flowitem+"%")+" ) ");
  param.append("&flowitem=").append(java.net.URLEncoder.encode(flowitem,"UTF-8"));
}




int count = Flowitem.count(teasession._strCommunity,sql.toString());
...
<input type="hidden" name="workproject" value="<%=workproject%>">
    ...
    <table border="0" cellpadding="0" cellspacing="0" id="tablecenter">
      <tr>
        <td>客户名称：</td>
        <td><input type="text" name="flowitem" value="<%if(flowitem!=null)out.print(flowitem);%>"> </td>
        <td>创建日期：</td>
        <td>
        从&nbsp;
    ...
    //  java.util.Enumeration e=Workproject.find(teasession._strCommunity,sql.toString(),pos,pageSize);
      java.util.Enumeration e=Flowitem.find(teasession._strCommunity,sql.toString());
      ...
      while(e.hasMoreElements())
      {
        ...
        %>
        <tr onMouseOver="javascript:this.bgColor='#BCD1E9'" onMouseOut="javascript:this.bgColor=''" onclick="f_xz('/<%=workproject%>/<%=flid%>/');">
          <td><%=flobj.getName(teasession._nLanguage)%></td>
          <td><%=flobj.getCtimeToString()%></td>
