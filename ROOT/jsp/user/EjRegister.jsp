...
if(request.getMethod().equals("POST"))
{
  if(teasession.getParameter("vertify").equals(session.getAttribute("sms.vertify")))
  {
    ...
    if(member==null)//创建
    {
      ...
      {
        out.print(new tea.html.Script("alert('对不起，会员ID已经存在。');history.back();"));
        return;
      }else
      {
        ...
        if(teasession._rv==null)
        {
          ...
          String conent=community_obj.getMailBefore(teasession._nLanguage)+"欢迎您注册为“三水吉屋”会员，我们会竭诚为您服务，请您登陆www.timade.com预定您的服务内容。业务咨询电话62358177。谢谢！"+community_obj.getMailAfter(teasession._nLanguage);
          ...
          {}
          out.print(new tea.html.Script("alert('注册成功（已登录）。');window.location='/servlet/Login?LoginType=0&LoginId="+member+"&Password="+password+"';"));
        }else
        {
          out.print(new tea.html.Script("alert('注册成功。');history.back();"));
        }
      }
    }else//编辑
    {
      ...
      if(!add)
      {
        out.print(new tea.html.Script("alert('编辑成功。');window.location='/servlet/Node?node="+tea.entity.site.Community.find(teasession._nNode).getWebName()+"';"));
      }
    }
    SClient sclient=SClient.find(community,member);
...
//      sclient.setPrice(new java.math.BigDecimal(price));
  ...
  {
    out.print(new tea.html.Script("alert('对不起，验证码错误。');history.back();"));
    return;
  }
}
