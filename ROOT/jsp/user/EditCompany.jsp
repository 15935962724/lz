...
java.util.Enumeration e1 = Node.find(" and hidden =0 and father= "+father,0,Integer.MAX_VALUE);
while(e1.hasMoreElements())
{
  int n1 = ((Integer)e1.nextElement()).intValue();
  Node nobj1 = Node.find(n1);

  s1.append(",new Array("+n1+",\""+nobj1.getSubject(teasession._nLanguage)+"\")");
  s2.append("var c"+n1+"=new Array(new Array(0,'----------')");
  java.util.Enumeration e2 = Node.find(" and type = 1 and hidden = 0 and father ="+n1,0,Integer.MAX_VALUE);

  while(e2.hasMoreElements())
  {
    int n2 = ((Integer)e2.nextElement()).intValue();
    Node nobj2 = Node.find(n2);
    s2.append(",new Array("+n2+",\""+nobj2.getSubject(teasession._nLanguage)+"\")");
  }
   s2.append(");\r\n");
}
  s1.append(");");
...
{
  var o1=form1.father1.options;
  var o2=form1.father2.options;
  while(o2.length>0)
  {
    o2[0]=null;
  }
    var cd=eval("c"+o1.value);
     if("c"+o1.value != 'c0'){
       for(var i=0;i<cd.length;i++)
       {
         o2[o2.length]=new Option(cd[i][1],cd[i][0]);
       }
     }else
      {
         o2[0]=new Option('---------',0);
...
%>
  ...
  %>
   <tr>
    ...
    <tr>
      <td id="log_tab_td01">所属行业</td><td id="log_tab_td02">*</td><td id="log_tab_td03"><div class="xlk sb">
        <select name="father1" onChange="f_load2();" id="xuanz">
          <option value="0">------</option>
        </select><div class="xlan"></div>

      </div>
        <div class="xlk sb"><select name="father2" id="xuanz" onChange="f_submit();" >
          <option value="0">------</option>
        </select><div class="xlan"></div></div><span id="sfather1">&nbsp;</span>
</td>
