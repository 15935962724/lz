...
sql.append(" AND ciocompany>0 and ciocompany in(select ciocompany from ciocompany where receipt=1) and ( cioclerkid is null  or cioclerkid=0) and ciopart not in(select ciopart from ciopart where shuttle=0 or shuttle is null)");
...
function f_actionA(act,ccid)
{
  switch(act)
  {
    ...
    break;
    case "edit":
    window.showModalDialog("/jsp/cio/CioClerklist.jsp?community=<%=teasession._strCommunity%>&ccid="+ccid,"","width:450,height:200,scrollbars:yes,toolbar:no,status:no,menubar:no,location:no,resizable:yes");location.reload();
...
for(int i=1;e.hasMoreElements();i++)
{
  ...
  out.print("<td id='qiye'>"+cp.getJob());
  out.print("<td id='daog'>"+cc.getName());
  out.print("<td id='lig'>"+cp.getComeTimeToString());
  out.print("<td id='caozuo'>"+cp.getBackTimeToString());
  out.print("<td id='bianji'>");
  if(cp.getComeTimeToString().equals(""))
  {
    out.print("<a id='hs' href=# title='无行程安排'>确认接送</a>");
  }else
  {
    out.print("<a href=javascript:f_actionA('edit',"+cpid+")>确认接送</a>");
  }
...
<input type="button" value="查看已安排接送的人员" onClick="window.open('/jsp/cio/CioMeetingyapj.jsp','_self')"/>
...
</select></td><td><input type="submit" id="tianjia" value="提交" ></td></tr>
