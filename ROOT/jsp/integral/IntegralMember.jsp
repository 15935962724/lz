 ...
 <td nowrap>会员编号</td>
  ...
  for(int index=1;prme.hasMoreElements();index++)
  {
    ...
    <td nowrap><%=probj.getCode() %></td>
      ...
      <td nowrap><%=probj.getEmail()%></td>
      <td><%=probj.getMyintegral()-in%></td>
      <td><input type="button" onClick="location='/jsp/integral/IntegralChangeAdminAdd.jsp?member=<%=URLEncoder.encode(member,"UTF-8")%>&nexturl=<%=nexturl%>'" value="申请"/></td>
