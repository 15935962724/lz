...
<ul>
<li><span class="sp1">账　户</span><input placeholder="用户名" name="username1" id="username1" class="texts lo1" type="text" /><input type="button"  id="button"   onclick="fnclick()" value="获取验证码"></li>
<li><span class="sp1">密　码</span><input placeholder="请输入密码" name="password" type="password" class="texts lo2" /></li>
<li><span class="sp1"><%=r.getString(h.language, "verificationcode")%></span><input placeholder="请输入验证码" style="text-transform: uppercase; width:122px;" class="texts" alt="<%=r.getString(h.language, "verificationcode")%>"  maxlength="4" name="verify" autocomplete="off" />
<!-- <a href="javascript:mt.verifys()"><img id="img_verifys" src="/Imgs.do?act=verify&amp;t=0.8841180045674784" alt="" style="visibility: hidden;position:relative;top:7px;left:3px;"></a> --></li>
...
<script>
	...
	function fnclick(){
		...
		mt.send("/Members.do?act=findusername&username="+username1,function(d)
				{
		  //alert(d);
					        if(d!='0')
					        {
