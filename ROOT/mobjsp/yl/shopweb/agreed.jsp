...
<%
    ...
    //根据用户查公司
    ArrayList<ProcurementUnitJoin> pujs = ProcurementUnitJoin.find(" AND profile=" + profile.profile, 0, Integer.MAX_VALUE);
...
<div class="body">
    ...
    <div class="Content">
        <form name="form2" id="form2" action="/Questions.do" method="get" target="_ajax" onsubmit="return mysupmit()">
            <input type="hidden" name="act" value="addAgreed">
            <input type="hidden" name="nexturl" value="<%=h.get("orderUrl")%>">

            <p align="center"><span style="font-size: 20px">中国同辐股份有限公司碘-125密封籽源</span></p>
            ...
            <p class="bor-b">
                ...
                <select name="company">
                    ...
                    <%
                        for (int i = 0; i < pujs.size(); i++) {
                            ProcurementUnitJoin puj = pujs.get(i);
                    %>
                    <option value="<%=puj.jid%>"><%=puj.getCompany()%>
                    </option>
                    <%}%>
                </select>
            </p>

            <p class="bor-b"><span class="span">公司地址</span><input type="text"  name="gsdz" value="" /></p>

            <p class="bor-b"><span class="span">传真</span><input type="text"  name="cz" value="" /></p>
