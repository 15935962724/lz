...
if(h.member<1)
{
  //response.sendRedirect("/xhtml/folder/14102033-1.htm?community="+h.community);
...
<link rel="stylesheet" href="/tea/dateDialog.css" type="text/css" />
...
<div id="xiangx">
<div class='xiangxi'>
<div class="h2"><h2>填写并核对订单信息</h2></div>
<!-- 收货人信息 -->
<form name="form1" action="/ShopOrderForms.do" method="post" onsubmit="return setCityName(this)">
	...
	<input type="hidden" name="nexturl" value="/mobjsp/yl/shopweb/ShopOrderForm.jsp"/>
	...
	<div class='f-tables' style='border:none;padding-bottom:0'><table id="nvoice_show" style="display: <%=soa.getMember()>0?"":"none" %>">
		...
		<tr><td>医院名称:</td><td><%=MT.f(soa.getHospital()) %></td></tr>
		<%} %>
...
<div class='f-tables'>
	<table id="nvoice_edit" style="display: <%=soa.getMember()>0?"none":"" %>">
		<tr><td align='right' nowrap><em>*</em>收货人：</td><td><input type="text" class='bors' name="consignees" value="<%=MT.f(soa.getConsignees()) %>" alt="收货人" /></td></tr>
	<%
		if(profile.membertype == 2){
			out.print("<tr><td align='right' nowrap><em>*</em>医院名称：</td><td>");
	...
	%>
		<tr><td align='right' nowrap>医院名称：</td><td>
			<input type="hidden" name="hospital" value="<%= (sqq.hospital_id==0?"无":ShopHospital.find(sqq.hospital_id).getName()) %>">
		 	科室：<input type="hidden" name="department" value="<%= (sqq.department==0?"无":ShopQualification.DEPARTMENT_ARR[sqq.department]) %>">
		</td></tr>
	<%
		}
	%>
		<tr><td align='right' nowrap><em>*</em>所在地区：</td><td>
		<script>mt.city('city0','city1','city2','<%= soa.getCity()%>')</script>
		</td></tr>
		<tr><td align='right' nowrap valign="top" style="line-height:25px;"><em>*</em>详细地址：</td><td><input type="text" style='width:100% !important' class='bors' name="address" value="<%=MT.f(soa.getAddress()) %>" alt="详细地址" /><br/><em>&nbsp;&nbsp;注：购买粒子，请填写医院详细地址。</em></td></tr>
		<tr><td align='right' nowrap><em>*</em>手机号：</td><td><input type="text" class='bors' name="mobile" value="<%=MT.f(soa.getMobile()) %>" alt="手机号" /> 或 固定电话：<input type="text" name="telphone" class='bors' value="<%=MT.f(soa.getTelphone()) %>" /></td></tr>
		<tr><td align='right' nowrap>邮编：</td><td><input class='bors' type="text" name="zipcode" value="<%=MT.f(soa.getZipcode()) %>" /></td></tr>
...
<div class="h3"><h3>支付方式：</h3></div>
	<table id='zhif'>
		<!-- <tr><td valign="top" nowrap ><input name="payType" type="radio" value="1" checked id="che1"/><label for="che1">在线支付：</label></td><td class='pl10'>即时到帐，支持绝大数银行借记卡及部分银行信用卡</td></tr> -->
		<tr><td nowrap valign="top"><input name="payType" type="radio" checked="checked" value="2" id="che2"/><label for="che2">公司转账：</label></td><td class='pl10'></td></tr>
		...
		<table width="94%" align="center">
			<tr>
				...
				<td style="padding:0px;width:100%;">
					<textarea  name="userRemarks" rows="6" cols="100" style="width:100%;float:left;border-top: 1px #aaaaaa solid;border-left: 1px #aaaaaa solid;border-right: 1px #f3f3f3 solid;-webkit-box-sizing:border-box;border-bottom: 1px #f3f3f3 solid;"></textarea>
<!-- 					<textarea name="userRemarks" rows="" cols=""></textarea>
 -->
	...
	<div class='f-tabless'>
		...
		<input type="hidden" name="nexturl" value="/xhtml/folder/14110391-1.htm" />
		...
		<div class='f-tables'>
		<h3>商品清单：</h3>
			<table  class="cart" cellpadding="0" cellspacing="0" width="94%" align="center">
			<!-- <tr>
			  <th>商品编号</th>
			  <th>商品图片</th>
			  <th>商品名称</th>
			...
			<%
				...
				/* if(ps.length<1){
			  		out.print("<tr><td colspan='5' align='center'>商品清单内暂时没有商品，您可以去<a href='javascript:void();' onclick='parent.location.reload();'>首页</a>挑选喜欢的商品，系统将自动为您跳转......<script>setTimeout(\"location='/';\",6000);</script></td></tr>");
				}else{
		  		...
		  		for(int i=1;i<ps.length;i++){
		    	    ...
		    	    if(p.isExist){
		    			  ...
		    			  for(int m=1;m<sarr.length;m++){
		    				  ...
		    				  //if(!isLiziClazz&&s1.category==ShopCategory.getCategory("lzCategory")){isLiziClazz=true;includeLzCar=true;break;}
		    				  spagePList.add(m,s1);
		    			  }
...
/* 			    		out.println("<td>");
		    		...
		    		out.println("</td>");
		    		out.println("<td class='price' ><strong>"+MT.f(price,2)+"</strong>×"+quantity+"</td>");
		    		out.println("</tr>");
		    		if(isLiziClazz){
			    	    ...
			    	    if(p.model_id>0){
			    	    	...
			    	    	if(activityStr.indexOf("-")!=-1){
			    	    		...
			    	    		caliBuff.append(" 粒子活度：<input alt='粒子活度' title='粒子活度范围："+activityStr+"' style='width:25px;' name='activity' value='' onkeyup=\"keyupActivity(this)\" onblur=\"blurActivity(this,"+activityMin+","+activityMax+")\" />");
			    	    	}else
			    	    		caliBuff.append(" 粒子活度：<input type='hidden' name='activity' value='"+activityStr+"' />"+activityStr);
			    	    }
			    	    //caliBuff.append(" 校准时间：<input alt='校准时间' id='teeDate' name='calidate_"+product_id+"' type='hidden'  readonly style='width:100px' class='mydate'/></span><span class='date' id='teeDateShow'>");
			    	    caliBuff.append(" 校准时间：<input alt='校准时间' id='teeDate' name='calidate' type='hidden'  readonly style='width:100px' class='mydate'/></span><span class='date' id='teeDateShow'>");
		    		...
		    		if(!p.isExist){
		    			for(int n=0;n<spagePList.size();n++){
		    		    	...
		    		    	out.println("<td>"+s1.getAnchorX(h.language)+"</td>");
		    		    	out.println("<td class='price'>"+MT.f(price,2)+"</td>");
		    		    	out.println("</tr>");
		    			}
		    		}
		    		out.print("<tr class='tr_bor'><td colspan='5'></td></tr>");
		  	    }
		  		isLzCategory = checkLength==1;
				//out.println("<tr><td><td colspan='4'>"+(includeLzCar?(isLzCategory?"粒子":"有粒子不唯一"):"不是粒子")+"</tr>");
			%>
			</table>
			<div class="dateDiv" isEmpty=true isshow=true>
			</div>
