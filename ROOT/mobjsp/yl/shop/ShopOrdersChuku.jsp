...
h.setCook("community", "Home", -1);
	/*Filex.logs("log_qywx/"+MT.f(new Date())+".log","h.member=="+h.member);*/
	int qywxMember = h.getInt("qywxMember");
	Filex.logs("log_qywx/"+MT.f(new Date())+".log","qywxMember=="+qywxMember);
	if(qywxMember>0){
	    h.member = qywxMember;
	}
	if(h.member<1){
	...
	if(param != null)
		url = url + "?" + param;
	Filex.logs("log_qywx/"+MT.f(new Date())+".log","url2=="+url);
  response.sendRedirect("/PhoneProjectReport.do?act=qy&agentid=1000006&community=Home&nexturl="+Http.enc(url));
  return;
}
...
<input type="hidden" name="qywxMember" value="<%=h.member%>">
