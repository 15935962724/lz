    ...
    public void service(HttpServletRequest request,HttpServletResponse response) throws ServletException,IOException
    {
        ...
        try
        {
            ...
            if((node.getOptions1() & 1) == 0)
            {
                ...
                {
                    out.print("<script>top.location.replace('/servlet/StartLogin?node=" + teasession._nNode + "');</script>");
                    return;
                }
            ...
            } else if("edit".equals(act))
            {
                ...
                {
                    out.print("<script>mt.show(\"" + r.getString(h.language,"1395304377843") + "\");</script>");
                    return;
                }
                String sv = (String) session.getAttribute("verify");
                if(sv == null)
                    sv = (String) session.getAttribute("sms.vertify");
                String vertify = h.get("vertify");
                if(vertify == null || !vertify.trim().equalsIgnoreCase(sv))
                {
                    out.print("<script>mt.show(\"" + r.getString(h.language,"1395304322796") + "\");</script>");
                    return;
                }
                ...
                if(node.getType() == 1)
                {
                    ...
                    teasession._nNode = Node.create(teasession._nNode,sequence,node.getCommunity(),teasession._rv,obj.getCategory(),(options1 & 2) != 0,node.getOptions(),options1,teasession._nLanguage,null,null,new java.util.Date(),0,0,0,0,null,teasession._nLanguage,subject,null,"",text,null,"",0,null,"","","","",null,null);
                } else
                {
                ...
                if(mail.length() > 0)
                {
                    ...
                    if(email != null)
                        sb.append("E-Mail: " + email + "<br/>");
                    if(sb.length() > 0)
                        sb.append("<hr>");
                    sb.append(text.replaceAll("\n","<br/>"));
                    sb.append("<br/><br/><a href='" + request.getHeader("Referer") + "' style='color:#999999'>source</a>");
                    Email.create(h.community,null,mail,subject,sb.toString(),true);
                }
