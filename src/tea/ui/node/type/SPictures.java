...
public class SPictures extends HttpServlet
{
        ...
        try
        {
            ...
            } else if("edit".equals(act)) //编辑
            {
                ...
                if(n.getType() == 1)
                {
                    ...
                    Category cat = Category.find(h.node); //108
                    h.node = Node.create(h.node,sequence,n.getCommunity(),new RV(h.username),cat.getCategory(),(options1 & 2) != 0,n.getOptions(),options1,h.language,null,null,new Date(),n.getStyle(),n.getRoot(),n.getKstyle(),n.getKroot(),null,h.language,subject,keywords,"",content,null,"",0,null,"","","","",null,null);
                    n.finished(h.node);
                } else
                {
                    n.set(h.language,subject,content);
                    n.setKeywords(keywords,h.language);
                }
                SPicture t = SPicture.find(h.node);
                t.specimen = h.getInt("specimen");
                ...
                if(nexturl.length() < 1)
                    nexturl = "/html/" + h.community + "/spicture/" + h.node + "-" + h.language + ".htm";
            }
            TeaServlet.delete(n);
