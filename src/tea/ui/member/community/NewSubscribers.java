package tea.ui.member.community;

import java.io.IOException;
//import java.io.PrintWriter;
import java.util.StringTokenizer;
import javax.servlet.*;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import tea.entity.RV;
import tea.entity.site.Subscriber;
//import tea.html.*;
//import tea.htmlx.Languages;
import tea.http.RequestHelper;
//import tea.resource.Resource;
import tea.ui.TeaServlet;
import tea.ui.TeaSession;

public class NewSubscribers extends TeaServlet
{

    public NewSubscribers()
    {
    }

    public void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
    {
        request.setCharacterEncoding("UTF-8");
        try
        {
            TeaSession teasession = new TeaSession(request);
            if (teasession._rv == null)
            {
                response.sendRedirect("/servlet/StartLogin?node=" + teasession._nNode);
                return;
            }
            String s = request.getParameter("Community");
            if (!teasession._rv.isWebMaster() && !teasession._rv.isOrganizer(s))
            {
                response.sendError(403);
                return;
            }
            if (request.getMethod().equals("GET"))
            {
                String nexturl = request.getParameter("nexturl");
                if (nexturl != null)
                {
                    nexturl = "&nexturl=" + nexturl;
                } else
                {
                    nexturl = "";
                }
                response.sendRedirect("/jsp/community/NewSubscribers.jsp?Community=" + s + nexturl);
                /*  Form form = new Form("foNew", "POST", "NewSubscribers");
                    form.setOnSubmit("return(submitText(this.Members,'" + super.r.getString(teasession._nLanguage, "InvalidMemberIds") + "'));");
                    form.add(new HiddenField("Community", s));
                    form.add(new Text(super.r.getString(teasession._nLanguage, "MemberId") + ":"));
                    form.add(new TextField("Members"));
                    form.add(new Button(super.r.getString(teasession._nLanguage, "Submit")));
                    PrintWriter printwriter = response.getWriter();
                    printwriter.print(form);
                    printwriter.print(new Script("document.foNew.Members.focus();"));
                    printwriter.print(new Languages(teasession._nLanguage, request));
                    printwriter.close();
                 */
            } else
            {
                String member[] = request.getParameterValues("Members");
                String act = request.getParameter("act");
                if (member != null)
                {
                    if ("Edit".equals(act))
                    {
                        int options = Integer.parseInt(request.getParameter("options"));
                        for (int index = 0; index < member.length; index++)
                        {
                            String cmember = request.getParameter(member[index]);
                            RV rv = new RV(member[index], cmember);
                            if (!rv.isExisted())
                            {
                                outText(teasession, response, RequestHelper.format(super.r.getString(teasession._nLanguage, "InvalidMember"), rv.toString()));
                                return;
                            }
                            Subscriber obj = Subscriber.find(s, rv);
                            if (obj.isExists())
                            {
                                obj.set(options);
                            } else
                            {
                                Subscriber.create(s, rv, options);
                            }
                        }
                    } else
                    {
                        String cmember = request.getParameter("cmember");
                        for (int index = 0; index < member.length; index++)
                        {
                            RV rv = new RV(member[index], cmember);
                            if (!rv.isExisted())
                            {
                                outText(teasession, response, RequestHelper.format(super.r.getString(teasession._nLanguage, "InvalidMember"), rv.toString()));
                                return;
                            }
                            Subscriber.create(s, rv, 1);
                        }
                    }
                }
                /*
                                 String s1;
                                 for (StringTokenizer stringtokenizer = new StringTokenizer(request.getParameter("Members"), " ,"); stringtokenizer.hasMoreTokens(); Subscriber.create(s, new RV(s1)))
                                 {
                    s1 = stringtokenizer.nextToken();
                    RV rv = new RV(s1);
                    if (!rv.isExisted())
                    {
                        outText(teasession,response, RequestHelper.format(super.r.getString(teasession._nLanguage, "InvalidMember"), s1));
                        return;
                    }
                                 }*/
                String nexturl = request.getParameter("nexturl");
                if (nexturl != null)
                {
                    response.sendRedirect(nexturl);
                } else
                {
                    response.sendRedirect("Subscribers?Community=" + s);
                }
            }
        } catch(Exception ex)
{
  ex.printStackTrace();
  response.sendError(500,ex.toString());
}
    }

    public void init(ServletConfig servletconfig) throws ServletException
    {
        super.init(servletconfig);
        super.r.add("tea/ui/member/community/NewSubscribers");
    }
}
