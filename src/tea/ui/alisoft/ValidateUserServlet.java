 ...
 *2008-10-22
 */
public class ValidateUserServlet extends javax.servlet.http.HttpServlet implements javax.servlet.Servlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		String sip_apiname = "alisoft.validateUser";//API名称
		...
		//System.out.println(userId+"||"+appId+"||"+appInstanceId+"||"+token+"||"+sign);
		//组织要提交的参数
		...
		RequestDispatcher rd = getServletContext().getRequestDispatcher("/jsp/alisoft/failure.jsp");
//		String url = "/jsp/info/failure.jsp?userId="+userId+"&appInstanceId="+appInstanceId+"&token="+token+"&message="+message+"&sip_appkey="+sip_appkey+"&sip_apiname="+sip_apiname+"&sip_timestamp="+sip_timestamp+"&sip_sign="+sign;
		if (result != null) {
			...
			if ("9999".equals(code)) {
                ...
                try
                {
                    if("1".equals(value))
                    {
                        //应用的订购者：公司信息是否初始化、个人信息是否初始化
                        if(!AliCompany.isExisted(appInstanceId))
                        {
							//initType 1:公司0:个人   role 1:管理员0:普通用户
                            rd = getServletContext().getRequestDispatcher("/jsp/alisoft/CompanyInit.jsp?initType=1&role=1");
                        } else
                        {
                            if(!Profile.isExisted("admin"+userId))
                            {
                                rd = getServletContext().getRequestDispatcher("/jsp/alisoft/CompanyInit.jsp?initType=0&role=1");
                            } else
                            {
								HttpSession session = request.getSession(true);
								OnlineList ol_obj = OnlineList.find(sip_sessionid); //添加到用户临时表中
								ol_obj.setMember("admin"+userId);
								Profile p = Profile.find("admin"+userId);
								RV rv = new RV("admin"+userId);
								session.setAttribute("tea.Community",appInstanceId);
								session.setAttribute("tea.RV",rv); //创建用户登陆 session
								session.setAttribute("LoginId","admin"+userId);
                    ...
                    if("0".equals(value))
                    {
                        ...
                        {
							value = "-5";
                            rd = getServletContext().getRequestDispatcher("/jsp/alisoft/failure.jsp");
                        } else
                        {
                            if(!Profile.isExisted("member"+userId))
                            {
                                rd = getServletContext().getRequestDispatcher("/jsp/alisoft/CompanyInit.jsp?initType=0&role=0");
                            }else{
								HttpSession session = request.getSession(true);

                                Profile p = Profile.find("member"+userId);
                                RV rv = new RV("member"+userId);
								OnlineList ol_obj = OnlineList.find(sip_sessionid); //添加到用户临时表中
								ol_obj.setMember("member"+userId);
                                session.setAttribute("tea.RV",rv); //创建用户登陆 session
                                session.setAttribute("LoginId","member"+userId);
                                session.setAttribute("password",p.getPassword());
		...
		request.setAttribute("appInstanceId", appInstanceId);
		request.setAttribute("token", token);
		request.setAttribute("message", message);
		request.setAttribute("sip_appkey", sip_appkey);
		request.setAttribute("sip_apiname", sip_apiname);
		request.setAttribute("sip_timestamp", sip_timestamp);
...
//		response.sendRedirect(url);
	...
	private String returnMessage(String code) {
		...
		switch (Integer.valueOf(code)) {
			...
			case -5: message="管理员未对系统信息进行初始化，暂时无法登陆"; break;
