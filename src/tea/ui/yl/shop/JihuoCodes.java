 ...
 * 收货人信息
 */
public class JihuoCodes extends HttpServlet {
    ...
    protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if ("asda".equals(act)) {
            ...
            try {
                ...
                int type_child=h.getInt("type_child");
                JihuoCode jihuoCode = JihuoCode.find(h.getInt("id"));
                if (h.getInt("id") != 0) {//修改
                    jihuoCode.setUsetype(jihuoCode.getUsetype());
                    if(h.get("code").equals(jihuoCode.getCode())){//激活码没变
                        jihuoCode.setMember(member);
                        jihuoCode.setCode(code);
                        jihuoCode.setMail(mail);
                        jihuoCode.setType(type);
                        jihuoCode.setType_child(type_child);
                        jihuoCode.set();
                    }else {//激活码改变了
                        List<JihuoCode> jihuoCodes = JihuoCode.find("AND code=" + code, 0, Integer.MAX_VALUE);
                        if (jihuoCodes.size() > 0) {//激活码重复
                            message = "激活码重复，修改失败";
                        } else {
                            jihuoCode.setMember(member);
                            jihuoCode.setCode(code);
                    ...
                    jihuoCode.setType(type);
                    jihuoCode.setType_child(type_child);
                    jihuoCode.setUsetype(0);
                    List<JihuoCode> jihuoCodes = JihuoCode.find("AND code=" + DbAdapter.cite(code), 0, Integer.MAX_VALUE);
                    if (jihuoCodes.size() > 0) {//激活码重复
                        message = "激活码重复，增加失败";
                    } else {
                        jihuoCode.set();
