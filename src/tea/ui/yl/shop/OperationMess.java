...
public class OperationMess extends HttpServlet {
    ...
    protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if("findPSList".equals(act)){
            try {
                ...
                int sum = OperationMes.count(sql.toString());
                sql.append(" order by time desc");
                List<OperationMes> psList = OperationMes.find(sql.toString(), pos, page_size);
                // sum = 0;
                //int sum = psList.size();
                ...
                if (sum > 0) {
                    ...
                    try {
                        ...
                        for (int i = 0; i < psList.size(); i++) {
                            OperationMes p = psList.get(i);
                            Profile p1 = Profile.find(p.getMember());

                            p.setProfile(p1.getMember());
                            p.setTimeStr(MT.f(p.getTime(),1));
