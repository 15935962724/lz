...
public class Educate extends Entity
{
    ...
    public static final String ME[] =
                                      {"哲学类", "经济学类", "法学类", "马克思主义理论类", "社会学类", "政治学类", "公安学类",
                                      "教育学类",
                                      "体育学类",
                                      "中国语言文学类",
                                      "外国语言文学类",
    ...
    public static int count(int node, int language) throws SQLException
{
    ...
    try
    {
        db.executeQuery("SELECT COUNT(*) FROM Educate WHERE node=" + node + " AND language=" + language);
        while (db.next())
        {
            i = db.getInt(1);
        }
	...
	public void set() throws SQLException
	{
		...
		if (this.isExists())
		{
			...
			try
			{
				...
				db.executeUpdate("UPDATE Educate SET	node =" + node + ", 	 language=" + language + ",starttime =" + db.cite(starttime) + "	,	stoptime =" + db.cite(stoptime) + ",	school=" + DbAdapter.cite(school) + ",	city =" + DbAdapter.cite(city) + ",	majorcategory =" + (majorCategory) + ",	majorname =" + DbAdapter.cite(majorName) + ",	degree =" + DbAdapter.cite(degree) + ",	content=" + DbAdapter.cite(content) + ",sland  =" + DbAdapter.cite(sland) + " ,anzkl  =" + anzkl + " ,anzeh  ="
						+ DbAdapter.cite(anzeh) + " ,zzjylx =" + (zzjylx ? "1" : "0") + " ,zzzymc2=" + DbAdapter.cite(zzzymc2) + ",zzxlbh =" + DbAdapter.cite(zzxlbh) + " ,zzxwsj =" + db.cite(zzxwsj) + " ,zzjgdz =" + DbAdapter.cite(zzjgdz) + " ,zzxxxs =" + DbAdapter.cite(zzxxxs) + ",zzzgxl=" + (zzzgxl ? "1" : "0") + ",zzzgxw=" + (zzzgxw ? "1" : "0") + ",zzbsh=" + (zzbsh ? "1" : "0") + ",zzxlbh2=" + DbAdapter.cite(zzxlbh2) + " WHERE  id=" + id);
			}  finally
			{
				db.close();
			}
	...
	public void create() throws SQLException
	{
		...
		try
		{
			...
			// DbAdapter.cite(zzxlbh2));
			db.executeUpdate("INSERT INTO Educate(node,starttime,stoptime,school,city,majorcategory,majorname,degree,content,language, sland, anzkl , anzeh , zzjylx , zzzymc2, zzxlbh, zzxwsj, zzjgdz, zzxxxs,zzzgxl,zzzgxw,zzbsh,zzxlbh2)VALUES(" + node + "," + db.cite(starttime) + "," + db.cite(stoptime) + "," + DbAdapter.cite(school) + "," + DbAdapter.cite(city) + "," + (majorCategory) + "," + DbAdapter.cite(majorName) + "," + DbAdapter.cite(degree) + "," + DbAdapter.cite(content) + ","
					+ language + ", " + DbAdapter.cite(sland) + "," + anzkl + " , " + DbAdapter.cite(anzeh) + " , " + (zzjylx ? "1" : "0") + " , " + DbAdapter.cite(zzzymc2) + ", " + DbAdapter.cite(zzxlbh) + ", " + db.cite(zzxwsj) + ", " + DbAdapter.cite(zzjgdz) + ", " + DbAdapter.cite(zzxxxs) + "," + (zzzgxl ? "1" : "0") + "," + (zzzgxw ? "1" : "0") + "," + (zzbsh ? "1" : "0") + "," + DbAdapter.cite(zzxlbh2) + ")");
		}  finally
		{
			db.close();
		}
    ...
    public String getZzxwsjToString()
    {
        if(zzxwsj==null)
            return "";
        return sdf.format(zzxwsj);
    }
