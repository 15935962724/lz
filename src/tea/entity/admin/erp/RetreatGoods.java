...
//退货单
public class RetreatGoods extends Entity
{
    private String purchase; //退货单
    ...
    private int type; //0 刚添加，1 等待审核 2 同意审核
    private boolean exists;

    public RetreatGoods(String purchase) throws SQLException
    {
        this.purchase = purchase;
        load();
    }

    public static RetreatGoods find(String purchase) throws SQLException
    {
        return new RetreatGoods(purchase);
    }
    ...
    public void load() throws SQLException
    {
        ...
        try
        {
            db.executeQuery("SELECT supplname,waridname,quantityarr,totalarr,quantity,total,time_s,member,community,remarks,rsgoods,type FROM RetreatGoods WHERE purchase =" + db.cite(purchase));
            if(db.next())
            {
    ...
    public static Enumeration find(String community,String sql,int pos,int size)
    {
        ...
        try
        {
            db.executeQuery("SELECT purchase FROM RetreatGoods WHERE community= " + db.cite(community) + sql);
    ...
    public void delete() throws SQLException
    {
        ...
        try
        {
            db.executeUpdate("DELETE FROM  RetreatGoods WHERE purchase = " + db.cite(purchase));
        } finally
        {
            db.close();
        }
	...
	public void setType(int type) throws SQLException
	{
	   ...
	   try
	   {
		   db.executeUpdate("UPDATE RetreatGoods SET type ="+type+" WHERE purchase = " + db.cite(purchase));
	   } finally
	   {
		   db.close();
	   }
    ...
    public static int count(String community,String sql) throws SQLException
    {
        ...
        try
        {
            db.executeQuery("SELECT COUNT(purchase) FROM RetreatGoods  WHERE community=" + db.cite(community) + sql);
            if(db.next())
            {
                count = db.getInt(1);
            }
