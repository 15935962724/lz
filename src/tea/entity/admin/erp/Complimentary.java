...
public class Complimentary extends Entity
{
	private String dibid;
	private int lsid; //加盟店ID
	private int warid; //仓库ID
	private Date time_s; //赠送日期
	...
	public Complimentary(String dibid)throws SQLException
	{
		this.dibid = dibid;
		load();
	}
	public static Complimentary find(String dibid)throws SQLException
	{
		return  new Complimentary(dibid);
	}
	...
	public void load() throws SQLException
	{
		...
		try
		{
			db.executeQuery(" SELECT lsid,warid,time_s,member,member2,telephone,address,remarks,community,quantity,total,type,city,address2,consignee,zip,telephone2,stime,ftime FROM Complimentary  WHERE dibid =" + db.cite(dibid));
			if(db.next())
			{
				lsid = db.getInt(1);
				warid = db.getInt(2);
	...
	public static void create(String dibid,int lsid,int warid,Date time_s,String member,String member2,String telephone,String address,String remarks,String community,int quantity,java.math.BigDecimal total,int type) throws SQLException
   {
	   ...
	   try
	   {
		   db.executeUpdate("INSERT INTO Complimentary (dibid,lsid,warid,time_s,member,member2,telephone,address,remarks," +
   ...
   public void set(int lsid,int warid,Date time_s,String member,String member2,String telephone,String address,String remarks,String community,int quantity,java.math.BigDecimal total,int type) throws SQLException
   {
	   ...
	   try
	   {
		   db.executeUpdate("UPDATE Complimentary SET lsid=" +lsid+ ",warid=" + warid + ",time_s=" + db.cite(time_s) + ",member=" + db.cite(member) + ",member2=" + db.cite(member2) + ",telephone=" + db.cite(telephone) + ",address=" + db.cite(address) + ",remarks=" + db.cite(remarks) + ",community=" + db.cite(community) + ",quantity="+quantity+",total ="+total+",type="+type+" WHERE dibid = " +
							db.cite(dibid));
	   } finally
	   {
		   db.close();
   ...
   public void set(String city,String address2,String consignee,String zip,String telephone2,Date stime,Date ftime)throws SQLException
   {
	  ...
	  try
	  {
		  db.executeUpdate("UPDATE Complimentary SET city="+db.cite(city)+",address2="+db.cite(address2)+",consignee="+db.cite(consignee)+",zip="+db.cite(zip)+",telephone2="+db.cite(telephone2)+",stime="+db.cite(stime)+",ftime="+db.cite(ftime)+"  WHERE dibid = " +
						   db.cite(dibid));
	  } finally
	  {
		  db.close();
   ...
   public static Enumeration find(String community,String sql,int pos,int size)
   {
	   ...
	   try
	   {
		   db.executeQuery("SELECT dibid FROM Complimentary WHERE community= " + db.cite(community) + sql,pos,size);
		   while(db.next())
		   {
   ...
   public void delete() throws SQLException
   {
	   ...
	   try
	   {
		   db.executeUpdate("DELETE FROM  Complimentary WHERE dibid = " + db.cite(dibid));
   ...
   public static int count(String community,String sql) throws SQLException
   {
	   ...
	   try
	   {
		   db.executeQuery("SELECT COUNT(dibid) FROM Complimentary  WHERE community=" + db.cite(community) + sql);
		   if(db.next())
		   {
			   count = db.getInt(1);
		   }
	...
	public void setType(int type) throws SQLException
	{
		...
		try
		{
			db.executeUpdate("UPDATE Complimentary SET type =" + type + " WHERE dibid = " + db.cite(dibid));
		} finally
		{
			db.close();
		}
