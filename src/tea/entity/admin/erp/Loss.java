...
public class Loss extends Entity
{
   ...
   public Loss(String  purchase) throws SQLException
   {
	   this.purchase = purchase;
	   load();
   }

   public static Loss find(String purchase) throws SQLException
   {
	   return new Loss(purchase);
   }
   ...
   public void load() throws SQLException
   {
	   ...
	   try
	   {
		   db.executeQuery("SELECT supplname,waridname,quantityarr,totalarr,quantity,total,time_s,member,community,remarks,rsgoods,type FROM Loss WHERE purchase =" + db.cite(purchase));
		   if(db.next())
		   {
   ...
   public static Enumeration find(String community,String sql,int pos,int size)
   {
	   ...
	   try
	   {
		   db.executeQuery("SELECT purchase FROM Loss WHERE community= " + db.cite(community) + sql);
   ...
   public void delete() throws SQLException
	{
		...
		try
		{
			db.executeUpdate("DELETE FROM  Loss WHERE purchase = " + db.cite(purchase));
		} finally
		{
			db.close();
		}
	...
	public static int count(String community,String sql) throws SQLException
	{
		...
		try
		{
			db.executeQuery("SELECT COUNT(purchase) FROM Loss  WHERE community=" + db.cite(community) + sql);
			if(db.next())
			{
				count = db.getInt(1);
			}
	...
	public void setType(int type) throws SQLException
	{
		...
		try
		{
			db.executeUpdate("UPDATE Loss SET type =" + type + " WHERE purchase = " + db.cite(purchase));
		} finally
		{
			db.close();
		}
